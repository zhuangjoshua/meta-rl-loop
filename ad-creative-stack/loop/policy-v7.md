...
Policy version: `meta-policy-v7`. Parent: `meta-policy-v6`.
...
When no compatible Meta history identifies the best acquisition configuration, create one
portfolio with exactly the four profiles below. A link-clicks profile may re-enter only as an explicitly
declared hypothesis card with recorded rationale. Every profile sends traffic to the same canonical
...
EXECUTION NOTE: the current production batch wrapper v1 contractually requires the
original five-profile shape; until a four-profile wrapper exists, batches under this
policy remain `blocked_runtime` for live parallel execution — the portfolio change is a
policy decision awaiting its execution surface, not permission to improvise with the
single-launch tool.
