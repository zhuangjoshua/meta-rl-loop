...
> **For Takyon:** Use test-driven-development for implementation. Use subagent-driven-development only after this plan is split into small reviewed tasks.

**Goal:** Add an opt-in Telegram DM multi-session mode where Telegram user-created private-chat topics become independent Takyon session lanes, while the root DM becomes a system lobby.

**Architecture:** Rely on Telegram's native private-chat topic UI. Users create new topics with the `+` button; Takyon maps each `message_thread_id` to a separate session lane. Takyon does not create topics for normal `/new` flow and does not try to manage topic lifecycle beyond activation/status, root-lobby behavior, and restoring legacy sessions into a user-created topic.

**Tech Stack:** Takyon gateway, Telegram Bot API 9.4+, python-telegram-bot adapter, SQLite SessionDB / side tables, pytest.
...
Create new Takyon chats with the + button in this bot interface. Each Telegram topic is an independent Takyon session, so you can work on different tasks in parallel.
...
To chat with Takyon, create a new topic using the + button in this bot interface. Each topic works as an independent Takyon session.
```

`/new` in root DM does not create a session/topic. Reply:

```text
To start a new parallel Takyon chat, create a new topic with the + button in this bot interface.

Each topic is an independent Takyon session. Use /new inside a topic only if you want to replace that topic's current session.
```
...
1. Takyon receives a Telegram DM message with `message_thread_id`.
2. Takyon derives the existing thread-aware `session_key` from `(platform=telegram, chat_type=dm, chat_id, thread_id)`.
3. If no binding exists, Takyon creates a fresh Takyon session for this topic lane and persists the binding.
...
Create new Takyon chats with the + button in this bot interface.
...
6. send two messages into the topic:
   ...
   - last Takyon assistant message if available.
...
- upgrading Takyon and starting the gateway must not create Telegram topic-mode tables or columns;
...
Stores Telegram topic/thread to Takyon session binding. Created only by `apply_telegram_topic_migration()`.
...
Normal Takyon agent flow for that topic's session lane.
...
5. restore emits confirmation and last Takyon assistant message.
...
```bash
python -m pytest tests/test_takyon_state.py -q
...
- User-created topics automatically become independent Takyon session lanes.
...
- `/topic <session_id>` inside a topic restores that session and sends confirmation + last Takyon assistant message.
