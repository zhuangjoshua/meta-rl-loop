# Takyon Agent - Development Guide
...
- `plugins/takyon/billing.py` and `plugins/takyon/control_api.py` are the control-plane Takyon user -> platform money rail.
- `plugins/takyon/app_usage.py` is the per-business product runtime usage rail for normal app/customer AI spend.
- Future fixed-price business creative/ad actions should use a separate business-scoped creative credit rail. The intended canonical home is a leaf like `plugins/takyon/business_credits.py`, not a skill-local ledger and not product subuser entitlements.
...
- Credit purchases should reuse the control-plane Stripe checkout/webhook pattern in `plugins/takyon/control_api.py`, not the product app checkout rails.
...
`$HOME/.takyon/takyon-agent/venv` (for worktrees that share a venv with the
main checkout).
...
├── toolsets.py           # Toolset definitions, _TAKYON_CORE_TOOLS list
├── cli.py                # TakyonCLI class — interactive CLI orchestrator (~11k LOC)
├── takyon_state.py       # SessionDB — SQLite session store (FTS5 search)
├── takyon_constants.py   # get_takyon_home(), display_takyon_home() — profile-aware paths
├── takyon_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # Agent internals (provider adapters, memory, caching, compression, etc.)
├── takyon_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
│   ├── takyon-achievements/  # Gamified achievement tracking
...
**User config:** `~/.takyon/config.yaml` (settings), `~/.takyon/.env` (API keys only).
**Logs:** `~/.takyon/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_takyon_home()`.
Browse with `takyon logs [--follow] [--level ...] [--session ...]`.
...
- **Skin engine** (`takyon_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `TakyonCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `agent/skill_commands.py` scans `~/.takyon/skills/`, injects as **user message** (not system prompt) to preserve prompt caching
...
- **Slack** — `slack_subcommand_map()` generates `/takyon` subcommand routing
- **Autocomplete** — `COMMANDS` flat dict feeds `SlashCommandCompleter`
- **CLI help** — `COMMANDS_BY_CATEGORY` dict feeds `show_help()`

### Adding a Slash Command

1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `takyon_cli/commands.py`:
```python
CommandDef("mycommand", "Description of what it does", "Session",
           aliases=("mc",), args_hint="[arg]"),
```
2. Add handler in `TakyonCLI.process_command()` in `cli.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `takyon --tui` or `TAKYON_TUI=1`.
...
npm run dev       # watch mode (rebuilds takyon-ink + tsx --watch)
npm start         # production
npm run build     # full build (takyon-ink + tsc)
...
### TUI in the Dashboard (`takyon dashboard` → `/chat`)
...
**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `takyon --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.
...
For most custom or local-only tools, do **not** edit Takyon core. Use the plugin
route instead: create `~/.takyon/plugins/<name>/plugin.yaml` and
`~/.takyon/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core Takyon tool that should ship in the base system.
...
**2. Add to `toolsets.py`** — either `_TAKYON_CORE_TOOLS` (all platforms) or a new toolset. **This step is required:** auto-discovery imports the tool and registers its schema, but the tool is only *exposed to an agent* if its name appears in a toolset. `_TAKYON_CORE_TOOLS` is not dead code — it's the default bundle every platform's base toolset inherits from.

Auto-discovery: any `tools/*.py` file with a top-level `registry.register()` call is imported automatically — no manual import list to maintain. Wiring into a toolset is still a deliberate, manual step.

The registry handles schema collection, dispatch, availability checking, and error wrapping. All handlers MUST return a JSON string.

**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_takyon_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `TAKYON_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_takyon_home()` for the base directory — never `Path.home() / ".takyon"`. This ensures each profile gets its own state.

**Agent-level tools** (todo, memory): intercepted by `run_agent.py` before `handle_function_call()`. See `tools/todo_tool.py` for the pattern.

### Paid-provider secret keys live behind TK (the safebox) — never `os.environ` in a business tool

Every secret that lets a business tool call a **paid** provider — model/image/video
APIs (Gemini, FAL, OpenAI `gpt-image`), search/extract (Tavily), social/ads
(Composio) — is held by the safebox (private secret-authority host
`67.205.158.170`), **not** in the business runtime. On the VPS the key is **not** in
the process env: `os.environ.get("GEMINI_API_KEY")` is **empty** there, so a tool
that reads its key from `os.environ` both breaks the secret boundary **and** fails
closed at runtime (observed: a logo rewrite that read `os.environ` raised
`GEMINI_API_KEY required` on prod and never worked).

Canonical pattern (`GOAL_RULES` §7) — see
`plugins/takyon/creative_gateway.py::_resolve_gemini_image_key`:

1. Resolve the key **only** in an authority route via
   `safebox.first_env_backed_value(*ALIASES)` (e.g. aliases
   `TAKYON_GEMINI_API_KEY` / `GEMINI_API_KEY` / `GOOGLE_API_KEY`). The business
   runtime never reads the raw key.
2. Pass it as an **explicit argument** to the provider client
   (`genai.Client(api_key=...)`), never via `os.environ`.
3. **Fail closed:** if the key is absent, return a clear `*_unconfigured` error /
   `503` **before** any credit reserve or provider call — never proceed or
   fabricate.
4. Register the alias in `core._API_ENV_ALIASES` and declare `requires_api=[...]`
   on the tool; gate the spend (creative-credit or usage rail) in the same
   authority route.

Any **new** paid-provider skill/tool resolves its key this way — same as logo
(`business_generate_logo`), UGC, static-ad, and web-search. Reading a provider key
from `os.environ` in a business tool is a bug: it breaks the secret boundary and
fails on the VPS.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `takyon_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `takyon_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `takyon tools`, `takyon setup`, most CLI subcommands | `takyon_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`takyon_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
takyon_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.takyon/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
- `default` — Classic Takyon gold/kawaii (the current look)
...
Add to `_BUILTIN_SKINS` dict in `takyon_cli/skin_engine.py`:
...
Users create `~/.takyon/skins/<name>.yaml`:
...
Takyon has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.takyon/plugins/` and pip-installed entry points.

### General plugins (`takyon_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.takyon/plugins/`, `./.takyon/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `takyon` at startup so
  `takyon <pluginname> <subcmd>` works with no change to `main.py`
...
`post_setup(takyon_home, config)` for setup-wizard integration.

**CLI commands via `plugins/memory/<name>/cli.py`:** if a memory plugin
defines `register_cli(subparser)`, `discover_plugin_cli_commands()` finds
it at argparse setup time and wires it into `takyon <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `takyon --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `takyon_cli/main.py`, etc.).
...
into `~/.takyon/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `takyon memory setup` / `post_setup()` without
...
2. User: `$TAKYON_HOME/plugins/model-providers/<name>/`
...
[`takyon-example-plugins`](https://github.com/NousResearch/takyon-example-plugins)
...
`metadata.takyon.tags`, `metadata.takyon.category`,
`metadata.takyon.related_skills`, `metadata.takyon.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.takyon.*` by the loader.
...
2. **Tools referenced in SKILL.md prose must be native Takyon tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Takyon Agent" is the secondary collaborator. If the
   contributor's commit shows "Takyon Agent" as author (because they
   used Takyon to draft the skill), replace it with their actual name
...
lives in the `takyon-agent-dev` skill at
...
`"messaging"`); `_TAKYON_CORE_TOOLS` is the default bundle most
...
Enable/disable per platform via `takyon tools` (the curses UI) or the
...
go to `~/.takyon/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `takyon_cli/curator.py` wires `takyon curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.takyon/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `takyon cron <verb>`
...
- File lock at `~/.takyon/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `takyon kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `takyon_cli/kanban.py` wires `takyon kanban` with verbs
...
- **Plugin assets:** `plugins/kanban/dashboard/` (web UI) +
  `plugins/kanban/systemd/` (`takyon-kanban-dispatcher.service` for
  standalone dispatcher deployment).

Isolation model:
- **Board** is the hard boundary — workers are spawned with
  `TAKYON_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
Takyon-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
in config.yaml (or `TAKYON_BACKGROUND_NOTIFICATIONS` env var):
...
Takyon supports **profiles** — multiple fully isolated instances, each with its own
`TAKYON_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `takyon_cli/main.py` sets
`TAKYON_HOME` before any module imports. All `get_takyon_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_takyon_home()` for all TAKYON_HOME paths.** Import from `takyon_constants`.
   NEVER hardcode `~/.takyon` or `Path.home() / ".takyon"` in code that reads/writes state.
   ```python
   # GOOD
   from takyon_constants import get_takyon_home
   config_path = get_takyon_home() / "config.yaml"

   # BAD — breaks profiles
   config_path = Path.home() / ".takyon" / "config.yaml"
   ```

2. **Use `display_takyon_home()` for user-facing messages.** Import from `takyon_constants`.
   This returns `~/.takyon` for default or `~/.takyon/profiles/<name>` for profiles.
   ```python
   # GOOD
   from takyon_constants import display_takyon_home
   print(f"Config saved to {display_takyon_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.takyon/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_takyon_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_takyon_home()`,
   not `Path.home() / ".takyon"`.

4. **Tests that mock `Path.home()` must also set `TAKYON_HOME`** — since code now uses
   `get_takyon_home()` (reads env var), not `Path.home() / ".takyon"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"TAKYON_HOME": str(tmp_path / ".takyon")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not TAKYON_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".takyon" / "profiles"`, NOT `get_takyon_home() / "profiles"`.
   This is intentional — it lets `takyon -p coder profile list` see all profiles regardless
   of which one is active.

## Known Pitfalls

### DO NOT hardcode `~/.takyon` paths
Use `get_takyon_home()` from `takyon_constants` for code paths. Use `display_takyon_home()`
for user-facing print/log messages. Hardcoding `~/.takyon` breaks profiles — each profile
has its own `TAKYON_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### DO NOT introduce new `simple_term_menu` usage
Existing call sites in `takyon_cli/main.py` remain for legacy fallback only;
the preferred UI is curses (stdlib) because `simple_term_menu` has
ghost-duplication rendering bugs in tmux/iTerm2 with arrow keys. New
interactive menus must use `takyon_cli/curses_ui.py` — see
`takyon_cli/tools_config.py` for the canonical pattern.
...
with actual imports (not mocks) against a temp `TAKYON_HOME`.

### Tests must not write to `~/.takyon/`
The `_isolate_takyon_home` autouse fixture in `tests/conftest.py` redirects `TAKYON_HOME` to a temp dir. Never hardcode `~/.takyon/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_takyon_home()` resolve within the temp dir.
Use the pattern from `tests/takyon_cli/test_profiles.py`:
```python
@pytest.fixture
...
| HOME / `~/.takyon/` | Your real config+auth.json | Temp dir per test |
