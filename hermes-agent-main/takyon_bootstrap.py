"""Windows UTF-8 bootstrap for Takyon entry points.
...
should be imported at the very top of every Takyon entry point
(``takyon``, ``takyon-agent``, ``takyon-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import takyon_bootstrap``
# (or ``from takyon_bootstrap import apply_windows_utf8_bootstrap``) at
