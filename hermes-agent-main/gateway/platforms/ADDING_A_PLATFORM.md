...
There are two ways to add a platform to the Takyon gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.takyon/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Takyon code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `takyon gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `takyon_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Takyon core.
...
And add it to the `takyon-gateway` composite:

```python
"takyon-gateway": {
    "includes": [..., "takyon-your-platform"]
}
```
...
## 12. Status Display (`takyon_cli/status.py`)
...
## 13. Gateway Setup Wizard (`takyon_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ takyon_cli/ toolsets.py \
  --include="*.py" -l | sort -u
