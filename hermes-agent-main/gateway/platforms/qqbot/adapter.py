...
class QQAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # to ~/.takyon/.update_response. Set here so the cross-adapter gateway
    ...
    async def _open_ws(self, gateway_url: str) -> None:
        ...
        # Honor WSL proxy env for QQ WebSocket. Takyon upgrades overwrite this
    ...
    async def _default_interaction_dispatch(
        ...
        - ``update_prompt:<answer>`` →
          writes the answer to ``~/.takyon/.update_response`` for the
          detached ``takyon update --gateway`` process to consume.
    ...
    def _write_update_response(answer: str, operator: str = "") -> None:
        ...
        ``takyon update --gateway`` watcher polls this file for a ``y``/``n``
    ...
    async def send_update_prompt(
        ...
        ``gateway/run.py``'s ``takyon update --gateway`` watcher. Button
        ...
        ``~/.takyon/.update_response`` so the detached update process
        can read it.
        """
