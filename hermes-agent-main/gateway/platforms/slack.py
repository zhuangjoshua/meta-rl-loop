...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        app_token = get_env_value("SLACK_APP_TOKEN")
        ...
        tokens_file = get_takyon_home() / "slack_tokens.json"
        ...
        try:
            ...
            # /model work directly without /takyon prefix). A single regex
            ...
            # manifest (see `takyon slack manifest`). In Socket Mode, Slack
            ...
            from takyon_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/takyon$")

            @self._app.command(_slash_pattern)
            async def handle_takyon_command(ack, command):
            ...
            for _action_id in (
                "takyon_approve_once",
                "takyon_approve_session",
                "takyon_approve_always",
                "takyon_deny",
            ):
            ...
            for _action_id in (
                "takyon_confirm_once",
                "takyon_confirm_always",
                "takyon_confirm_cancel",
            ):
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = f":thread: Takyon handoff — *{(name or 'session').strip()[:80]}*"
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Takyon session — matching the per-thread behavior channels already
    ...
    async def _handle_slack_message(self, event: dict) -> None:
        ...
        if original_text.startswith("!"):
            try:
                from takyon_cli.commands import is_gateway_known_command
                first_token = original_text[1:].split(maxsplit=1)[0]
                # Strip "@suffix" the same way get_command() does, so
                # forms like ``!stop@takyon`` still resolve.
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "takyon_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "takyon_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "takyon_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "takyon_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "takyon_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "takyon_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "takyon_confirm_once": "once",
            "takyon_confirm_always": "always",
            "takyon_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "takyon_approve_once": "once",
            "takyon_approve_session": "session",
            "takyon_approve_always": "always",
            "takyon_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/takyon <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/takyon
        ...
        if slash_name in {"takyon", ""}:
            # Legacy /takyon <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from takyon_cli.commands import slack_subcommand_map
            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/takyon   ``).
        ...
        # "Running /cmd…" ack shown by handle_takyon_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/takyon <question>" must produce public replies so
