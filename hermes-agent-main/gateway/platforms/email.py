"""
Email platform adapter for the Takyon gateway.

Allows users to interact with Takyon by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "takyon-agent" "version" "{_takyon_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Takyon Agent")
        ...
        msg_id = f"<takyon-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
