...
The PID file lives at ``{TAKYON_HOME}/gateway.pid``.  TAKYON_HOME defaults to
``~/.takyon`` but can be overridden via the environment variable.  This means
separate TAKYON_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting TAKYON_HOME."""
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("TAKYON_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "takyon" / _LOCKS_DIRNAME
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Takyon gateway."""
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different TAKYON_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``takyon.service`` + ``takyon-
# gateway.service``).
#
