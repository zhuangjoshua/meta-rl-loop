...
``{title, url, description, position}`` rows every other Takyon web
provider produces.
...
prefers Takyon-managed xAI Grok OAuth (via ``takyon auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.takyon/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``takyon tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `takyon auth` to sign in with "
