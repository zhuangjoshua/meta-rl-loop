...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/takyon>
...
```bash
takyon honcho setup    # full interactive wizard (cloud or local)
takyon memory setup    # generic picker, also works
```

Or manually:
```bash
takyon config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.takyon/.env
```
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$TAKYON_HOME/honcho.json` | Profile-local (isolated Takyon instances) |
| 2 | `~/.takyon/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Takyon profile: `takyon` (default) or `takyon.<profile>`.
...
| 4 | `per-session` strategy | Takyon session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`takyon-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`takyon`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-takyon-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening takyon in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Takyon session ID (timestamp + hex). Every `takyon` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Takyon profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`takyon`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Takyon profile: `takyon` (default) or `takyon.<profile>` (e.g. `takyon -p coder` → host key `takyon.coder`).
...
| `TAKYON_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `takyon honcho setup` | Full interactive setup wizard |
| `takyon honcho status` | Show resolved config for active profile |
| `takyon honcho enable` / `disable` | Toggle Honcho for active profile |
| `takyon honcho mode <mode>` | Change recall or observation mode |
| `takyon honcho peer --user <name>` | Update user peer name |
| `takyon honcho peer --ai <name>` | Update AI peer name |
| `takyon honcho tokens --context <N>` | Set context token budget |
| `takyon honcho tokens --dialectic <N>` | Set dialectic max chars |
| `takyon honcho map <name>` | Map current directory to a session name |
| `takyon honcho sync` | Create host blocks for all Takyon profiles |
