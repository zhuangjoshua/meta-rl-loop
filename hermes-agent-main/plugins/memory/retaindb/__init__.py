...
Config (env vars or takyon config.yaml under retaindb:):
...
class RetainDBMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        # Project resolution: RETAINDB_PROJECT > takyon-<profile> > "default"
        # If unset, the API auto-creates and uses the "default" project — no config required.
        explicit = os.environ.get("RETAINDB_PROJECT")
        if explicit:
            project = explicit
        else:
            takyon_home = str(kwargs.get("takyon_home", ""))
            profile_name = os.path.basename(takyon_home) if takyon_home else ""
            project = f"takyon-{profile_name}" if (profile_name and profile_name not in {"", ".takyon"}) else "default"

        self._client = _Client(api_key, base_url, project)
        self._session_id = session_id
        self._user_id = kwargs.get("user_id", "default") or "default"
        self._agent_id = kwargs.get("agent_id", "takyon") or "takyon"

        from takyon_constants import get_takyon_home
        takyon_home_path = get_takyon_home()
        db_path = takyon_home_path / "retaindb_queue.db"
        self._queue = _WriteQueue(self._client, db_path)

        # Seed agent identity from SOUL.md in background
        soul_path = takyon_home_path / "SOUL.md"
        if soul_path.exists():
