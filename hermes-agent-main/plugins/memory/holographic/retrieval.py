...
class FactRetriever:
    ...
    def search(
        ...
        # Encode the query vector once — it's constant across all candidates.
        query_vec = (
            hrr.encode_text(query, self.hrr_dim) if self.hrr_weight > 0 else None
        )
        scored = []
    ...
    def related(
        ...
        # Role vectors are query-independent constants — encode once.
        role_entity = hrr.encode_atom("__hrr_role_entity__", self.hrr_dim)
        role_content = hrr.encode_atom("__hrr_role_content__", self.hrr_dim)
        scored = []
        for row in rows:
