...
Config file: `$TAKYON_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `takyon` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `takyon-{identity}` → `takyon-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Takyon profile:

```json
{
  "container_tag": "takyon-{identity}"
}
```

For a profile named `coder`, this resolves to `takyon-coder`. The default profile resolves to `takyon-default`. Without `{identity}`, all profiles share the same container.
