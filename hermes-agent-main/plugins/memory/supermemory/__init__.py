...
def _load_supermemory_config(takyon_home: str) -> dict:
    config = _default_config()
    config_path = Path(takyon_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, takyon_home: str) -> None:
    config_path = Path(takyon_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `takyon memory setup`.
        # All other options are documented for $TAKYON_HOME/supermemory.json
    ...
    def save_config(self, values, takyon_home):
        ...
        _save_supermemory_config(sanitized, takyon_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from takyon_constants import get_takyon_home
        self._takyon_home = kwargs.get("takyon_home") or str(get_takyon_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._takyon_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "takyon", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "takyon_memory", "target": target, "type": "explicit_memory"},
