"""disk_cleanup — ephemeral file cleanup for Takyon Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under TAKYON_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly TAKYON_HOME and /tmp/takyon-*
Never touches: ~/.takyon/logs/ or any system directory.
"""
...
try:
    ...
    def get_takyon_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("TAKYON_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".takyon").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$TAKYON_HOME/logs/``."""
    return get_takyon_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$TAKYON_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under TAKYON_HOME or ``/tmp/takyon-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    takyon_home = get_takyon_home()
    try:
        path.resolve().relative_to(takyon_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/takyon-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("takyon-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside TAKYON_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under TAKYON_HOME (but leave TAKYON_HOME itself and
    ...
    empty_removed = 0
    try:
        for dirpath in sorted(takyon_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == takyon_home:
                continue
            try:
                rel_parts = dirpath.relative_to(takyon_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    takyon_home = get_takyon_home()
    try:
        rel = path.resolve().relative_to(takyon_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "takyon-agent",
        }:
            return None
        ...
        # Path isn't under TAKYON_HOME (e.g. /tmp/takyon-*) — fall through.
        pass
