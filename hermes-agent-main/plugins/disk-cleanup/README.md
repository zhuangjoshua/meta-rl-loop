...
Auto-tracks and cleans up ephemeral files created during Takyon Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$TAKYON_HOME` and `/tmp/takyon-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `TAKYON_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `TAKYON_HOME` or `/tmp/takyon-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$TAKYON_HOME/disk-cleanup/` is itself excluded
- `$TAKYON_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
