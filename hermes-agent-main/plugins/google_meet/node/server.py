...
Launched by ``takyon meet node run``.

Token handling
--------------
On first boot we mint 32 hex chars of entropy and persist them at
``$TAKYON_HOME/workspace/meetings/node_token.json``. Subsequent boots
...
via ``takyon meet node approve <name> <url> <token>``.
...
def _default_token_path() -> Path:
    return Path(get_takyon_home()) / "workspace" / "meetings" / "node_token.json"
