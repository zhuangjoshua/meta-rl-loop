...
session-scoped state file under ``$TAKYON_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $TAKYON_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_takyon_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this takyon install, leave it first —
    ...
    env = os.environ.copy()
    env["TAKYON_MEET_URL"] = url
    env["TAKYON_MEET_OUT_DIR"] = str(out)
    env["TAKYON_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["TAKYON_MEET_HEADED"] = "1"
    if auth_state:
        env["TAKYON_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["TAKYON_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if TAKYON_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["TAKYON_MEET_MODE"] = mode
    if realtime_model:
        env["TAKYON_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["TAKYON_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["TAKYON_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["TAKYON_MEET_REALTIME_KEY"] = realtime_api_key
