...
Let the takyon agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where takyon runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `takyon meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `takyon meet` CLI |
...
| `cli.py` | `takyon meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$TAKYON_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `takyon meet node {run,list,approve,remove,status,ping}` |
...
takyon meet install                                      # pip + Chromium
takyon meet setup                                        # preflight
takyon meet auth                                         # optional
takyon meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
takyon meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.takyon/.env
takyon meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
takyon meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
takyon meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.takyon/.env
takyon meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, takyon will **not** switch your system audio input automatically — the
...
takyon plugins enable google_meet
takyon meet node run --display-name my-mac --host 0.0.0.0 --port 18789
