...
``$TAKYON_HOME/workspace/meetings/<meeting-id>/``. The main takyon process
...
Run standalone for debugging::
    ...
    TAKYON_MEET_URL=https://meet.google.com/abc-defg-hij \\
    TAKYON_MEET_OUT_DIR=/tmp/meet-debug \\
    TAKYON_MEET_HEADED=1 \\
...
# Filenames the bot reads/writes in ``TAKYON_MEET_OUT_DIR``.
...
# and exposes ``window.__takyonMeetDrain()`` to pull new entries. This
# mirrors the OpenUtter caption scraping approach.
_CAPTION_OBSERVER_JS = r"""
(() => {
  if (window.__takyonMeetInstalled) return;
  window.__takyonMeetInstalled = true;
  window.__takyonMeetQueue = [];
  ...
  window.__takyonMeetDrain = () => {
    const out = window.__takyonMeetQueue.slice();
    window.__takyonMeetQueue = [];
    return out;
  };
})();
...
def _start_realtime_speaker(
    *,
    ...
    if platform_tag == "linux":
        ...
        sink = (bridge_info or {}).get("write_target") or "takyon_meet_sink"
        try:
            proc = _sp.Popen(
                [
                    "paplay",
...
def run_bot() -> int:  # noqa: C901 — orchestration, explicit branches
    url = os.environ.get("TAKYON_MEET_URL", "").strip()
    out_dir_env = os.environ.get("TAKYON_MEET_OUT_DIR", "").strip()
    headed = os.environ.get("TAKYON_MEET_HEADED", "").lower() in {"1", "true", "yes"}
    auth_state = os.environ.get("TAKYON_MEET_AUTH_STATE", "").strip()
    guest_name = os.environ.get("TAKYON_MEET_GUEST_NAME", "Takyon Agent")
    duration_s = _parse_duration(os.environ.get("TAKYON_MEET_DURATION", ""))
    # v2: optional realtime mode. Enabled when TAKYON_MEET_MODE=realtime.
    mode = os.environ.get("TAKYON_MEET_MODE", "transcribe").strip().lower()
    ...
    if not url or not _is_safe_meet_url(url):
        sys.stderr.write(
            "google_meet bot: refusing to launch — TAKYON_MEET_URL must be a "
            "meet.google.com URL. got: %r\n" % url
        )
        return 2
    if not out_dir_env:
        sys.stderr.write("google_meet bot: TAKYON_MEET_OUT_DIR is required\n")
        return 2
    ...
    if rt["enabled"]:
        if not realtime_api_key:
            state.set(error="realtime mode requested but no API key in TAKYON_MEET_REALTIME_KEY/OPENAI_API_KEY — falling back to transcribe")
            rt["enabled"] = False
        else:
            try:
    ...
    try:
        with sync_playwright() as pw:
            ...
            lobby_deadline = time.time() + float(
                os.environ.get("TAKYON_MEET_LOBBY_TIMEOUT", "300")
            )
            ...
            while not stop_flag["stop"]:
                ...
                try:
                    queued = page.evaluate("window.__takyonMeetDrain && window.__takyonMeetDrain()")
...
def _detect_admission(page) -> bool:
    ...
    (() => {
      ...
      if (window.__takyonMeetInstalled) {
