...
def _xai_user_agent() -> str:
    try:
        ...
        return takyon_xai_user_agent()
    except Exception:
        return "takyon-agent/video_gen"
...
class XAIVideoGenProvider(VideoGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        # hook (``takyon_cli/tools_config.py``) so the picker doesn't blindly
    ...
    async def _generate_async(
        ...
        *,
        ...
        if not api_key:
            return error_response(
                error=(
                    "No xAI credentials found. Sign in via `takyon auth add xai-oauth` "
