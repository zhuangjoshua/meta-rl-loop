...
def composition_from_dict(data: dict) -> PlanComposition:
    """Rebuild a `PlanComposition` from `composition_to_dict` output — and equally the ONE parser
    for the CEO tool's `composition` input (the `business_upsert_app_plan` schema mirrors the
    serialized shape exactly, by construction). This is the "adding a feature = adding a
    PricedComponent" surface: the caller passes priced components + a margin policy; price/budget/
    features are then DERIVED by `compose_plan`.

    Raises `InvalidComponent` / `InvalidMarginPolicy` (fail closed) on anything malformed — a bad
    composition never composes a guessed shape and never persists a broken plan. Validation on
    rebuild also rides the frozen dataclasses' __post_init__, so a tampered stored composition
    fails closed too.
    ...
    for raw in raw_components:
        ...
        if kind == "metered":
            ...
            raise InvalidComponent(
                f"unknown cost basis kind: {kind!r}; must be metered, fixed, or per_unit"
            )
        grants = raw.get("grants")
        if grants is not None and not isinstance(grants, dict):
            # Fail closed, never coerce: silently dropping malformed grants would silently drop
            # the features/model_allowlist a component was meant to turn on.
    ...
    if policy_data is not None and not isinstance(policy_data, dict):
        # Fail closed: a malformed policy must never silently become "no margin floor".
