    ...
    *,
    ...
    lines = [
        ...
        *(
        ...
        "- instruction: Use the pinned Vite scaffold as the runtime base. Keep shared wiring through `src/lib/takyon.ts` and `src/lib/hooks.ts`. Apply the COMPLETE pinned Taste skill in this same worker session: read the brief and assets; emit its one-line Design Read; set exact `DESIGN_VARIANCE`, `MOTION_INTENSITY`, and `VISUAL_DENSITY` values; choose the appropriate foundation; then persist that one-line Design Read, all three dial values, the selected foundation, and durable asset decisions in `DESIGN.md` at the product/site workspace root BEFORE editing the landing. Treat `product/site/DESIGN.md` as the canonical design source for every later product pass. Implement the landing, run build/typecheck, and complete Taste's full preflight. Then serve the built app locally and use the installed agent-browser + Chromium rail to render and READ actual screenshots at 1440x900 and 390x844. The header and hero must form a deliberate complete first viewport with the primary CTA visible and no accidental early next-section intrusion; fix and rerender until both desktop and mobile pass. Do not split Taste into a planner call and a later coding call. The route graph and required public/auth behavior are immutable; the section structure is not.",
