    ...
    *,
    ...
    lines = [
        ...
        *(
        ...
        "- If `Explicit product workflow requested: yes`, this SAME second pass owns the real workflow build, so also pass `effort: high`, `max_turns: 90`, `budget_usd: 25.0`, and `timeout_ms: 1800000`. For `model`: PASS NOTHING — ride the configured Claude agent default so the workflow build uses the runtime's chosen model; never pin or downgrade it here.",
        "",
