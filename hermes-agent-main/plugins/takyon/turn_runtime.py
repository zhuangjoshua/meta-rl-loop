...
def _business_bootstrap_instruction(
    ...
    *,
    ...
    lines = [
        ...
        "- If `Explicit product workflow requested: no`, ALSO pass `wait_ms: 0` — this FIRES the build on the worker plane and returns immediately with `detached: true` (status queued or running). That detached result is NOT a blocker and NOT a failure: keep the product milestone card running and move IMMEDIATELY on to step 3 (research) and step 4 (the launch post) while the build runs in parallel. You collect the finished build in step 5. Do NOT pass wait_ms on the workflow path — the workflow build must complete and be verified before anything else.",
        ...
        "",
        "If `Explicit product workflow requested: yes` (the blocking build): inspect the structured result from business_claude_agent_task, trusting only its exact success/blocker and surface_refresh publish status. If the product build or publish is blocked, record that exact blocker in research/strategy.md and stop bootstrap there. Do not paraphrase a different platform diagnosis and do not continue to X as if the product build completed.",
        "If `Explicit product workflow requested: no` (the fired build): the immediate `detached: true` response needs no inspection now — step 5 collects and judges it.",
        "",
        ...
        "",
        "### 5. Collect the app-shell build",
        "If `Explicit product workflow requested: yes`, the build already completed and was inspected in 2b — skip this step.",
        "If `Explicit product workflow requested: no`: re-call business_claude_agent_task with the IDENTICAL arguments you fired in 2b — the SAME workspace, instruction, guidance_skills, max_turns, effort, timeout_ms, refresh_surface, reason, actor, and the SAME idempotency_key (any argument drift is rejected as an idempotency conflict) — changing ONLY `wait_ms` to 600000. A `detached: true` result means it is still building: re-call once more the exact same way. Only an explicit blocker/error is a failure.",
        "When it returns, trust only its exact success/blocker and surface_refresh publish status. If it is blocked, record the exact blocker in research/strategy.md and stop there — the launch post is already live and points at the published landing, which remains correct and truthful; note in the final response that the post-sign-in polish did not complete, and do NOT attempt to retract or edit the X post.",
        "If it is still `queued` (never started) after the second wait, record a platform blocker (worker pickup failure) in research/strategy.md and stop.",
        "",
        "## Constraints",
