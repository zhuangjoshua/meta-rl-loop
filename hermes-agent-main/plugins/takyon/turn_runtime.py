    ...
    *,
    ...
    lines = [
        ...
        *(
        ...
        "- For that workflow-required path, call the action from `/app` through the shared `useDecodedActionRunner(name, value => decodeActionResult(value, decoder))` hook. Do not call legacy `useActionRunner`, `invokeAction`, or `createActionRunner` directly, use provider SDKs, provider env vars, direct provider URLs, mock outputs, fixtures, static canned AI responses, browser-only fake generation, localStorage as authority, or unsupported server routes.",
        "- Define one explicit JSON result schema for every action consumed by `/app`. Use identical field types in the generation prompt, action-boundary validator/normalizer, TypeScript decoder, and renderer. The decoder returns the normalized typed value or null/throws on mismatch; `decodeActionResult` tags it unambiguously, including boolean values. AppKit always renders a global `invalid_result` alert; also render the runner error contextually. Never silently discard a successful action payload.",
