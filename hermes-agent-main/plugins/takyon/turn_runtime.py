...
def _business_bootstrap_instruction(
    ...
    *,
    ...
    lines = [
        ...
        "- effort: high — the landing is the customer's first impression; full reasoning depth on this one design-pack-guided screen. The tight turn budget above (not a lower effort) is what keeps this pass fast.",
        "- timeout_ms: 900000 — a 15-minute ceiling so a wandering worker can never stall the customer's first paint for the full 20-minute default; the auto-escalation retry covers the rare genuine need. (Calibrated for high effort — turns think longer, so a tighter ceiling would guillotine healthy builds.)",
        "- model: PASS NOTHING — keep the default Sonnet model for the landing pass. The landing is the customer's first impression, so do NOT downgrade to a cheaper/faster model (e.g. Haiku) to shave time: landing QUALITY takes priority over speed, and a first paint in roughly five minutes is acceptable. The `max_turns: 24` lever above keeps this pass reasonably fast without lowering the model or the effort.",
        "",
        ...
        "- If `Explicit product workflow requested: no`, also pass `max_turns: 30`, `effort: high`, and `timeout_ms: 900000` — this pass restyles the two seeded access/account screens on EXISTING rails (no new architecture, no new routes), so the tight TURN budget is sufficient and keeps it fast at full reasoning depth; the runtime auto-escalates and retries once on a genuine turn-cap hit, exactly like 2a.",
