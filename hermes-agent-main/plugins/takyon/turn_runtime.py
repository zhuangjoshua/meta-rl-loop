    ...
    *,
    ...
    lines = [
        ...
        *(
        ...
        "Goal: get the customer a real, branded landing page live FAST. Derive the landing brief from the BUSINESS IDEA ALONE. Taste governs art direction above the selected design method and visual system — do NOT do any web research before the first landing publishes.",
        ...
        "- Before the coding task, inspect existing business/site assets. If the inferred direction genuinely needs an image and no truthful one exists, load taste-imagegen-web, then call business_generate_site_image with its art-directed, page-role-specific prompt and a stable slug; include every returned public_path in the coding instruction. A typography-led direction may use no generated image. Never generate filler or hotlink stock.",
        "- instruction: Use the pinned Vite scaffold as the runtime base. Keep shared wiring through `src/lib/takyon.ts` and `src/lib/hooks.ts`. Use Taste as the top-level art director: interpret the brief, adapt the selected lower design system to this business, and compose the landing freely around the real offer and supplied assets. Use the design system for concrete typography, tokens, spacing, and component craft without copying its house aesthetic verbatim. The route graph and required public/auth behavior are immutable; the section structure is not.",
        '- guidance_skills: pass exactly THREE — "taste-frontend" first, "claude-design" second, then ONE fitting visual system selected from the brief: "claude-design-stripe" (fintech / payments / B2B / commerce), "claude-design-openai" (AI / research / calm serious tools), "claude-design-doodle" (playful / casual / pet / kid-adjacent), "claude-design-superhuman" (premium productivity / speed / focus), or "claude-design-brutalist" (developer tools / security / infra / experimental technical brands). Taste sits above and adapts the selected system; it never replaces it. If no preset fits cleanly, pass only "taste-frontend" plus "claude-design" and let Taste establish the business-specific system.',
        ...
        *(
        ...
        '- guidance_skills: pass the SAME layered set used in 2a — "taste-frontend", "claude-design", and the same selected claude-design-* system when one was used. Taste remains the top layer. Reuse the landing token language and visual thesis, but design `/app` for task completion rather than copying marketing-page composition.',
