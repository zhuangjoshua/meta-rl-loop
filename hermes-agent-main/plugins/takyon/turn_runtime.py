    ...
    *,
    ...
    lines = [
        ...
        *(
        ...
        "- instruction: Use the pinned Vite scaffold as the runtime base. Keep shared wiring through `src/lib/takyon.ts` and `src/lib/hooks.ts`. Apply the COMPLETE pinned Taste skill in this same worker session: read the brief and assets; emit its one-line Design Read; set exact `DESIGN_VARIANCE`, `MOTION_INTENSITY`, and `VISUAL_DENSITY` values; choose the appropriate foundation; then persist that one-line Design Read, all three dial values, the selected foundation, and durable asset decisions in `DESIGN.md` at the product/site workspace root BEFORE editing the landing. Treat `product/site/DESIGN.md` as the canonical design source for every later product pass. Implement the landing, run build/typecheck, verify responsive breakpoint/collapse and reduced-motion behavior in the source, and finish every applicable item in Taste's full upstream preflight before returning. The SDK coding worker has no live browser and must not claim responsive visual inspection; live responsive browser QA belongs to the post-publish acceptance review. Do not split Taste into a planner call and a later coding call. The route graph and required public/auth behavior are immutable; the section structure is not.",
        ...
        *(
        ...
        '- guidance_skills: pass exactly ONE — "claude-design". Do NOT pass "taste-frontend" or a public-page style preset in 2b: upstream Taste explicitly excludes dashboards, data tables, and multi-step product UI. Use the product-design method for hierarchy, density, real states, and task completion while treating the landing tokens and assets as the fixed brand source of truth.',
