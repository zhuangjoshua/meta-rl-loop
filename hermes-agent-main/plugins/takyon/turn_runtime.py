    ...
    *,
    ...
    lines = [
        ...
        *(
        ...
        "- instruction: Use the pinned Vite scaffold as the runtime base and preserve shared wiring through `src/lib/takyon.ts` and `src/lib/hooks.ts`. Invoke the native `design-taste-frontend` skill and follow it from brief read through implementation and verification in this same worker session. The worker exposes Safebox-gated image generation and bounded rendered-viewport preflight tools; use them as the skill directs. A new landing must publish with real, used generated assets and a durable `DESIGN.md` handoff for later product work.",
        ...
        *(
            [
                "- The operator explicitly requested continuous landing animation. Tell the native Taste skill about that requirement and implement a product-relevant, reduced-motion-safe result without replacing the required generated assets.",
            ]
            if animations
            else []
        ),
        ...
        "- workspace: product/site",
        "- instruction: Use the pinned Vite scaffold, invoke the native `design-taste-frontend` skill, and read the durable `DESIGN.md`, landing source, tokens, and existing assets before editing. Honor the skill's own scope boundary: preserve its landing-page direction, but do not apply marketing-page layout rules to the multi-step `/app` product UI. Preserve the published landing direction and existing generated assets while building the real product UI; the available image tool is a capability, not a requirement to spend or regenerate assets on every pass. Keep shared runtime wiring through `src/lib/takyon.ts` and `src/lib/hooks.ts`.",
        ...
        "- Persist every created/generated customer artifact with `saveRecord({ data: normalizedResult, ... })`, load it with `listRecords(...)` or `useRecords(...)`, decode `record.data` through the same schema, and render persisted records after route/tab changes. Never keep the only copy of completed work in component state or an action result.",
