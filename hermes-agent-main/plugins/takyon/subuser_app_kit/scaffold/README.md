...
- **Record references:** `saveRecord(...)`, `listRecords(...)`, and `readRecord(...)` return each record with a canonical opaque `record.ref`. Pass that exact ref to `readRecord(ref)` (or `getRecord(ref)`), `saveRecord({ ref, data, ...fields })`, and `deleteRecord(ref)`; never derive or reuse a raw `id`/`record_id` from a title, route slug, form value, or response. `saveRecord` is an upsert, so `data` is required on create and update. Positional record reads and raw-ID updates are not part of the generated-app SDK. Record mutation/read responses retain the `{ record }` envelope and mirror that same record at the top level for compatibility; use one shape consistently.
