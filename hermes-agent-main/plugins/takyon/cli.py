...
def _business_bootstrap_instruction(
    ...
    *,
    ...
    lines = [
        ...
        "",
        "1. Read research/strategy.md and add to it so it becomes a short working strategy, not just the seeded goal.",
        "   Use current evidence to state the customer, problem, offer, why they care now, and the current X angle.",
        "2. Build the first real product/site page or app page people can actually see or use.",
        "3. Draft or publish one top-level X post.",
        "",
        ...
        "If a product feature is not wired to Hermes/Takyon rails, keep the customer surface normal and unavailable instead of inventing browser-only auth state, hardcoded users, fake checkout, fake billing, or customer-facing developer/debug framing.",
        "For the first app surface, seed the minimal monthly landing and access shell only: `product/site`, required routes `/` and `/app`, with `/app` focused on sign-in, subscription, and account access. Do not invent product workflow, extra tabs, or speculative routes unless the operator explicitly asked for them.",
