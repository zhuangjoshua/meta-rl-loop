...
def _business_bootstrap_instruction(
    ...
    *,
    ...
    lines = [
        ...
        "",
        "#### 2.0. Instant branded first-paint (a real branded landing live in ~2 minutes, BEFORE the design pass)",
        "The full design pass in 2a takes several minutes on the strong model. So FIRST publish a fast, deterministic, idea-branded landing the customer can see almost immediately — this is the customer's first paint. Two quick steps, no design model involved:",
        "- Call business_write_file with path `product/hero.json` and a JSON object drawn straight from the step-1 brief: {\"eyebrow\": <a 2-4 word kicker, e.g. the product category>, \"headline\": <the punchy one-line tagline/headline>, \"subhead\": <one truthful sentence on the core value proposition>}. Keep it TRUTHFUL — no fabricated stats, counts, or claims. The seeded landing renders this copy, so it is already branded to the business instead of a generic welcome.",
        "- Then PUBLISH the seeded-but-branded landing RIGHT NOW via business_claude_agent_task (the same tool that publishes in 2a — its refresh_surface path reliably builds the Vite `dist` and pushes it to the edge). Call business_claude_agent_task with: workspace `product/site`; refresh_surface: true; max_turns: 6; effort: low; idempotency_key a fresh unique value; and instruction = \"PUBLISH-ONLY pass. The seeded landing is ALREADY branded by the hero.json copy injected into surface-context — do NOT edit, redesign, or customize ANY file (no changes to landing.tsx or anything else). Make ZERO file edits. Your ONLY job: run the build green and PUBLISH the current seeded state so the customer's branded landing goes live immediately. The full custom design happens in the next pass.\" Pass NOTHING for `model` (default). Confirm the structured result's surface_refresh.publish.status == \"published\" with a real public_url — that is the customer's fast first paint, a real branded page in ~2-3 minutes, before the heavier 2a design pass.",
        "- This instant pass is best-effort and must never block bootstrap: a `detached: true` result is NOT a blocker (re-call with the SAME idempotency_key to collect the published result). If it returns a real blocker, record it in research/strategy.md and CONTINUE to 2a anyway (2a independently builds and publishes the landing). Do not stop bootstrap for the instant pass.",
        "",
        "#### 2a. Build and publish the polished landing page",
        "Now upgrade that fast first-paint into the full, polished, custom landing so the customer's site looks designed, not templated. Call business_claude_agent_task with:",
