...
def _business_bootstrap_instruction(
    ...
    *,
    ...
    lines = [
        ...
        "- Call business_write_file with path `product/instant_landing.json` and a JSON object from the step-1 brief: {\"eyebrow\": <2-4 word kicker, e.g. the product category>, \"headline\": <the punchy one-line tagline>, \"subhead\": <one truthful sentence on the core value proposition>, \"primary_cta\": \"Continue with Google\", \"features\": [{\"title\": <short>, \"body\": <one truthful sentence>}, ...2-4 truthful points...], \"accent\": <a brand-fitting hex like #0e7c66, or omit for the clean default>}. Keep it TRUTHFUL — no fabricated stats, counts, or claims. The publish step reads this and renders a real designed landing.tsx + themed tokens.css automatically (so the publish gate accepts it).",
