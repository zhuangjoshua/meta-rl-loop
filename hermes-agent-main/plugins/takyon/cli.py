...
def _business_bootstrap_instruction(
    ...
    *,
    ...
    lines = [
        ...
        "- Then call business_refresh_product_surface with source_path `product/site` to build and publish the seeded-but-branded landing RIGHT NOW. Confirm the result's publish status is `published` and a real `public_url` is returned. That is the customer's fast first paint — a real branded page within ~2 minutes, well before the heavier 2a design pass finishes.",
        "- This instant pass is best-effort and must never block bootstrap: if business_refresh_product_surface returns a real blocker, record it in research/strategy.md and CONTINUE to 2a anyway (2a independently builds and publishes the landing). Do not stop bootstrap for the instant pass.",
        "",
