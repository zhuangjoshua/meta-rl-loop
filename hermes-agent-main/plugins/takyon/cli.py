...
def _business_bootstrap_instruction(
    ...
    *,
    ...
    lines = [
        ...
        "- model: claude-haiku-4-5 — the landing is ONE well-specified, design-pack-guided screen, so use the faster Haiku model for THIS pass to bring the customer's first paint under 4 minutes. Pass NOTHING for `model` on the heavier 2b app-shell pass so it keeps the default Sonnet quality. If the published landing comes back visually broken or off-brand (not merely plainer), re-run 2a once WITHOUT `model` (Sonnet) before continuing.",
        "",
