...
def _business_bootstrap_instruction(
    ...
    *,
    ...
    lines = [
        ...
        "",
        "#### 2.0. Pin the idea-branded hero copy (so the FIRST published landing is already on-message)",
        "Before the design pass, capture the brand voice deterministically so the landing 2a builds is already on-message from its first paint. Call business_write_file with path `product/hero.json` and a JSON object drawn straight from the step-1 brief: {\"eyebrow\": <a 2-4 word kicker, e.g. the product category>, \"headline\": <the punchy one-line tagline/headline>, \"subhead\": <one truthful sentence on the core value proposition>}. Keep it TRUTHFUL — no fabricated stats, counts, or claims. The seeded landing renders this copy via surface-context, so the page is branded to the business instead of a generic welcome. (Do NOT try to publish the seeded scaffold directly here — the publish gate requires the real rewritten landing, which is the 2a pass below; 2a publishes the landing branded by this hero.json.)",
        "",
        "#### 2a. Build and publish the landing page",
        "Build the full, polished, custom landing — already on-message via the hero.json copy above — so the customer's site looks designed, not templated. Call business_claude_agent_task with:",
