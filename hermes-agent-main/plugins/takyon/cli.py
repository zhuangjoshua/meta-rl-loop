...
def _business_bootstrap_instruction(
    ...
    *,
    ...
    lines = [
        ...
        "The full design pass in 2a takes several minutes. So FIRST publish a real, deterministic, branded landing the customer sees almost immediately — this is the customer's first paint, no design model involved. TWO steps:",
        "- Call business_write_instant_landing with the step-1 brief: business; eyebrow (a 2-4 word kicker, e.g. the product category); headline (the punchy one-line tagline); subhead (one truthful sentence on the core value proposition); primary_cta (usually \"Continue with Google\"); features (2-4 {title, body} points drawn from the brief, truthful — no fabricated stats); accent (a brand-fitting hex color like #0e7c66, or omit for the clean default); idempotency_key. This writes a REAL, designed `landing.tsx` + a themed `tokens.css` (NOT the seeded scaffold), so the publish gate accepts it.",
        ...
        "- Best-effort, never blocks bootstrap: a `detached: true` result is NOT a blocker (re-call business_refresh_product_surface with the SAME idempotency_key to collect the published result). If it returns a real blocker, record it in research/strategy.md and CONTINUE to 2a anyway (2a independently builds and publishes the landing). Do not stop bootstrap for the instant pass.",
        "",
