...
def _business_bootstrap_instruction(slug: str, goal: str, active_mode: str) -> str:
    ...
    lines = [
        ...
        "Do a minimal existence check for current product/source/publish state instead of touring the whole workspace first.",
        "If relevant durable assets already exist, advance the missing highest-impact pieces instead of recreating them.",
        "",
        ...
        "Do bounded market research first: enough to know the ICP, problem, offer, tone, and safe public claims for the first surface.",
        "If enough signal already exists to ship the first honest surface, do not spend the create-time critical path on a full pulse/metrics pass first.",
        "Establish metrics/summary.md and refresh research/strategy.md after the first public surface lands, or immediately when product work is blocked and diagnosis is the real job.",
        ...
        "",
        "In this bootstrap turn, make visible durable progress where safe. After bounded market research is good enough to keep claims truthful,",
        "start product/site work immediately. If the operator explicitly asks for a first site/app publish,",
        "or the main blocker is that no truthful public product surface exists yet, ship that first honest product/site surface first.",
        "Do only the minimum research needed to keep that first surface truthful; do not hold first publication hostage to full",
        "ICP/channel/pricing writeups when the immediate blocker is missing source and publish state.",
        "Then use takyon-build-product to create or materially advance the smallest useful business-owned product/site surface, and",
        "backfill the next research artifacts as soon as the first surface is honest and visible.",
        ...
        "or when one or two high-confidence touches are obvious from current evidence.",
        "Do not treat a full multi-lane opening campaign batch as part of the first-site critical path.",
