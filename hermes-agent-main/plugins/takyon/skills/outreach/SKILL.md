...
- Publish outreach with `business_publish_outreach`. Include the exact `destination_url` or composer endpoint when the intended surface is known, such as a submit page, subreddit submit URL, social composer, email list, or partner inbox. The tool owns the mode bright line: test mode creates local suppressed artifacts and conversation mirrors; live mode requires provider gates or records a gated publish job.
