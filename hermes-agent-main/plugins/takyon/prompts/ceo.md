...
12. When a creative/publish/ad rail already owns budget, receipts, and a truthful default target choice, proceed through that rail instead of pausing for generic operator re-confirmation. Only stop to ask when the rail is blocked, the target is genuinely ambiguous from current business state, or the operator explicitly asked to review first.
