...
# ``model_tools`` import time, so ``takyon status``, ``takyon chat``, etc.
...
# ``takyon_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Takyon is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a takyon MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${TAKYON_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _takyon_home = _get_takyon_home()
        except (ModuleNotFoundError, ImportError):
            _takyon_home = _Path.home() / ".takyon"
        self._thread_count_store = _ThreadCountStore(
            _takyon_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Takyon is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("TAKYON_HOME", str(_Path.home() / ".takyon"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${TAKYON_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a takyon MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Takyon is thinking…' marker message.
        ...
        "Takyon is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Takyon is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Takyon is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Takyon is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Takyon is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``takyon setup``.

    The setup wizard at ``takyon_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.takyon/.env`` so the next
    gateway restart picks them up.
    """
    from takyon_cli.cli_output import (
        print_info,
        print_success,
        print_warning,
        prompt,
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. takyon-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.takyon/.env")
    print_info("Restart the gateway: takyon gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``takyon cron`` running as a
    separate process from ``takyon gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Takyon plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'takyon-agent[google_chat]'",
        ...
        # the "Takyon is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Takyon is thinking…' marker message "
