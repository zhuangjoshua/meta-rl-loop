"""
IRC Platform Adapter for Takyon Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Takyon agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "takyon-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Takyon Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Takyon Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: takyon_, takyon_1, takyon_2...
...
def check_requirements() -> bool:
    ...
    # takyon setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `takyon gateway setup` flow for the IRC platform.

    Lazy-imports ``takyon_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Takyon to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #takyon — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.takyon/.env")
    print_info("Restart the gateway for changes to take effect: takyon gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "takyon-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "takyon-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Takyon Agent (cron)")
