"""
LINE Messaging API platform adapter for Takyon Agent.
...
to Takyon Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.channel_access_token = (
            get_env_value("LINE_CHANNEL_ACCESS_TOKEN")
            or extra.get("channel_access_token", "")
        )
        self.channel_secret = (
            get_env_value("LINE_CHANNEL_SECRET")
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``TAKYON_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            takyon_home = Path.home().joinpath(".takyon").resolve()
...
def check_requirements() -> bool:
    ...
    if not get_env_value("LINE_CHANNEL_ACCESS_TOKEN"):
        return False
    if not get_env_value("LINE_CHANNEL_SECRET"):
        return False
    try:
...
def validate_config(config) -> bool:
    extra = getattr(config, "extra", {}) or {}
    has_token = bool(
        get_env_value("LINE_CHANNEL_ACCESS_TOKEN") or extra.get("channel_access_token")
    )
    has_secret = bool(
        get_env_value("LINE_CHANNEL_SECRET") or extra.get("channel_secret")
    )
    return has_token and has_secret


def is_connected(config) -> bool:
    """Surface in ``takyon status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``takyon status`` reflect a LINE configuration that lives entirely
    in ``.env`` without a ``platforms.line`` block in ``config.yaml``.
    Mirrors the IRC plugin's pattern.
    """
    if not (get_env_value("LINE_CHANNEL_ACCESS_TOKEN") and get_env_value("LINE_CHANNEL_SECRET")):
        return None
    seeded: Dict[str, Any] = {}
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``takyon setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.takyon/.env`` via ``takyon_cli.config``.
    """
    print()
    ...
    try:
        from takyon_cli.config import get_env_var, set_env_var
    except ImportError:
        print("takyon_cli.config not available; set LINE_* vars manually in ~/.takyon/.env")
        return
