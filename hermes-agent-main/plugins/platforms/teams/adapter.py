"""
Microsoft Teams platform adapter for Takyon Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Takyon"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        takyon_action = data.get("takyon_action", "")
        session_key = data.get("session_key", "")

        if not takyon_action or not session_key:
        ...
        choice = choice_map.get(takyon_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="takyon_approve",
                    data={**btn_data_base, "takyon_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="takyon_approve",
                    data={**btn_data_base, "takyon_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="takyon_approve",
                    data={**btn_data_base, "takyon_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="takyon_approve",
                    data={**btn_data_base, "takyon_action": "deny"},
                    style="destructive",
                ),
            ])
        )
    ...
    async def send_image(
        ...
        try:
            ...
            if image_url.startswith("http://") or image_url.startswith("https://"):
                ...
                raw_bytes = await asyncio.to_thread(_read_image_bytes)
                content_url = f"data:{mime_type};base64,{base64.b64encode(raw_bytes).decode()}"
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Takyon\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.takyon/.env")
