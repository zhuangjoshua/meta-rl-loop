"""SimpleX Chat platform adapter (Takyon plugin).
...
This adapter ships as a Takyon platform plugin under
``plugins/platforms/simplex/``. The Takyon plugin loader scans the
...
discoverable and `takyon setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Takyon — no
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"takyon-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``takyon setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / home channel.
    Writes to ``~/.takyon/.env`` via ``takyon_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("takyon_cli.config not available; set SIMPLEX_* vars manually in ~/.takyon/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `takyon gateway status` without
