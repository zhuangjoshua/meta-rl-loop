# Takyon Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: takyon-achievements plugin
...
### `GET /api/plugins/takyon-achievements/achievements`
...
### `POST /api/plugins/takyon-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/takyon-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/takyon-achievements/overview`
...
- Persist plugin data under:
  - `~/.takyon/plugins/takyon-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.takyon/plugins/takyon-achievements/scan_checkpoint.json`
...
- `~/.takyon/plugins/takyon-achievements/`
