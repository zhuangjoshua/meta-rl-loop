"""Shared fixtures for the takyon-agent test suite.
...
2. **Isolated TAKYON_HOME.** TAKYON_HOME points to a per-test tempdir so
   code reading ``~/.takyon/*`` via ``get_takyon_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".takyon"`` instead of the canonical
   ``get_takyon_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No TAKYON_SESSION_* inheritance** — the agent's current gateway
...
def _normalized_host_role() -> str:
    ...
    # (core/cli) into every test session at collection time. environment.py is a stdlib-only
    # leaf, so a direct file load is cheap and dependency-free.
    import importlib.util

    mod = sys.modules.get("plugins.takyon.environment")
    if mod is None:
        path = Path(__file__).resolve().parents[1] / "plugins" / "takyon" / "environment.py"
...
# TAKYON_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_TAKYON_BEHAVIORAL_VARS = frozenset({
    "TAKYON_YOLO_MODE",
    "TAKYON_INTERACTIVE",
    "TAKYON_QUIET",
    ...
    "TAKYON_SESSION_CHAT_ID",
    "TAKYON_SESSION_CHAT_NAME",
    "TAKYON_SESSION_THREAD_ID",
    "TAKYON_SESSION_SOURCE",
    "TAKYON_SESSION_KEY",
    "TAKYON_GATEWAY_SESSION",
    "TAKYON_PLATFORM",
    "TAKYON_MODEL",
    "TAKYON_INFERENCE_MODEL",
    "TAKYON_INFERENCE_PROVIDER",
    "TAKYON_TUI_PROVIDER",
    "TAKYON_MANAGED",
    "TAKYON_DEV",
    "TAKYON_CONTAINER",
    ...
    "TAKYON_TIMEZONE",
    "TAKYON_REDACT_SECRETS",
    "TAKYON_BACKGROUND_NOTIFICATIONS",
    "TAKYON_EXEC_ASK",
    "TAKYON_HOME_MODE",
    ...
    "TAKYON_KANBAN_BOARD",
    "TAKYON_KANBAN_HOME",
    "TAKYON_KANBAN_WORKSPACES_ROOT",
    "TAKYON_KANBAN_LOGS_ROOT",
    "TAKYON_KANBAN_TASK",
    "TAKYON_KANBAN_WORKSPACE",
    "TAKYON_KANBAN_RUN_ID",
    "TAKYON_KANBAN_CLAIM_LOCK",
    "TAKYON_KANBAN_DISPATCH_IN_GATEWAY",
    "TAKYON_TENANT",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and TAKYON_HOME to per-test tempdirs so code that
    reads ``~/.takyon/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral TAKYON_* vars that could change test semantics.
    for name in _TAKYON_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect TAKYON_HOME to a per-test tempdir. Code that reads
    #    ``~/.takyon/*`` via ``get_takyon_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.takyon/*`` via
    #    ``Path.home() / ".takyon"`` instead of ``get_takyon_home()``
    #    is a bug to fix at the callsite.
    fake_takyon_home = tmp_path / "takyon_test"
    fake_takyon_home.mkdir()
    (fake_takyon_home / "sessions").mkdir()
    (fake_takyon_home / "cron").mkdir()
    (fake_takyon_home / "memories").mkdir()
    (fake_takyon_home / "skills").mkdir()
    monkeypatch.setenv("TAKYON_HOME", str(fake_takyon_home))
    ...
    #    ~/.takyon/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import takyon_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_takyon_home(_hermetic_environment):
...
def _reset_module_state():
    ...
    for _logger_name in ("tools", "run_agent", "trajectory_compressor", "cron", "takyon_cli"):
...
@pytest.fixture()
def mock_config():
    """Return a minimal takyon config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``takyon-gateway`` process
...
# correlated with a ``tests/takyon_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> takyon-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart takyon-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting takyon/python patterns are also blocked.
    """
    ...
    def _matches_takyon_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _TAKYON_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_takyon_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch takyon-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m takyon_cli.main".
                if (
                    "takyon" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live takyon-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting takyon/python could hit the live gateway. "
