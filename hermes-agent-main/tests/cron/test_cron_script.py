...
- Path resolution (absolute, relative to TAKYON_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp TAKYON_HOME."""
    takyon_home = tmp_path / ".takyon"
    takyon_home.mkdir()
    (takyon_home / "cron").mkdir()
    (takyon_home / "cron" / "output").mkdir()
    (takyon_home / "scripts").mkdir()
    monkeypatch.setenv("TAKYON_HOME", str(takyon_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "TAKYON_DIR", takyon_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", takyon_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", takyon_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", takyon_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.takyon/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("TAKYON_SESSION_PLATFORM") is None
        assert os.environ.get("TAKYON_SESSION_CHAT_ID") is None
        assert os.environ.get("TAKYON_SESSION_CHAT_NAME") is None
