...
def cron_env(tmp_path, monkeypatch):
    """Isolated TAKYON_HOME with an empty skills tree.

    `tools.skills_tool` snapshots `SKILLS_DIR` at module-import time, so
    setting `TAKYON_HOME` alone doesn't reach it. We also patch the
    ...
    # `~/.takyon/skills/` and our planted skills are invisible.
    import tools.skills_tool as _skills_tool
    monkeypatch.setattr(_skills_tool, "SKILLS_DIR", skills_dir)
    monkeypatch.setattr(_skills_tool, "TAKYON_HOME", takyon_home)
...
def _plant_skill(takyon_home: Path, name: str, body: str) -> None:
    """Drop a SKILL.md into ~/.takyon/skills/<name>/ bypassing skills_guard."""
    skill_dir = takyon_home / "skills" / name
...
class TestBuildJobPromptScansSkillContent:
    def test_clean_skill_builds_normally(self, cron_env):
        ...
        _plant_skill(takyon_home, "news-digest", "Fetch the top 5 headlines and summarize.")
    ...
    def test_skill_with_invisible_unicode_raises(self, cron_env):
        ...
        _plant_skill(takyon_home, "zwsp-skill", "clean looking\u200bskill content")
