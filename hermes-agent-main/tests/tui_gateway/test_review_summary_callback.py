...
def server():
    with patch.dict(
        "sys.modules",
        {
            "takyon_constants": MagicMock(
                get_takyon_home=MagicMock(return_value="/tmp/takyon_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'takyon-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'takyon-release' patched"
    }
