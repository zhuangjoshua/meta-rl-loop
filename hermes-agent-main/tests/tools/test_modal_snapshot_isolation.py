...
def _restore_tool_modules():
    original_takyon_home = os.environ.get("TAKYON_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_takyon_home is None:
            os.environ.pop("TAKYON_HOME", None)
        else:
            os.environ["TAKYON_HOME"] = original_takyon_home
        _reset_modules(("tools", "takyon_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    takyon_cli = types.ModuleType("takyon_cli")
    takyon_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["takyon_cli"] = takyon_cli
    takyon_home = tmp_path / "takyon-home"
    os.environ["TAKYON_HOME"] = str(takyon_home)
    sys.modules["takyon_cli.config"] = types.SimpleNamespace(
        get_takyon_home=lambda: takyon_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="takyon-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": takyon_home / "modal_snapshots.json",
