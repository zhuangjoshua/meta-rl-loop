...
class TestWriteDenyExactPaths:
    ...
    def test_takyon_env(self):
        # ``.env`` under the active TAKYON_HOME (profile-aware, not just
        # ``~/.takyon``) must be write-denied. The hermetic test conftest
        # points TAKYON_HOME at a tempdir — resolve via get_takyon_home()
        # to match the denylist.
        from takyon_constants import get_takyon_home
        path = str(get_takyon_home() / ".env")
        assert _is_write_denied(path) is True
...
class TestWriteAllowed:
    ...
    def test_takyon_config_not_env(self):
        path = os.path.join(str(Path.home()), ".takyon", "config.yaml")
