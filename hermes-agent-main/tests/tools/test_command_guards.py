...
def _clean_state():
    ...
    saved = {}
    for k in ("TAKYON_INTERACTIVE", "TAKYON_GATEWAY_SESSION", "TAKYON_EXEC_ASK", "TAKYON_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["TAKYON_INTERACTIVE"] = "1"
        session_key = os.getenv("TAKYON_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No TAKYON_INTERACTIVE or TAKYON_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
