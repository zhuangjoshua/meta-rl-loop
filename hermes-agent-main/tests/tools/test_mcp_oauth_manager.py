...
def test_takyon_provider_subclass_exists():
    """TakyonMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _TAKYON_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _TAKYON_PROVIDER_CLS is not None
    assert issubclass(_TAKYON_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_takyon_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns TakyonMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _TAKYON_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("TAKYON_HOME", str(tmp_path))
    ...
    assert _TAKYON_PROVIDER_CLS is not None
    assert isinstance(provider, _TAKYON_PROVIDER_CLS)
    assert provider._takyon_server_name == "srv"
