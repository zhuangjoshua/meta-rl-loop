...
class TestCronjobRequirements:
    def test_requires_no_crontab_binary(self, monkeypatch):
        ...
        # because takyon uses an internal scheduler, not system crontab.
    ...
    def test_accepts_gateway_session(self, monkeypatch):
        ...
        monkeypatch.setenv("TAKYON_GATEWAY_SESSION", "1")
        monkeypatch.delenv("TAKYON_EXEC_ASK", raising=False)
    ...
    def test_accepts_exec_ask(self, monkeypatch):
        monkeypatch.delenv("TAKYON_INTERACTIVE", raising=False)
        monkeypatch.delenv("TAKYON_GATEWAY_SESSION", raising=False)
        monkeypatch.setenv("TAKYON_EXEC_ASK", "1")
    ...
    def test_rejects_false_like_interactive_env(self, monkeypatch, false_like_value):
        monkeypatch.setenv("TAKYON_INTERACTIVE", false_like_value)
        monkeypatch.delenv("TAKYON_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("TAKYON_EXEC_ASK", raising=False)
        assert check_cronjob_requirements() is False
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["TAKYON_INTERACTIVE", "TAKYON_GATEWAY_SESSION", "TAKYON_EXEC_ASK"],
    )
    @pytest.mark.parametrize("false_like_value", ["0", "false", "no", "off"])
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("TAKYON_INTERACTIVE", "TAKYON_GATEWAY_SESSION", "TAKYON_EXEC_ASK"):
