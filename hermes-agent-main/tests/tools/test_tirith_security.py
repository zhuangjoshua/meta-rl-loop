...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._takyon_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_takyon_bin(self):
        """After _INSTALL_FAILED, manual install in TAKYON_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        takyon_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(takyon_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(takyon_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._takyon_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == takyon_bin
            assert _tirith_mod._resolved_path == takyon_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# TAKYON_HOME isolation
# ---------------------------------------------------------------------------

class TestTakyonHomeIsolation:
    def test_takyon_bin_dir_respects_takyon_home(self):
        """_takyon_bin_dir must use TAKYON_HOME, not hardcoded ~/.takyon."""
        from tools.tirith_security import _takyon_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"TAKYON_HOME": tmpdir}):
            result = _takyon_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_takyon_home(self):
        """_failure_marker_path must use TAKYON_HOME, not hardcoded ~/.takyon."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"TAKYON_HOME": "/custom/takyon"}):
            result = _failure_marker_path()
        assert result == "/custom/takyon/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets TAKYON_HOME; verify it's active."""
        takyon_home = os.getenv("TAKYON_HOME")
        assert takyon_home is not None, "TAKYON_HOME should be set by conftest"
        assert "takyon_test" in takyon_home, "Should point to test temp dir"

    def test_get_takyon_home_fallback(self):
        """Without TAKYON_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_takyon_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove TAKYON_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".takyon")
            result = _get_takyon_home()
