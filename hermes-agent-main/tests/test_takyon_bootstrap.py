"""Tests for takyon_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Takyon entry point
(takyon, takyon-agent, takyon-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Takyon entry point imports takyon_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported takyon_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("takyon_bootstrap", None)
    import takyon_bootstrap  # noqa: WPS433
    return takyon_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Takyon CLI
...
class TestEntryPointsImportBootstrap:
    """Every Takyon entry point must import takyon_bootstrap as its
    ...
    # Entry points that invoke Takyon as a process.  Each one must
    # import takyon_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "takyon_cli/main.py",   # takyon CLI (console_script)
        "run_agent.py",          # takyon-agent (console_script)
        "acp_adapter/entry.py",  # takyon-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import takyon_bootstrap' and that
        ...
        half-finished ``takyon update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``takyon_bootstrap`` as a top-level module) leaves takyon
        ...
        """
        # Resolve relative to the takyon-agent repo root.  Tests live
        # at tests/test_takyon_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # takyon start even when takyon_bootstrap hasn't been
        ...
        assert first_import_name == "takyon_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'takyon_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import takyon_bootstrap' line to be the first import."
        )
