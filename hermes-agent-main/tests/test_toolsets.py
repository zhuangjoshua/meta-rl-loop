...
class TestToolsetConsistency:
    ...
    def test_takyon_platforms_share_core_tools(self):
        """All takyon-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on takyon-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["takyon-cli", "takyon-telegram", "takyon-discord", "takyon-whatsapp", "takyon-slack", "takyon-signal", "takyon-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_takyon_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("takyon-whatsapp")

    def test_takyon_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("takyon-api-server")
