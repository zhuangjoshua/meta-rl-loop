...
When ``TAKYON_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.takyon/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.takyon            (has its own auth.json fixture)
    * Profile     -> tmp_path/.takyon/profiles/coder   (active, TAKYON_HOME points here)
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (TAKYON_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.takyon as the "real home" — which is NOT equal
    # to the TAKYON_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    takyon_home = tmp_path / "classic"
    takyon_home.mkdir()
    monkeypatch.setenv("TAKYON_HOME", str(takyon_home))

    _write(takyon_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from takyon_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: TAKYON_HOME is set to a custom path that is NOT under
    # ~/.takyon/profiles/ — get_default_takyon_root() returns TAKYON_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from takyon_cli.auth import read_credential_pool, write_credential_pool
