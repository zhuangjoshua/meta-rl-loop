"""Tests for takyon_cli/skills_config.py and skills_tool disabled filtering."""
import pytest
...
class TestSaveDisabledSkills:
    @patch("takyon_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from takyon_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("takyon_cli.config.load_config")
    ...
    @patch("takyon_cli.config.load_config")
    @patch.dict("os.environ", {"TAKYON_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """TAKYON_SESSION_PLATFORM should be used when TAKYON_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_takyon_platform_takes_precedence(self, tmp_path, monkeypatch):
        """TAKYON_PLATFORM should win over TAKYON_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
    ...
    def test_explicit_param_overrides_env_vars(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("TAKYON_HOME", str(tmp_path))
        monkeypatch.setenv("TAKYON_PLATFORM", "telegram")
        monkeypatch.setenv("TAKYON_SESSION_PLATFORM", "telegram")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from takyon_cli.skills_config import _get_categories
        skills = [
