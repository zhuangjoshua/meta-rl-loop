...
def test_main_top_level_tui_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(sys, "argv", ["takyon", "--tui", "--toolsets", "web,terminal"])
    monkeypatch.setitem(
        sys.modules,
        "takyon_cli.plugins",
...
def test_termux_fast_tui_launch_uses_light_parser(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys, "argv", ["takyon", "--tui", "--toolsets", "web,terminal"]
    )
    monkeypatch.setattr(
        main_mod,
        "cmd_chat",
...
def test_termux_fast_tui_launch_skips_help(monkeypatch, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.setattr(sys, "argv", ["takyon", "--tui", "--help"])
...
def test_fast_tui_launch_is_termux_only(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(sys, "argv", ["takyon", "--tui"])
...
def test_main_top_level_oneshot_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys, "argv", ["takyon", "-z", "hello", "--toolsets", "web,terminal"]
    )
    monkeypatch.setitem(
        sys.modules,
        "takyon_cli.plugins",
...
def test_oneshot_filters_invalid_toolsets_before_redirect(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    from takyon_cli.oneshot import _validate_explicit_toolsets
...
def test_oneshot_rejects_disabled_mcp_toolset(monkeypatch, capsys):
    ...
    assert valid is None
    assert error == "takyon -z: --toolsets did not contain any valid toolsets.\n"
    err = capsys.readouterr().err
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """takyon -z bypasses TakyonCLI, but recall still needs SessionDB."""
    from takyon_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "takyon_state", mod("takyon_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "takyon_cli.config",
        mod("takyon_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "takyon_cli.models",
        mod("takyon_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
        "takyon_cli.runtime_provider",
        mod(
            "takyon_cli.runtime_provider",
    ...
    monkeypatch.setitem(
        sys.modules,
        "takyon_cli.tools_config",
        mod("takyon_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["TAKYON_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="nous/takyon-test", provider="nous", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["TAKYON_MODEL"] == "nous/takyon-test"
    assert env["TAKYON_INFERENCE_MODEL"] == "nous/takyon-test"
    assert env["TAKYON_TUI_PROVIDER"] == "nous"
    assert env["TAKYON_INFERENCE_PROVIDER"] == "nous"
    assert env["TAKYON_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["TAKYON_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("takyon-tui-active-session-")
...
def test_launch_tui_exit_code_42_relaunches_update(monkeypatch, main_mod):
    ...
    with patch("takyon_cli.relaunch.relaunch") as mock_relaunch:
        with pytest.raises(SystemExit) as exc:
            main_mod._launch_tui()
...
def test_launch_tui_drops_stale_resume_env_without_resume_arg(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("TAKYON_TUI_RESUME", "stale-missing-session")
    monkeypatch.setattr(
        main_mod,
        "_make_tui_argv",
        lambda tui_dir, tui_dev: (["node", "dist/entry.js"], Path(".")),
    ...
    assert "TAKYON_TUI_RESUME" not in captured["env"]
...
def test_launch_tui_sets_resume_env_from_resume_arg(monkeypatch, main_mod):
    ...
    assert captured["env"]["TAKYON_TUI_RESUME"] == "20260518_000000_goodid"


def test_make_tui_argv_dev_prebuilds_takyon_ink(monkeypatch, main_mod, tmp_path):
    ...
    monkeypatch.delenv("TAKYON_TUI_DIR", raising=False)
...
def test_print_tui_exit_summary_includes_resume_and_token_totals(monkeypatch, capsys):
    ...
    monkeypatch.setitem(
        sys.modules, "takyon_state", types.SimpleNamespace(SessionDB=lambda: _FakeDB())
    )
    ...
    assert "takyon --tui --resume 20260409_000001_abc123" in out
    assert 'takyon --tui -c "demo title"' in out
...
def test_print_tui_exit_summary_prefers_actual_active_session_file(
    ...
    assert "takyon --tui --resume actual_session" in out
