"""Tests for issue #26670 — concurrent takyon.exe detection and improved
quarantine retry / reboot-deferred fallback during `takyon update` on Windows.
...
from takyon_cli import main as cli_main


# Tests in this module either exercise the REAL _detect_concurrent_takyon_instances
# helper (and need the autouse stub in tests/takyon_cli/conftest.py disabled),
...
# ---------------------------------------------------------------------------
# _detect_concurrent_takyon_instances
# ---------------------------------------------------------------------------


def _make_proc(pid: int, exe: str, name: str = "takyon.exe"):
...
def test_detect_concurrent_returns_empty_when_no_other_processes(_winp, tmp_path):
    ...
    (scripts_dir / "takyon.exe").write_bytes(b"")
    (scripts_dir / "takyon-gateway.exe").write_bytes(b"")

    fake_psutil = types.SimpleNamespace(process_iter=lambda attrs: iter([]))
    with patch.dict(sys.modules, {"psutil": fake_psutil}):
        result = cli_main._detect_concurrent_takyon_instances(scripts_dir)
...
def test_detect_concurrent_excludes_self_pid(_winp, tmp_path):
    ...
    procs = [_make_proc(my_pid, str(shim), "takyon.exe")]
...
@patch.object(cli_main, "_is_windows", return_value=True)
def test_detect_concurrent_finds_other_takyon_process(_winp, tmp_path):
    scripts_dir = tmp_path
    shim = scripts_dir / "takyon.exe"
    shim.write_bytes(b"")
    ...
    procs = [
        _make_proc(other_pid, str(shim), "takyon.exe"),
    ...
    assert result == [(other_pid, "takyon.exe")]
...
def test_detect_concurrent_matches_case_insensitively(_winp, tmp_path):
    ...
    # Simulate the desktop spawning takyon.EXE (uppercase ext) from same path
    upper = str(shim).replace("takyon.exe", "TAKYON.EXE")
    procs = [_make_proc(9999, upper, "TAKYON.EXE")]
    fake_psutil = types.SimpleNamespace(process_iter=lambda attrs: iter(procs))
    with patch.dict(sys.modules, {"psutil": fake_psutil}):
        result = cli_main._detect_concurrent_takyon_instances(scripts_dir)

    assert result == [(9999, "TAKYON.EXE")]
...
def test_detect_concurrent_is_noop_off_windows(_winp, tmp_path):
    ...
    assert cli_main._detect_concurrent_takyon_instances(tmp_path) == []
...
def test_format_message_mentions_pids_and_remediation(tmp_path):
    matches = [(1234, "takyon.exe"), (5678, "takyon.exe")]
    msg = cli_main._format_concurrent_instances_message(matches, tmp_path)

    assert "1234" in msg
    assert "5678" in msg
    assert "takyon.exe" in msg
    assert "Takyon Desktop" in msg
    assert "--force" in msg
    # Mentions the file that would have been overwritten
    assert str(tmp_path / "takyon.exe") in msg


# ---------------------------------------------------------------------------
# _quarantine_running_takyon_exe — retry + reboot-deferred fallback
# ---------------------------------------------------------------------------
...
def test_quarantine_succeeds_first_attempt(_winp, tmp_path):
    ...
    shim = tmp_path / "takyon.exe"
    shim.write_bytes(b"old")

    pairs = cli_main._quarantine_running_takyon_exe(tmp_path)

    assert len(pairs) == 1
    orig, quarantine = pairs[0]
    assert orig == shim
    assert quarantine.name.startswith("takyon.exe.old.")
...
def test_quarantine_retries_then_succeeds(_winp, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cli_main, "_takyon_exe_shims", lambda d: [shim])
...
def test_quarantine_actionable_warning_when_everything_fails(
    ...
    assert "Takyon Desktop" in captured or "gateway" in captured.lower()
...
def test_cmd_update_aborts_on_concurrent_instance(_winp, tmp_path, capsys):
    """If another takyon.exe is running, the update bails out before
...
def test_cmd_update_force_bypasses_concurrent_check(_winp, tmp_path):
    ...
    detect = MagicMock(return_value=[(9, "takyon.exe")])
    ...
    with patch.object(
        ...
        cli_main, "_detect_concurrent_takyon_instances", detect
    ), patch.object(
