"""Comprehensive tests for takyon_cli.profiles module.
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and TAKYON_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.takyon/profiles)
    * TAKYON_HOME  -> tmp_path/.takyon  (so get_takyon_home() agrees)
    * Creates the bare-minimum ~/.takyon directory.
    """
...
class TestValidateProfileName:
    ...
    @pytest.mark.parametrize("name", ["takyon", "test", "tmp", "root", "sudo"])
    def test_reserved_names_rejected(self, name):
        """Reserved names collide with the Takyon install itself or with
...
class TestGetProfileDir:
    ...
    def test_default_returns_takyon_home(self, profile_env):
    ...
    def test_named_profile_returns_profiles_subdir(self, profile_env):
        ...
        assert result == tmp_path / ".takyon" / "profiles" / "coder"

    def test_named_profile_matching_is_case_insensitive(self, profile_env):
        tmp_path = profile_env
        assert get_profile_dir("Coder") == tmp_path / ".takyon" / "profiles" / "coder"
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.takyon must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".takyon"
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes takyon-agent, .worktrees,
        ...
        (default_home / "takyon-agent" / ".git").mkdir(parents=True)
        (default_home / "takyon-agent" / "venv" / "bin").mkdir(parents=True)
        (default_home / "takyon-agent" / "README.md").write_text("repo")
        ...
        assert not (profile_dir / "takyon-agent").exists()
...
class TestNoSkillsOptOut:
    """Tests for `takyon profile create --no-skills` and the opt-out marker."""
    ...
    def test_seed_profile_skills_respects_marker(self, profile_env):
        ...
        called directly (e.g. by `takyon update`'s all-profile sync loop)."""
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("takyon_cli.profiles._cleanup_gateway_service"):
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".takyon" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_takyon_home_returns_default(self, profile_env):
        # TAKYON_HOME points to tmp_path/.takyon which is the default
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom TAKYON_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("TAKYON_HOME", str(custom))
        # With Docker-aware roots, a custom TAKYON_HOME is the default —
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("takyon")
        assert result is not None
...
class TestRenameProfile:
    ...
    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        ...
        assert "takyon.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["takyon.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["takyon.heimdall"]["peerName"] == "user-peer"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "takyon.ssi_health": {"workspace": "takyon", "enabled": True}
            }
        }))
        ...
        assert "takyon.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["takyon.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["takyon.heimdall"]["workspace"] == "takyon"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "takyon.ssi_health": {"aiPeer": "ssi_health"},
                "takyon.heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["takyon.ssi_health"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["takyon.heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("takyon-agent", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".takyon_history",
        ...
        excluded_prefixes = [
            "default/takyon-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.takyon_history",
...
# ===================================================================
# TestGetProfilesRoot / TestGetDefaultTakyonHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_takyon_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".takyon" / "profiles"

    def test_default_takyon_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_takyon_home()
        assert home == tmp_path / ".takyon"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (TAKYON_HOME outside ~/.takyon), profiles go under TAKYON_HOME."""
    ...
    def test_default_takyon_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_takyon_home() returns TAKYON_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("TAKYON_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (TAKYON_HOME under ~/.takyon), profiles root is still ~/.takyon/profiles."""
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under TAKYON_HOME."""
        from takyon_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("TAKYON_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under TAKYON_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("TAKYON_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".takyon" / "profiles" / "mybot"
        assert result == expected
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from takyon_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".takyon"
