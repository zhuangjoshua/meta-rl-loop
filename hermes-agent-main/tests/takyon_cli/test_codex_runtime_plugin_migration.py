...
from takyon_cli.codex_runtime_plugin_migration import (
    MIGRATION_MARKER,
    MIGRATION_END_MARKER,
    MigrationReport,
    _build_takyon_tools_mcp_entry,
    _format_toml_value,
...
class TestMigrate:
    ...
    def test_no_servers_still_writes_permissions_default(self, tmp_path):
        ...
        report = migrate({}, codex_home=tmp_path, discover_plugins=False, expose_takyon_tools=False)
        assert report.written
        text = (tmp_path / "config.toml").read_text()
    ...
    def test_plugin_discovery_writes_plugin_blocks(self, tmp_path, monkeypatch):
        ...
        from takyon_cli import codex_runtime_plugin_migration as crpm
    ...
    def test_plugin_discovery_skips_unavailable_plugins(self):
        ...
        from takyon_cli.codex_runtime_plugin_migration import _query_codex_plugins
    ...
    def test_plugin_discovery_failure_non_fatal(self, tmp_path, monkeypatch):
        ...
        report = migrate({"mcp_servers": {"x": {"command": "y"}}},
                         codex_home=tmp_path, discover_plugins=True, expose_takyon_tools=False)
        assert report.written
    ...
    def test_discover_plugins_false_skips_query(self, tmp_path, monkeypatch):
        ...
        migrate({"mcp_servers": {"x": {"command": "y"}}},
                codex_home=tmp_path, discover_plugins=False, expose_takyon_tools=False)
    ...
    def test_dry_run_skips_plugin_query(self, tmp_path, monkeypatch):
        ...
        migrate({"mcp_servers": {"x": {"command": "y"}}},
                codex_home=tmp_path, dry_run=True, discover_plugins=True, expose_takyon_tools=False)
        assert called["yes"] is False
    ...
    def test_expose_takyon_tools_writes_callback_mcp_entry(self, tmp_path):
        """When expose_takyon_tools=True (production default), an
        [mcp_servers.takyon-tools] entry is written so codex calls back
        into Takyon for browser/web/delegate_task/vision/memory tools.

        This is the fix for 'all other tools that codex doesn't provide
        should be useable by takyon' — quirk #7."""
        report = migrate({}, codex_home=tmp_path,
                         discover_plugins=False,
                         default_permission_profile=None,
                         expose_takyon_tools=True)
        text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.takyon-tools]" in text
        assert "takyon_tools_mcp_server" in text
        ...
        assert "takyon-tools" in report.migrated

    def test_expose_takyon_tools_disabled_skips_entry(self, tmp_path):
        """expose_takyon_tools=False suppresses the callback registration."""
        migrate({}, codex_home=tmp_path,
                discover_plugins=False,
                default_permission_profile=None,
                expose_takyon_tools=False)
        text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.takyon-tools]" not in text
        assert "takyon_tools_mcp_server" not in text

    def test_dry_run_doesnt_write(self, tmp_path):
        report = migrate({"mcp_servers": {"x": {"command": "y"}}},
                         codex_home=tmp_path, dry_run=True, expose_takyon_tools=False)
    ...
    def test_full_migration_round_trip(self, tmp_path):
        ...
        report = migrate(takyon_cfg, codex_home=tmp_path, expose_takyon_tools=False)
        assert report.written
        text = (tmp_path / "config.toml").read_text()
    ...
    def test_idempotent_re_run_replaces_managed_block(self, tmp_path):
        # First migration
        migrate({"mcp_servers": {"a": {"command": "x"}}}, codex_home=tmp_path, expose_takyon_tools=False)
        first_text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.a]" in first_text
        # Second migration with different servers
        migrate({"mcp_servers": {"b": {"command": "y"}}}, codex_home=tmp_path, expose_takyon_tools=False)
    ...
    def test_managed_root_keys_stay_top_level_when_config_ends_in_table(self, tmp_path):
        """TOML has no explicit 'leave current table' syntax. If Takyon appends
        ...
        migrate({}, codex_home=tmp_path, discover_plugins=False, expose_takyon_tools=False)
        new_text = target.read_text()
    ...
    def test_preserves_user_mcp_server_outside_managed_block(self, tmp_path):
        ...
        to ~/.codex/config.toml outside Takyon' managed block, re-running
        ...
        migrate({"mcp_servers": {"takyon-mcp": {"command": "npx"}}},
        ...
        assert "[mcp_servers.takyon-mcp]" in final
    ...
    def test_skipped_keys_reported(self, tmp_path):
        ...
        }, codex_home=tmp_path, expose_takyon_tools=False)
    ...
    def test_invalid_mcp_servers_value(self, tmp_path):
        report = migrate({"mcp_servers": "notadict"}, codex_home=tmp_path, expose_takyon_tools=False)
...
class TestStripUnmanagedPluginTables:
    ...
    ``takyon codex-runtime migrate`` would re-emit them inside the managed
    ...
    def test_migrate_dedups_codex_owned_plugin_tables(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "takyon_cli.codex_runtime_plugin_migration._query_codex_plugins",
            fake_query,
        )
        migrate({}, codex_home=tmp_path, discover_plugins=True, expose_takyon_tools=False)
        new_text = target.read_text()
...
# ---- Bug C: TAKYON_HOME tempdir leak into ~/.codex/config.toml ----


class TestTakyonHomeLeakGuard:
    """Regression tests for issue #26250 Bug C.

    Previously ``_build_takyon_tools_mcp_entry()`` read ``TAKYON_HOME``
    ...
    once codex spawned the takyon-tools MCP subprocess.
    """

    def test_tempdir_detector_recognizes_pytest_paths(self):
        assert _looks_like_test_tempdir(
            "/private/var/folders/abc/pytest-of-kshitij/pytest-137/popen-gw2/test_X/takyon_test"
        )
        assert _looks_like_test_tempdir(
            "/tmp/pytest-of-user/pytest-12/test_X/takyon"
        )
        assert _looks_like_test_tempdir(
            "/private/var/folders/zz/T/pytest-of-bob/pytest-1"
        )

    def test_tempdir_detector_accepts_real_takyon_home(self):
        assert not _looks_like_test_tempdir("/Users/alice/.takyon")
        assert not _looks_like_test_tempdir("/home/bob/.takyon")
        assert not _looks_like_test_tempdir("/opt/takyon")
        assert not _looks_like_test_tempdir("")

    def test_pytest_tempdir_not_burned_into_mcp_env(self, monkeypatch):
        """The headline regression: even when TAKYON_HOME points at a pytest
        tempdir, _build_takyon_tools_mcp_entry() must NOT propagate it."""
        monkeypatch.setenv(
            "TAKYON_HOME",
            "/private/var/folders/xx/pytest-of-user/pytest-99/test_x/takyon_test",
        )
        entry = _build_takyon_tools_mcp_entry()
        env = entry.get("env", {})
        assert "TAKYON_HOME" not in env, (
            f"pytest-tempdir TAKYON_HOME leaked into codex MCP entry: "
            f"{env.get('TAKYON_HOME')!r}"
        )

    def test_real_takyon_home_propagates(self, monkeypatch, tmp_path):
        """A legitimate TAKYON_HOME (not a tempdir path) DOES propagate so the
        ...
        real_path = "/Users/alice/.takyon"
        monkeypatch.setenv("TAKYON_HOME", real_path)
        entry = _build_takyon_tools_mcp_entry()
        env = entry.get("env", {})
        assert env.get("TAKYON_HOME") == real_path

    def test_unset_takyon_home_omits_env_key(self, monkeypatch):
        """When TAKYON_HOME is unset in the environment, the MCP entry MUST
        NOT bake in a resolved-default path. The codex subprocess should
        inherit whatever TAKYON_HOME its launcher (systemd, gateway, shell)
        ...
        env = entry.get("env", {})
        assert "TAKYON_HOME" not in env, (
            f"TAKYON_HOME should not be set when env var is unset, got: "
            f"{env.get('TAKYON_HOME')!r}"
        )
