"""Tests for the top-level `./takyon` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./takyon` should use `takyon_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "takyon"
    called = []

    fake_main_module = types.ModuleType("takyon_cli.main")

    def fake_main():
        called.append("takyon_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "takyon_cli.main", fake_main_module)
