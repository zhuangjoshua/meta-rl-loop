"""Tests for --ignore-user-config and --ignore-rules flags on `takyon chat`.
...
files. In Takyon the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.takyon/config.yaml`` in ``load_cli_config()``
...
argparse → cmd_chat → cli.main() → TakyonCLI → AIAgent call chain.
"""
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("TAKYON_IGNORE_USER_CONFIG", "TAKYON_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``TAKYON_IGNORE_USER_CONFIG=1``.

    When the env var is set, user config at ``<takyon_home>/config.yaml`` is
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._takyon_home at tmp_path and return a fresh load_cli_config."""
        import cli
        monkeypatch.setattr(cli, "_takyon_home", tmp_path)
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "anthropic/claude-sonnet-4.6")
        monkeypatch.setenv("TAKYON_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``TakyonCLI.ignore_rules``
    so ``AIAgent`` is built with ``skip_context_files=True`` and
    ``skip_memory=True``.
    """

    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting TAKYON_IGNORE_RULES=1 flips TakyonCLI.ignore_rules True."""
        monkeypatch.setenv("TAKYON_IGNORE_RULES", "1")

        # Import TakyonCLI lazily — cli.py has heavy module-init side effects
        # that we don't want to run at test collection time.
        import cli
        importlib.reload(cli)

        # Build only enough of TakyonCLI to reach the ignore_rules assignment.
        ...
        obj = object.__new__(cli.TakyonCLI)
        # Replicate the exact logic from cli.py TakyonCLI.__init__:
        ignore_rules = False  # constructor default
        obj.ignore_rules = ignore_rules or os.environ.get("TAKYON_IGNORE_RULES") == "1"

        assert obj.ignore_rules is True

    def test_constructor_flag_alone_enables_ignore_rules(self, monkeypatch):
        monkeypatch.delenv("TAKYON_IGNORE_RULES", raising=False)
        import cli
        obj = object.__new__(cli.TakyonCLI)
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``takyon_cli/main.py`` must set
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("TAKYON_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("TAKYON_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("TAKYON_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("TAKYON_IGNORE_RULES") == "1"
    ...
    def test_only_ignore_user_config(self, monkeypatch):
        ...
        assert os.environ.get("TAKYON_IGNORE_USER_CONFIG") == "1"
        assert "TAKYON_IGNORE_RULES" not in os.environ
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "TAKYON_IGNORE_USER_CONFIG" not in os.environ
        assert "TAKYON_IGNORE_RULES" not in os.environ
...
class TestArgparseFlagsRegistered:
    ...
    def test_flags_present_in_chat_parser(self):
        ...
        import argparse
        parser = argparse.ArgumentParser(prog="takyon")
    ...
    def test_main_py_registers_both_flags(self):
        """E2E: the real takyon parser accepts both flags."""
        ...
        import inspect
        import takyon_cli.main as hm
        src = inspect.getsource(hm)
        assert "TAKYON_IGNORE_USER_CONFIG" in src
        assert "TAKYON_IGNORE_RULES" in src
