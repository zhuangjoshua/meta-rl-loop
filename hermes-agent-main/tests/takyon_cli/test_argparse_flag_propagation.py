...
subparser, placing the flag BEFORE the subcommand (e.g. 'takyon --yolo chat')
...
def _build_parser():
    """Build the takyon argument parser from the real code.
...
class TestYoloEnvVar:
    """Verify --yolo sets TAKYON_YOLO_MODE regardless of flag position.
    ...
    def test_no_yolo_no_env(self):
        ...
        self._simulate_cmd_chat_yolo_check(args)
        assert os.environ.get("TAKYON_YOLO_MODE") is None
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `takyon gateway run --accept-hooks` failed
    ...
    def test_accepted_at_every_position(self, argv):
        """Invoking `takyon <argv>` must exit 0 (help) rather than
