"""Tests for the Kanban DB layer (takyon_cli.kanban_db)."""
...
from takyon_cli import kanban_db as kb


@pytest.fixture
def kanban_home(tmp_path, monkeypatch):
    """Isolated TAKYON_HOME with an empty kanban DB."""
...
def test_connect_rejects_tls_record_in_sqlite_header(tmp_path, monkeypatch):
    ...
    home.mkdir()
    monkeypatch.setenv("TAKYON_HOME", str(home))
    monkeypatch.delenv("TAKYON_KANBAN_DB", raising=False)
    monkeypatch.delenv("TAKYON_KANBAN_HOME", raising=False)
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
...
def test_claim_uses_env_default_ttl(kanban_home, monkeypatch):
    monkeypatch.setenv("TAKYON_KANBAN_CLAIM_TTL_SECONDS", "3600")
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x", assignee="a")
...
def test_stale_claim_reclaimed(kanban_home, monkeypatch):
    import signal
    import takyon_cli.kanban_db as _kb
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real Takyon profile — preserves the
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="takyon-task", assignee="daily")
...
def test_scratch_workspace_created_under_takyon_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
...
# spawned with `takyon -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's TAKYON_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's TAKYON_HOME."""

    def _set_home(self, monkeypatch, tmp_path, takyon_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("TAKYON_HOME", str(takyon_home))
        monkeypatch.delenv("TAKYON_KANBAN_HOME", raising=False)

    def test_default_install_anchors_at_home_dot_takyon(
        self, tmp_path, monkeypatch
    ):
        # Standard install: TAKYON_HOME == ~/.takyon, no profile active.
        default_home = tmp_path / ".takyon"
        default_home.mkdir()
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.takyon/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.takyon/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.takyon/kanban.db.
        default_home = tmp_path / ".takyon"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # TAKYON_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `takyon -p coder`).
    ...
    def test_docker_custom_takyon_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: TAKYON_HOME points outside ~/.takyon.
        # `get_default_takyon_root()` returns env_home directly when it
        # is not a `<root>/profiles/<name>` shape and not under
        # `Path.home() / ".takyon"`.
        custom_root = tmp_path / "opt" / "takyon"
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: TAKYON_HOME=/opt/takyon/profiles/coder;
        # `get_default_takyon_root()` walks up to /opt/takyon because
    ...
    def test_explicit_override_via_takyon_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # Explicit override: TAKYON_KANBAN_HOME beats every other
        # resolution rule.
        default_home = tmp_path / ".takyon"
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("TAKYON_HOME", str(profile_home))
        monkeypatch.setenv("TAKYON_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        default_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("TAKYON_HOME", str(default_home))
        monkeypatch.setenv("TAKYON_KANBAN_HOME", "   ")
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # TAKYON_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile TAKYON_HOME and reads.
        monkeypatch.setenv("TAKYON_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_takyon_kanban_db_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # TAKYON_KANBAN_DB pins the file path directly and beats both
        # TAKYON_KANBAN_HOME and the `get_default_takyon_root()` path.
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("TAKYON_HOME", str(default_home))
        monkeypatch.setenv("TAKYON_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("TAKYON_KANBAN_DB", str(pinned_db))

        assert kb.kanban_db_path() == pinned_db
        # workspaces_root still follows TAKYON_KANBAN_HOME -- the pins
        # are independent.
        assert kb.workspaces_root() == umbrella / "kanban" / "workspaces"

    def test_takyon_kanban_workspaces_root_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # TAKYON_KANBAN_WORKSPACES_ROOT pins the workspaces root directly.
        default_home = tmp_path / ".takyon"
        default_home.mkdir()
        umbrella = tmp_path / "umbrella"
        umbrella.mkdir()
        ...
        monkeypatch.setenv("TAKYON_HOME", str(default_home))
        monkeypatch.setenv("TAKYON_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("TAKYON_KANBAN_WORKSPACES_ROOT", str(pinned_ws))

        assert kb.workspaces_root() == pinned_ws
        # kanban_db_path still follows TAKYON_KANBAN_HOME.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("TAKYON_HOME", str(default_home))
        monkeypatch.setenv("TAKYON_KANBAN_DB", "   ")
        monkeypatch.setenv("TAKYON_KANBAN_WORKSPACES_ROOT", "")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # The dispatcher's `_default_spawn` must inject TAKYON_KANBAN_DB
        # and TAKYON_KANBAN_WORKSPACES_ROOT into the worker env so the
        # worker converges on the dispatcher's paths even when the
        # `-p <profile>` flag rewrites TAKYON_HOME.
        default_home = tmp_path / ".takyon"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
        ...
        env = captured["env"]
        assert env["TAKYON_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["TAKYON_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["TAKYON_KANBAN_TASK"] == "t_dispatch_env"
        assert env["TAKYON_KANBAN_BRANCH"] == "wt/t_dispatch_env"
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see takyon_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(kanban_home, caplog):
    ...
    with _patch("takyon_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="takyon_state"):
            conn = kb.connect()
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'takyon kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_takyon_argv()
#
# Workers spawned by the dispatcher must use a `takyon` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["takyon", ...]`
...
def test_resolve_takyon_argv_prefers_path_shim(monkeypatch):
    """When `takyon` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
    import takyon_cli.kanban_db as kb

    monkeypatch.delenv("TAKYON_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: "/usr/local/bin/takyon")
    argv = kb._resolve_takyon_argv()
    assert argv == ["/usr/local/bin/takyon"]


def test_resolve_takyon_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
    """A relative executable override must not remain workspace-cwd-dependent."""
    import takyon_cli.kanban_db as kb

    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("TAKYON_BIN", ".\\takyon.exe")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_takyon_argv() == [os.path.abspath(".\\takyon.exe")]


def test_resolve_takyon_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    bin_dir.mkdir()
    (bin_dir / "takyon.CMD").write_text("@echo off\n", encoding="utf-8")
    monkeypatch.delenv("TAKYON_BIN", raising=False)
    monkeypatch.setenv("PATH", str(bin_dir))
    monkeypatch.setenv("PATHEXT", ".CMD")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_takyon_argv() == [sys.executable, "-m", "takyon_cli.main"]


def test_resolve_takyon_argv_honors_takyon_bin_path_override(monkeypatch, tmp_path):
    """An explicit path-like TAKYON_BIN lets service managers pin the executable."""
    import shutil
    import takyon_cli.kanban_db as kb

    shim = tmp_path / "bin" / "takyon"
    shim.parent.mkdir()
    shim.write_text("#!/bin/sh\n", encoding="utf-8")
    monkeypatch.setenv("TAKYON_BIN", str(shim))
    monkeypatch.setattr(shutil, "which", lambda name: None)

    assert kb._resolve_takyon_argv() == [str(shim)]


def test_resolve_takyon_argv_takyon_bin_bare_name_uses_path(monkeypatch, tmp_path):
    """Bare TAKYON_BIN values keep PATH semantics instead of cwd shadowing."""
    import stat
    import takyon_cli.kanban_db as kb

    cwd_takyon = tmp_path / "takyon"
    cwd_takyon.write_text("wrong\n", encoding="utf-8")
    cwd_takyon.chmod(cwd_takyon.stat().st_mode | stat.S_IXUSR)
    path_takyon = tmp_path / "bin" / "takyon"
    path_takyon.parent.mkdir()
    path_takyon.write_text("right\n", encoding="utf-8")
    path_takyon.chmod(path_takyon.stat().st_mode | stat.S_IXUSR)
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("PATH", str(path_takyon.parent))
    monkeypatch.setenv("TAKYON_BIN", "takyon")

    assert kb._resolve_takyon_argv() == [str(path_takyon)]


def test_resolve_takyon_argv_takyon_bin_bare_name_ignores_cwd(monkeypatch, tmp_path):
    """Bare TAKYON_BIN does not accept current-directory shadow executables."""
    import sys
    import takyon_cli.kanban_db as kb

    (tmp_path / "takyon.exe").write_text("wrong\n", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
...
def test_resolve_takyon_argv_takyon_bin_bare_cmd_uses_module_fallback(monkeypatch, tmp_path):
    """A PATH-resolved TAKYON_BIN batch shim is not used as worker argv[0]."""
    import sys
    import takyon_cli.kanban_db as kb
...
def test_resolve_takyon_argv_takyon_bin_unresolved_bare_name_falls_back(monkeypatch):
    """Unresolved TAKYON_BIN command names do not delegate cwd search to Popen."""
    import sys
    import takyon_cli.kanban_db as kb
...
def test_resolve_takyon_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m takyon_cli.main`.

    Pins the correct module name (NOT `takyon` — there is no top-level
    `takyon` package). Regression for #23198: the original PR shipped
    `python -m takyon` which fails with `No module named takyon` on every
    invocation.
    """
    import shutil
    import sys
    ...
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_takyon_argv()
    assert argv == [sys.executable, "-m", "takyon_cli.main"]


def test_resolve_takyon_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `takyon_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m takyon_cli.main --version`
    ...
    with mock.patch.dict(os.environ, {}, clear=False):
        os.environ.pop("TAKYON_BIN", None)
    ...
    assert "Takyon Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
