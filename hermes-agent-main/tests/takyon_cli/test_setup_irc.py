"""Tests for IRC gateway configuration via `takyon setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Takyon install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Takyon install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#takyon")
            monkeypatch.setenv("IRC_NICKNAME", "takyon-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `takyon setup gateway` experience with IRC present."""
