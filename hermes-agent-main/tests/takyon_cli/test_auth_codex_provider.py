"""Tests for Codex auth — tokens stored in Takyon auth store (~/.takyon/auth.json)."""
...
def _setup_takyon_auth(takyon_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Takyon auth store."""
...
def test_read_codex_tokens_success(tmp_path, monkeypatch):
    ...
    _setup_takyon_auth(takyon_home)
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_takyon_auth(takyon_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("TAKYON_HOME", str(takyon_home))
    ...
    monkeypatch.setattr("takyon_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    takyon_home = tmp_path / "takyon"
    _setup_takyon_auth(takyon_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("TAKYON_HOME", str(takyon_home))
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Takyon auth store, not ~/.codex/."""
    takyon_home = tmp_path / "takyon"
    codex_home = tmp_path / "codex-cli"
    takyon_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "takyon-at", "refresh_token": "takyon-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Takyon store
    assert not (codex_home / "auth.json").exists()

    # Takyon auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "takyon-at"


def test_resolve_returns_takyon_auth_store_source(tmp_path, monkeypatch):
    takyon_home = tmp_path / "takyon"
    _setup_takyon_auth(takyon_home)
    monkeypatch.setenv("TAKYON_HOME", str(takyon_home))
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "takyon_cli.auth.resolve_codex_runtime_credentials",
        lambda: {"base_url": DEFAULT_CODEX_BASE_URL},
    )
    monkeypatch.setattr(
        "takyon_cli.auth._import_codex_cli_tokens",
    ...
    monkeypatch.setattr("takyon_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("takyon_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
