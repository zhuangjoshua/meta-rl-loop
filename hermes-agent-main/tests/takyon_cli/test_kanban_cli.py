"""Tests for the kanban CLI surface (takyon_cli.kanban)."""
...
from takyon_cli import kanban as kc
...
def test_run_slash_session_filter(kanban_home):
    """`takyon kanban list --session <id>` filters by the originating
...
def test_kanban_in_autocomplete_table():
    from takyon_cli.commands import COMMANDS, SUBCOMMANDS
...
def test_kanban_not_gateway_only():
    ...
    from takyon_cli.commands import COMMAND_REGISTRY
...
def test_run_slash_board_override_restores_prior_env(kanban_home, monkeypatch):
    ...
    monkeypatch.setenv("TAKYON_KANBAN_BOARD", "beta")

    kc.run_slash("--board alpha list")

    assert os.environ.get("TAKYON_KANBAN_BOARD") == "beta"
