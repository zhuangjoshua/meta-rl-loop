...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Takyon passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("TAKYON_HOME", "/users/alice/.takyon/profiles/backend-worker")
        monkeypatch.setenv("TAKYON_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "TAKYON_KANBAN_DB",
            "/users/alice/.takyon/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.takyon/kanban/boards/smoke"]'
            in cmd
        )
