...
""",
        ...
        from takyon_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        takyon_home = tmp_path / ".takyon"
        takyon_home.mkdir()
        (takyon_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("TAKYON_HOME", str(takyon_home))
        from takyon_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from takyon_cli.plugins import PluginContext, PluginManifest, PluginManager
