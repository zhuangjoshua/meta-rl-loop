...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "Takyon Tests")
...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_takyon_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("TAKYON_HOME", str(tmp_path / ".takyon"))

    takyon_env = tmp_path / ".takyon" / ".env"
    takyon_env.parent.mkdir(parents=True)
    takyon_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.takyon/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
