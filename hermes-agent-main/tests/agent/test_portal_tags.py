...
def test_takyon_client_tag_includes_current_version():
    """The client tag must reflect takyon_cli.__version__ verbatim."""
    from takyon_cli import __version__
    from agent.portal_tags import takyon_client_tag

    assert takyon_client_tag() == f"client=takyon-client-v{__version__}"


def test_takyon_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import takyon_client_tag

    tag = takyon_client_tag()
    assert tag.startswith("client=takyon-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import takyon_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=takyon-agent" in tags
    assert takyon_client_tag() in tags
    assert len(tags) == 2
