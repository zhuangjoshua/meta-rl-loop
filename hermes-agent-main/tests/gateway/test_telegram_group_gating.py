...
def test_group_messages_can_require_direct_trigger_via_config():
    ...
    assert adapter._should_process_message(_group_message("hi @takyon_bot", entities=[_mention_entity("hi @takyon_bot")])) is True
    ...
    assert adapter._should_process_message(
        _group_message(
            "/status@takyon_bot",
            entities=[_bot_command_entity("/status@takyon_bot", "/status@takyon_bot")],
        ),
        is_command=True,
    ) is True
...
def test_raw_bot_mention_fallback_does_not_match_email_or_substring():
    adapter = _make_adapter(require_mention=True, bot_username="takyon_bot")

    assert adapter._should_process_message(_group_message("email ops@takyon_bot.example")) is False
    assert adapter._should_process_message(_group_message("prefix@takyon_bot hi")) is False
    assert adapter._should_process_message(_group_message("hi @takyon_bot")) is True
...
def test_guest_mode_allows_only_direct_mentions_outside_allowed_chats():
    ...
    mentioned = _group_message(
        "hi @takyon_bot",
        chat_id=-201,
        entities=[_mention_entity("hi @takyon_bot")],
    )
...
def test_allowed_topics_drop_other_forum_topics_before_other_gates():
    ...
    assert adapter._should_process_message(
        _group_message("hi @takyon_bot", chat_id=-100, thread_id=11, entities=[_mention_entity("hi @takyon_bot")])
    ) is False
