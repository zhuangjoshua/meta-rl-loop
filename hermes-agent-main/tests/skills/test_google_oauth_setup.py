...
class TestTakyonConstantsFallback:
    """Tests for _takyon_home.py fallback when takyon_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_takyon_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _takyon_home.py with takyon_constants blocked."""
        monkeypatch.setitem(sys.modules, "takyon_constants", None)
        spec = importlib.util.spec_from_file_location("_takyon_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_takyon_home_env_var(self, monkeypatch, tmp_path):
        """When takyon_constants is missing, TAKYON_HOME comes from env var."""
        monkeypatch.setenv("TAKYON_HOME", str(tmp_path / "custom-takyon"))
        module = self._load_helper(monkeypatch)
        assert module.get_takyon_home() == tmp_path / "custom-takyon"

    def test_fallback_defaults_to_dot_takyon(self, monkeypatch):
        """When takyon_constants is missing and TAKYON_HOME unset, default to ~/.takyon."""
        monkeypatch.delenv("TAKYON_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_takyon_home() == Path.home() / ".takyon"

    def test_fallback_ignores_empty_takyon_home(self, monkeypatch):
        """Empty/whitespace TAKYON_HOME is treated as unset."""
        monkeypatch.setenv("TAKYON_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_takyon_home() == Path.home() / ".takyon"

    def test_fallback_display_takyon_home_shortens_path(self, monkeypatch):
        """Fallback display_takyon_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("TAKYON_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_takyon_home() == "~/.takyon"

    def test_fallback_display_takyon_home_profile_path(self, monkeypatch):
        """Fallback display_takyon_home() handles profile paths under ~/."""
        monkeypatch.setenv("TAKYON_HOME", str(Path.home() / ".takyon/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_takyon_home() == "~/.takyon/profiles/coder"

    def test_fallback_display_takyon_home_custom_path(self, monkeypatch):
        """Fallback display_takyon_home() returns full path for non-home locations."""
        monkeypatch.setenv("TAKYON_HOME", "/opt/takyon-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_takyon_home() == "/opt/takyon-custom"

    def test_delegates_to_takyon_constants_when_available(self):
        """When takyon_constants IS importable, _takyon_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_takyon_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import takyon_constants
        assert module.get_takyon_home is takyon_constants.get_takyon_home
        assert module.display_takyon_home is takyon_constants.display_takyon_home
