...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("takyon/takyon-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned takyon/* and pr-* branches."""

    def test_prunes_orphaned_takyon_branch(self, git_repo):
        """takyon/takyon-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "takyon/takyon-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "takyon/takyon-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "takyon/takyon-deadbeef" in result.stdout
        ...
        assert "takyon/takyon-deadbeef" in orphaned
        ...
        assert "takyon/takyon-deadbeef" not in result.stdout
