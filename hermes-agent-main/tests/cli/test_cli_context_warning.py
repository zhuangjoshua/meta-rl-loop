...
def _isolate(tmp_path, monkeypatch):
    """Isolate TAKYON_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal TakyonCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = TakyonCLI.__new__(TakyonCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["takyon-core"]
        obj.compact = False
