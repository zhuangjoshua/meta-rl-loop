...
def test_raw_agent_events_print_tool_args_and_results():
    read_fd, write_fd = os.pipe()
    progress = cli._ShellProgress(False, raw_agent=True)
    ...
    assert "agent.raw tool_call" in output
    assert '"name": "business_read_business"' in output
    assert '"business": "homework-solver"' in output
    assert "agent.raw tool_result" in output
    assert '"result": "{\\"success\\":true,\\"business\\":{\\"slug\\":\\"homework-solver\\"}}"' in output


def test_agent_turn_prints_existing_interim_assistant_text_only_once():
    read_fd, write_fd = os.pipe()
    progress = cli._ShellProgress(False)
    progress.fd = write_fd

    try:
        progress.agent_turn("I am checking the current business state.", already_streamed=False)
        progress.agent_turn("This was already streamed.", already_streamed=True)
        os.close(write_fd)
        progress.fd = None
        output = os.read(read_fd, 65536).decode("utf-8")
    finally:
...
def test_runtime_progress_activity_age_resets_on_output(monkeypatch):
    ...
    progress._last_activity_monotonic -= 10.0
    assert progress.seconds_since_activity() >= 9.0

    progress.tool_progress("tool.started", "business_write_file", "research/strategy.md", {})
...
def test_run_agent_streams_exact_sdk_deltas_without_reprinting_final(monkeypatch, tmp_path):
    ...
    class FakeProgress:
        def __init__(self, enabled, *, raw_agent=False):
...
def test_runtime_event_tail_prints_claude_worker_runtime_events():
    ...
    assert "— Agent SDK:started —" in output
    assert "Claude worker started for product/site." in output
    assert "— Agent SDK:running —" in output
    ...
    assert "— Agent SDK:completed —" in output
