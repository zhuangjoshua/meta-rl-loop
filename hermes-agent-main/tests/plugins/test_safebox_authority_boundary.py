...
def test_env_egress_admits_infra_only():
    for k in ("DATABASE_URL", "POSTGRES_URL", "STRIPE_SECRET_KEY", "STRIPE_BILLING_WEBHOOK_SECRET",
              "AUTH0_CLIENT_SECRET", "SUPABASE_S3_SECRET_ACCESS_KEY", "R2_S3_SECRET_ACCESS_KEY",
              "UMAMI_API_KEY", "VERCEL_TOKEN", "TAKYON_GSC_SERVICE_ACCOUNT_KEY"):
...
def test_write_and_delete_routes_refuse_authority_secrets(client):
    assert client.post("/v1/env/TAKYON_CAP_SIGNING_KEY",
                       json={"value": "attacker-chosen"}, headers=_AUTH).status_code == 403
