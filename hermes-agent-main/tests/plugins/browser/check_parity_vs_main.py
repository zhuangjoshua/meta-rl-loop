...
Run from the PR worktree:
    ...
    cd ~/.takyon/takyon-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.takyon/takyon-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.takyon/takyon-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a takyon-agent checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a takyon-agent checkout"
)
...
# Isolated TAKYON_HOME for the config write.
