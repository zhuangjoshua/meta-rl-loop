...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    takyon_state = ModuleType("takyon_state")
    takyon_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "takyon_state", takyon_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Takyon"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Takyon"
