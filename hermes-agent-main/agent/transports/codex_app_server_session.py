...
Owns one Codex thread per Takyon session. Drives `turn/start`, consumes
...
# Takyon' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_TAKYON_TO_CODEX_PERMISSION_PROFILE = {
...
class CodexAppServerSession:
    """One Codex thread per Takyon session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _TAKYON_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("TAKYON_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="takyon",
            client_title="Takyon Agent",
            client_version=_get_takyon_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Takyon' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Takyon' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own takyon-tools callback we
            # auto-accept — the user already approved Takyon' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Takyon approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Takyon returns 'once', 'session', 'always', or 'deny'.
...
def _get_takyon_version() -> str:
    """Best-effort Takyon version string for codex's userAgent line."""
    try:
        from importlib.metadata import version

        return version("takyon-agent")
