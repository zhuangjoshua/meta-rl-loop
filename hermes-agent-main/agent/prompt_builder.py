...
_TAKYON_MD_NAMES = (".takyon.md", "TAKYON.md")


def _find_takyon_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.takyon.md`` or ``TAKYON.md``.
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.takyon/kanban.db`. Your task id is in "
    "`$TAKYON_KANBAN_TASK`; your workspace is `$TAKYON_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $TAKYON_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `takyon kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Takyon WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Takyon itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Takyon runs on.
    """
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Takyon itself is running. The host OS, home, and cwd "
                f"of the Takyon process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Takyon "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_takyon_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.takyon/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("TAKYON_PLATFORM")
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: takyon setup, takyon setup tools, takyon setup terminal, takyon status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from TAKYON_HOME and return its content, or None.
    ...
    """
    try:
        from takyon_cli.config import ensure_takyon_home
        ensure_takyon_home()
    except Exception as e:
        logger.debug("Could not ensure TAKYON_HOME before loading SOUL.md: %s", e)
...
def _load_takyon_md(cwd_path: Path) -> str:
    """.takyon.md / TAKYON.md — walk to git root."""
    takyon_md_path = _find_takyon_md(cwd_path)
    if not takyon_md_path:
        return ""
    try:
        content = takyon_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = takyon_md_path.name
        try:
            rel = str(takyon_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".takyon.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", takyon_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .takyon.md / TAKYON.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from TAKYON_HOME is independent and always included when present.
    ...
    # SOUL.md from TAKYON_HOME only — skip when already loaded as identity
    if not skip_soul:
