...
from takyon_constants import get_config_path, get_skills_dir
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``TAKYON_PLATFORM`` or
            ``TAKYON_SESSION_PLATFORM`` env vars.  Falls back to the
...
# which becomes the dominant cost of ``takyon`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.takyon/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``takyon`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against TAKYON_HOME, not cwd
        if not p.is_absolute():
            p = (takyon_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.takyon/skills/`` first, then external.
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    takyon = metadata.get("takyon")
    if not isinstance(takyon, dict):
        return []
    raw = takyon.get("config")
    if not raw:
        return []
