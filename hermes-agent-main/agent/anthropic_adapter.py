"""Anthropic Messages API adapter for Takyon Agent.

Translates between Takyon's internal OpenAI-style message format and
...
# Takyon effort → Anthropic adaptive-thinking effort (output_config.effort).
...
_XHIGH_EFFORT_SUBSTRINGS = ("4-7", "4.7", "4-8", "4.8", "sonnet-5")
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Takyon historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Takyon)
...
# ── Takyon-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.takyon/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_TAKYON_OAUTH_FILE = get_takyon_home() / ".anthropic_oauth.json"
...
def run_takyon_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Takyon-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Takyon with your Claude Pro/Max subscription.")
    print()
...
def read_takyon_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Takyon-managed OAuth credentials from ~/.takyon/.anthropic_oauth.json."""
    if _TAKYON_OAUTH_FILE.exists():
        try:
            data = json.loads(_TAKYON_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read Takyon OAuth credentials: %s", e)
    return None
    ...
    *base_url* is a Kimi / Moonshot host), unsigned thinking blocks
    ...
    for m in messages:
        ...
        if role == "assistant":
            ...
            # can validate the message history.  See takyon-agent#13848.
            #
    ...
    # are preserved.  See takyon-agent#13848 (Kimi) and #16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Takyon Agent", "Claude Code")
                text = text.replace("Takyon agent", "Claude Code")
                text = text.replace("takyon-agent", "claude-code")
    ...
    # silently hides reasoning text that Takyon surfaces in its CLI. We
