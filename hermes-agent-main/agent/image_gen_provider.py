...
as ``kind: backend``) or ``~/.takyon/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
def _images_cache_dir() -> Path:
    """Return ``$TAKYON_HOME/cache/images/``, creating parents as needed."""
    from takyon_constants import get_takyon_home

    path = get_takyon_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$TAKYON_HOME/cache/images/``.
