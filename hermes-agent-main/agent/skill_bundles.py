...
Storage
-------
Bundles live in ``~/.takyon/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``TAKYON_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``takyon bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under TAKYON_HOME.

    Honors ``TAKYON_BUNDLES_DIR`` for tests; falls back to
    ``<TAKYON_HOME>/skill-bundles``.
    """
    override = os.environ.get("TAKYON_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_takyon_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `takyon bundles` CLI subcommand.
# ---------------------------------------------------------------------------
