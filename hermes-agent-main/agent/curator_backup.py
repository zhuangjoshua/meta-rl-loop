...
A pre-run snapshot of ``~/.takyon/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.takyon/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.takyon/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def _backups_dir() -> Path:
    return get_takyon_home() / "skills" / ".curator_backups"
...
def snapshot_skills(reason: str = "manual") -> Optional[Path]:
    """Create a tar.gz snapshot of ``~/.takyon/skills/`` and prune old ones.
    ...
    if not skills.exists():
        logger.debug("No ~/.takyon/skills/ directory — nothing to back up")
        return None
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.takyon/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.takyon/skills/``.
    ...
    if target is None:
        return (
            ...
            + " (use `takyon curator rollback --list` to see available snapshots)",
            None,
        )
