...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Takyon starts and finds ``~/.openclaw/``.

    Points users at ``takyon claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``takyon claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Takyon, run "
        "`takyon claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `takyon claw cleanup` (renames it to ~/.openclaw.pre-migration — "
