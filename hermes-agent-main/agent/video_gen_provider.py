...
as ``kind: backend``) or ``~/.takyon/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``takyon tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``takyon tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``takyon tools`` picker."""
        return {
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``takyon tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$TAKYON_HOME/cache/videos/``, creating parents as needed."""
    from takyon_constants import get_takyon_home

    path = get_takyon_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$TAKYON_HOME/cache/videos/``.
