...
def init_agent(
    ...
    Args:
        ...
        load_soul_identity (bool): If True, still use ~/.takyon/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from takyon_cli.config import load_config as _load_pc_cfg
    ...
    # both live under ~/.takyon/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from takyon_logging import setup_logging, setup_verbose_logging
    setup_logging(takyon_home=_ra()._takyon_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that takyon_logging.setup_logging() does not
        ...
        # handler level inside takyon_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        try:
            from takyon_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `takyon model` to "
                        "select a provider, or run `takyon setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff TAKYON_KANBAN_TASK is set).
    ...
    os.environ["TAKYON_SESSION_ID"] = agent.session_id
    try:
    ...
    # Session logs go into ~/.takyon/sessions/ alongside gateway sessions
    takyon_home = get_takyon_home()
    agent.logs_dir = takyon_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.takyon/sessions/session_{sid}.json)
    ...
    try:
        from takyon_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from takyon_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "takyon"
                    except Exception:
                        pass
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from takyon_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            try:
                from takyon_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Takyon Agent.  Choose a model with at least "
