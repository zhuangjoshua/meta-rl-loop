...
from takyon_logging import set_session_context
...
def run_conversation(
    ...
    # ``takyon logs --session <id>`` can filter a single conversation.
    from takyon_logging import set_session_context
    ...
    # are reused.  The system prompt is Takyon's territory; plugins
    ...
    # all run inside Codex). Default Takyon path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for Takyon internals.
        #
        # Takyon invariant: the system prompt is built ONCE per session
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                if env_var_enabled("TAKYON_DUMP_REQUESTS"):
                ...
                if (
                    ...
                    print(f"{agent.log_prefix}     • Re-authenticate: takyon login --provider nous")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `takyon doctor` for credential-chain diagnostics, or")
                    ...
                    from takyon_constants import display_takyon_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Takyon-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: takyon config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: takyon config set ANTHROPIC_API_KEY \"\"")
                ...
                # providers (DeepSeek, Kimi, MiMo, Takyon).  A 429 can
                ...
                # which Takyon' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Takyon' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`takyon",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if _provider in {"openai-codex", "xai-oauth"} and status_code == 401:
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Then run `takyon auth` to re-authenticate.", force=True)
                            else:
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok Subscription) from `takyon model`.", force=True)
                        else:
                            agent._vprint(f"{agent.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: takyon setup", force=True)
    ...
    if final_response is None and (
        ...
        _kanban_task = os.environ.get("TAKYON_KANBAN_TASK")
        if _kanban_task:
            try:
