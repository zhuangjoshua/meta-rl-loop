...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - takyon_home (str): The active TAKYON_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.takyon``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "takyon").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'takyon memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], takyon_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'takyon memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``takyon_home`` is the active TAKYON_HOME directory path.
