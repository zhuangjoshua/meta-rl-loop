...
Plugins built on Takyon Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Takyon
...
:class:`~takyon_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Takyon' main config
...
handles every provider, fallback chain, and per-task override Takyon
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`takyon_cli.plugins.PluginContext`
