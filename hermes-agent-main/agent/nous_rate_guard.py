...
(3 SDK retries x 3 Takyon retries), and every one of those calls counts
...
def _state_path() -> str:
    ...
    try:
        ...
        base = get_takyon_home()
    except ImportError:
        base = os.path.join(os.path.expanduser("~"), ".takyon")
...
def is_genuine_nous_rate_limit(
    *,
    ...
    MiMo, Takyon, ...) behind one endpoint.  A 429 can mean either:
