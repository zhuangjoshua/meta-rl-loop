# nix/nixosModules.nix — NixOS module for takyon-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: takyon runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $TAKYON_HOME/.env
# and read by takyon at startup — no container recreation needed for env changes.
#
# Tool resolution: the takyon wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.takyon-agent;
    effectivePackage =
    ...
    takyon-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-takyon-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "takyon-config-attrs";
      description = "Takyon YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "takyon-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "takyon-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'TAKYON_DOC_EOF'\n${value}\nTAKYON_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "takyon-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/takyon";
    ...
    # takyon user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "takyon-container-entrypoint" ''
      set -eu

      TAKYON_UID="''${TAKYON_UID:?TAKYON_UID must be set}"
      TAKYON_GID="''${TAKYON_GID:?TAKYON_GID must be set}"

      # ── Group: ensure a group with GID=$TAKYON_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$TAKYON_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="takyon"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$TAKYON_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$TAKYON_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$TAKYON_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$TAKYON_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="takyon"
        TARGET_HOME="/home/takyon"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$TAKYON_UID" -g "$TAKYON_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$TAKYON_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$TAKYON_UID:$TAKYON_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure TAKYON_HOME is owned by the target user.
      ...
      if [ -n "''${TAKYON_HOME:-}" ] && [ -d "$TAKYON_HOME" ]; then
        find "$TAKYON_HOME" \! -user "$TAKYON_UID" -exec chown "$TAKYON_UID:$TAKYON_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/takyon-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/takyon-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/takyon ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/takyon
        chmod 0440 /etc/sudoers.d/takyon
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$TAKYON_UID" --regid="$TAKYON_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $TAKYON_HOME/.env.
    ...
    # Default: /var/lib/takyon/workspace → /data/workspace.
  ...
  in {
    options.services.takyon-agent = with lib; {
      enable = mkEnableOption "Takyon Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = takyon-agent;
        description = "The takyon-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/takyon";
        description = "State directory. Contains .takyon/ subdir (TAKYON_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Takyon config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $TAKYON_HOME/.env at activation time.
          Takyon reads this file on every startup via load_takyon_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $TAKYON_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $TAKYON_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `takyon gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the takyon user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the takyon plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Takyon discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          takyon_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as takyon (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in takyon-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the takyon CLI to environment.systemPackages and export
          TAKYON_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.takyon symlink to the service
            stateDir. These users are automatically added to the takyon group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.takyon-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the takyon CLI to system PATH and export TAKYON_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.takyon/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.TAKYON_HOME = "${cfg.stateDir}/.takyon";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.takyon-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the takyon user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.takyon-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed takyon
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure takyon is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.takyon        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.takyon/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.takyon/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.takyon/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.takyon/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.takyon/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."takyon-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.takyon
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.takyon ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.takyon ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.takyon -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.takyon/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.takyon/$_subdir"
            chmod 2770 "${cfg.stateDir}/.takyon/$_subdir"
            find "${cfg.stateDir}/.takyon/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.takyon/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.takyon/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.takyon/config.yaml
            chmod 0640 ${cfg.stateDir}/.takyon/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.takyon/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.takyon/.managed
          chmod 0644 ${cfg.stateDir}/.takyon/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.takyon/.container-mode <<'TAKYON_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    takyon_bin=${containerDataDir}/current-package/bin/takyon
    TAKYON_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.takyon/.container-mode
            chmod 0644 ${cfg.stateDir}/.takyon/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.takyon/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.takyon";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.takyon" ]; then
                  rm -f "${symlinkPath}"
                  echo "takyon-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.takyon -> stateDir/.takyon for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.takyon";
                target = "${cfg.stateDir}/.takyon";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "takyon-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.takyon/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.takyon/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.takyon/auth.json
              fi
            ''}
          ''}
          ...
          # Takyon reads $TAKYON_HOME/.env at startup via load_takyon_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.takyon/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'TAKYON_NIX_ENV_EOF'
    ${envFileContent}
    TAKYON_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.takyon/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.takyon/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.takyon/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.takyon-agent = {
          description = "Takyon Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            TAKYON_HOME = "${cfg.stateDir}/.takyon";
            TAKYON_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $TAKYON_HOME/.env by the activation script. load_takyon_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/takyon"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the takyon group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.takyon-agent = {
          description = "Takyon Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              TAKYON_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              TAKYON_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env TAKYON_UID="$TAKYON_UID" \
                --env TAKYON_GID="$TAKYON_GID" \
                --env TAKYON_HOME=${containerDataDir}/.takyon \
                --env TAKYON_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/takyon gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
