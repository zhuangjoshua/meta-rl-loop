...
One ``CDPSupervisor`` runs per Takyon ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "takyon-dialog-bridge.invalid"
...
(() => {
  if (window.__takyonDialogBridgeInstalled) return;
  window.__takyonDialogBridgeInstalled = true;
  const ENDPOINT = "http://takyon-dialog-bridge.invalid/";
