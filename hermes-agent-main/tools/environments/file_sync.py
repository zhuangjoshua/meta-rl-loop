...
_FORCE_SYNC_ENV = "TAKYON_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.takyon") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.takyon to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``TAKYON_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, takyon_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.takyon/`` directory as a tar archive,
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.takyon/skills/a.md`` →
        ``~/.takyon/skills/a.md``, a new remote file at
        ``/root/.takyon/skills/b.md`` maps to ``~/.takyon/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
