...
from tools.environments.local import _TAKYON_PROVIDER_ENV_BLOCKLIST
...
def _load_takyon_env_vars() -> dict[str, str]:
    """Load ~/.takyon/.env values without failing Docker command execution."""
    try:
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``TAKYON_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("TAKYON_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using TAKYON_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image entrypoint drops from root to the 'takyon'
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.takyon/sandboxes/). Non-persistent
        ...
        container_name = f"takyon-{uuid.uuid4().hex[:8]}"
        run_cmd = [
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Takyon secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _TAKYON_PROVIDER_ENV_BLOCKLIST)
        takyon_env = _load_takyon_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if value is None:
                value = takyon_env.get(key)
    ...
    def cleanup(self):
        ...
        if self._container_id:
            ...
            try:
                # Stop in background so cleanup doesn't block. Use a small
                # shell wrapper script with shell=False so no untrusted input
                # is interpolated into a shell command line.
                stop_script = (
                    f'timeout 60 "$0" stop "$1" >/dev/null 2>&1 || '
                    f'"$0" rm -f "$1" >/dev/null 2>&1'
                )
                subprocess.Popen(
                    ["/bin/sh", "-c", stop_script, self._docker_exe, container_id],
            ...
            if not self._persistent:
                ...
                try:
                    rm_script = 'sleep 3; "$0" rm -f "$1" >/dev/null 2>&1'
                    subprocess.Popen(
                        ["/bin/sh", "-c", rm_script, self._docker_exe, container_id],
                        stdout=subprocess.DEVNULL,
                        stderr=subprocess.DEVNULL,
                        start_new_session=True,
                    )
