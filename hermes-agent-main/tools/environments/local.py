...
# Takyon-internal env vars that should NOT leak into terminal subprocesses.
_TAKYON_PROVIDER_ENV_FORCE_PREFIX = "_TAKYON_FORCE_"
...
_TAKYON_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()


def _inject_context_takyon_home(env: dict) -> None:
    """Bridge the context-local Takyon home override into subprocess env."""
    try:
        from takyon_constants import get_takyon_home_override

        value = get_takyon_home_override()
        if value:
            env["TAKYON_HOME"] = value
    except Exception:
        pass
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("TAKYON_GIT_BASH_PATH")
    ...
    #   PortableGit: %LOCALAPPDATA%\takyon\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\takyon\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _takyon_portable_git = os.path.join(_local_appdata, "takyon", "git") if _local_appdata else ""
    if _takyon_portable_git:
        for candidate in (
            os.path.join(_takyon_portable_git, "bin", "bash.exe"),        # PortableGit (primary)
            os.path.join(_takyon_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
    ...
    raise RuntimeError(
        "Git Bash not found. Takyon Agent requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set TAKYON_GIT_BASH_PATH to your bash.exe location."
    )
...
def _make_run_env(env: dict) -> dict:
    ...
    # Takyon is driving via _find_bash (Git Bash), and Git Bash itself
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Takyon trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``TAKYON_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under TAKYON_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_takyon_home() / "cache" / "terminal"
            except Exception:
                cache_dir = Path(tempfile.gettempdir()) / "takyon_terminal"
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._takyon_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_takyon_pgid", None)
                    if pgid is None:
                        raise
