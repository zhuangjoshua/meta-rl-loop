...
This module provides the glue:
    - ``TakyonTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Takyon Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses TAKYON_HOME so each profile gets its own OAuth tokens.
    Layout: ``TAKYON_HOME/mcp-tokens/``
    """
    try:
        from takyon_constants import get_takyon_home
        base = Path(get_takyon_home())
    except ImportError:
        base = Path(os.environ.get("TAKYON_HOME", str(Path.home() / ".takyon")))
...
# ---------------------------------------------------------------------------
# TakyonTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class TakyonTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        TAKYON_HOME/mcp-tokens/<server_name>.json         -- tokens
        TAKYON_HOME/mcp-tokens/<server_name>.client.json   -- client info
        TAKYON_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Takyon records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Takyon.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  Then open the URL above. See: https://takyon-agent.nousresearch.com/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Takyon Agent")
    scope = cfg.get("scope")
