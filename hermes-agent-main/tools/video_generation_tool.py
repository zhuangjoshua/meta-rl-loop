...
provider** — turn on a backend by enabling a plugin (``takyon plugins
enable video_gen/<name>``) and selecting it in ``takyon tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/takyon-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `takyon tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"registered that name. Run `takyon plugins list` to see "
            f"installed video gen backends, or `takyon tools` → Video "
...
# `takyon tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`takyon tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `takyon tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
