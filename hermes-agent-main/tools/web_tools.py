...
Backend is selected during ``takyon tools`` setup (web.backend in config.yaml).
When available, Takyon can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``takyon tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `takyon tools` repaint.
        try:
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `takyon tools` to set one up."
                ),
            }
        else:
            logger.info(
