...
def _is_takyon_provider_credential(name: str) -> bool:
    """True if ``name`` is a Takyon-managed provider credential (API key,
    token, or similar) per ``_TAKYON_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Takyon API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    try:
        ...
        return False
    return name in _TAKYON_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Takyon-managed provider credentials (from
    ``_TAKYON_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Takyon-managed
    ...
    Non-Takyon third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_takyon_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Takyon provider "
                "credential %r (blocked by _TAKYON_PROVIDER_ENV_BLOCKLIST). "
