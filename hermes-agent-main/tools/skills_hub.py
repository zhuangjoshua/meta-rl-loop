#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Takyon Skills Hub.
...
Used by takyon_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            takyon_meta = metadata.get("takyon", {})
...
# ---------------------------------------------------------------------------
# Takyon centralized index source
# ---------------------------------------------------------------------------

TAKYON_INDEX_URL = "https://takyon-agent.nousresearch.com/docs/api/skills-index.json"
TAKYON_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "takyon-index.json"
TAKYON_INDEX_TTL = 6 * 3600  # 6 hours


def _load_takyon_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for TAKYON_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if TAKYON_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - TAKYON_INDEX_CACHE_FILE.stat().st_mtime
            if age < TAKYON_INDEX_TTL:
                return json.loads(TAKYON_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(TAKYON_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Takyon index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Takyon index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        TAKYON_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        TAKYON_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class TakyonIndexSource(SkillSource):
    """Skill source backed by the centralized Takyon Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_takyon_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "takyon-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "takyon-index"),
            identifier=entry.get("identifier", ""),
