...
Tracks per-skill usage metadata in a sidecar JSON file (~/.takyon/skills/.usage.json)
...
def _read_bundled_manifest_names() -> Set[str]:
    ...
    Reads ~/.takyon/skills/.bundled_manifest (format: "name:hash" per line).
...
def list_archived_skill_names() -> List[str]:
    """Enumerate skills in ``~/.takyon/skills/.archive/``.

    Archive layout is flat (``.archive/<skill>/``) as set by ``archive_skill``,
    so the directory name is the skill name. Used by ``takyon curator
    list-archived`` to help users pass a name to ``takyon curator restore``.
    """
...
def archive_skill(skill_name: str) -> Tuple[bool, str]:
    """Move an agent-created skill directory to ~/.takyon/skills/.archive/.
...
def restore_skill(skill_name: str) -> Tuple[bool, str]:
    """Move an archived skill back to ~/.takyon/skills/. Restores to the flat
...
def _find_skill_dir(skill_name: str) -> Optional[Path]:
    ...
    Handles both flat (~/.takyon/skills/<skill>/SKILL.md) and category-nested
    (~/.takyon/skills/<category>/<skill>/SKILL.md) layouts.
    """
    base = _skills_dir()
    if not base.exists():
        return None
