"""Central registry for all takyon-agent tools.
...
class ToolEntry:
    ...
    def __init__(self, name, toolset, schema, handler, check_fn,
        ...
        # runtime config. The callable is invoked
...
# timescales. Cache results for ~30 s so env-var flips via ``takyon tools``
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``takyon tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``takyon tools enable foo``)
        ...
        for name in sorted(tool_names):
            ...
            # Apply runtime-dynamic overrides. Caller side
            # (model_tools.get_tool_definitions) already keys its memo on
            # config.yaml mtime + size, so config changes invalidate the cache.
