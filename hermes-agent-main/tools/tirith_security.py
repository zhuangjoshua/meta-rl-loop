...
it is automatically downloaded from GitHub releases to $TAKYON_HOME/bin/tirith.
...
def _get_takyon_home() -> str:
    """Return the Takyon home directory, respecting TAKYON_HOME env var."""
    return str(get_takyon_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_takyon_home(), ".tirith-install-failed")
...
def _takyon_bin_dir() -> str:
    """Return $TAKYON_HOME/bin, creating it if needed."""
    d = os.path.join(_get_takyon_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $TAKYON_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_takyon_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $TAKYON_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $TAKYON_HOME/bin/tirith
    ...
    takyon_bin = os.path.join(_takyon_bin_dir(), "tirith")
    if os.path.isfile(takyon_bin) and os.access(takyon_bin, os.X_OK):
        _resolved_path = takyon_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return takyon_bin
