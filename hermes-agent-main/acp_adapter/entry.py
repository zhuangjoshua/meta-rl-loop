"""CLI entry point for the takyon-agent ACP adapter.

Loads environment variables from ``~/.takyon/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from TAKYON_HOME (default ``~/.takyon``)."""
    from takyon_cli.env_loader import load_takyon_dotenv

    takyon_home = get_takyon_home()
    loaded = load_takyon_dotenv(takyon_home=takyon_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", takyon_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="takyon-acp",
        description="Run Takyon Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Takyon version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _print_version() -> None:
    ...
    print(takyon_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import TakyonACPAgent  # noqa: F401

    print("Takyon ACP check OK")


def _run_setup() -> None:
    from takyon_cli.main import main as takyon_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "takyon", "model"]
        takyon_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``takyon postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting takyon-agent ACP adapter")
    ...
    import acp
    from .server import TakyonACPAgent
