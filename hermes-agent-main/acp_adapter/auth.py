"""ACP auth helpers — detect and advertise Takyon authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "takyon-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Takyon runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Takyon can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Takyon.
    ...
    not have Takyon provider credentials configured yet, so Takyon always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Takyon using the currently configured "
