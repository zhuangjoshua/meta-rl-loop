...
Usage (via cron with --no-agent):
    ...
    takyon cron create takyon-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$TAKYON_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name takyon-issues --repo NousResearch/takyon-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.takyon/.env to avoid the 60 req/hr
