...
Usage (via cron with --no-agent):
    ...
    takyon cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$TAKYON_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Takyon-Watcher/1.0"})
