...
takyon kanban watch --tenant <project-slug>

# Snapshot of the board
takyon kanban list --tenant <project-slug>
takyon kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
takyon kanban stats --tenant <project-slug>

# Visual dashboard (browser)
takyon dashboard

# Inspect a specific task (includes comments + events)
takyon kanban show <task-id>

# Follow a single task's event stream
takyon kanban tail <task-id>
```

Verify available subcommands with `takyon kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `takyon kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `takyon kanban block <id>` then `takyon kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `takyon kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `takyon kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `takyon profile list` (profile exists?), `takyon status` (gateway/dispatcher up?), and verify tenant. |
...
takyon kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
takyon kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(takyon kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
takyon kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
takyon kanban block <task-id>
takyon kanban archive <task-id>

# Diagnose what happened
takyon kanban show <task-id>      # task body, comments, recent events
takyon kanban tail <task-id>      # follow the live event stream
takyon kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `takyon kanban create` for the director
...
while true; do
    clear
    takyon kanban list --tenant <slug>
    echo "---"
    takyon kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `takyon kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `takyon profile list` and start
