...
Maps each role archetype to the Takyon skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public takyon-agent
...
## Takyon skills relevant to video production

### Visual / rendering skills (`takyon-agent/skills/creative/`)
...
### Design / pre-production skills (`takyon-agent/skills/creative/`)
...
### Audio / media skills (`takyon-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`takyon-agent/skills/devops/`)
...
These are **not** Takyon skills but external CLIs / APIs that profiles invoke.
...
## Built-in Takyon tools for media review

These are native Takyon tools — not invoked via terminal but through their own
...
key is present in `~/.takyon/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Takyon profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.takyon/.env` → environment variable.
...
When a new Takyon-public video skill ships:
