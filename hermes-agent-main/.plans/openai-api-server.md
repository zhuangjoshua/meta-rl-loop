# OpenAI-Compatible API Server for Takyon Agent
...
SSE streaming. By exposing this endpoint, takyon-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  takyon-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.takyon/.env`
2. Run `takyon gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with takyon-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns takyon-agent as a model) |
...
Authorization: Bearer takyon-api-key-here
...
But takyon-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "takyon-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured takyon-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of takyon-agent's core prompt
...
Once implemented, takyon-agent works as a drop-in backend for:
