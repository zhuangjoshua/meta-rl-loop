"""Persistent slash-command worker — one TakyonCLI per TUI session.
...
def _run(cli: TakyonCLI, command: str) -> str:
...
def main():
    ...
    os.environ["TAKYON_SESSION_KEY"] = args.session_key
    os.environ["TAKYON_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = TakyonCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
