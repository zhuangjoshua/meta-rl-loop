# ============================================================================
# Takyon Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$TakyonHome = "$env:LOCALAPPDATA\takyon",
    [string]$InstallDir = "$env:LOCALAPPDATA\takyon\takyon-agent",
...
$RepoUrlSsh = "git@github.com:NousResearch/takyon-agent.git"
$RepoUrlHttps = "https://github.com/NousResearch/takyon-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "|             * Takyon Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $TakyonHome)) {
        New-Item -ItemType Directory -Force -Path $TakyonHome | Out-Null
    }
    $envFile = Join-Path $TakyonHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $TakyonHome "node"
...
function Install-Git {
    ...
    which Takyon uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\takyon\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Takyon needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Takyon copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\takyon\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``TAKYON_GIT_BASH_PATH`` (User scope) so Takyon can find it in a fresh
    ...
    # Download PortableGit into $TakyonHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $TakyonHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Takyon features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Takyon needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``TAKYON_GIT_BASH_PATH`` (User env scope) so Takyon can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $TakyonHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $TakyonHome\git\usr\bin\bash.exe
    $candidates += "$TakyonHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$TakyonHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("TAKYON_GIT_BASH_PATH", $candidate, "User")
            $env:TAKYON_GIT_BASH_PATH = $candidate
            Write-Info "Set TAKYON_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Takyon may not find Git Bash."
    Write-Info "If needed, set TAKYON_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$TakyonHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$TakyonHome\node;$env:Path"
        Write-Success "Node.js $version found (Takyon-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # The portable zip path drops node.exe + npm into $TakyonHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $TakyonHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\takyon-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$TakyonHome\node") { Remove-Item -Recurse -Force "$TakyonHome\node" }
                Move-Item $extractedDir.FullName "$TakyonHome\node"
                ...
                $version = & "$TakyonHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $TakyonHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
    ...
    if (Test-Path $InstallDir) {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                Write-Info "terminals, running takyon processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/NousResearch/takyon-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/NousResearch/takyon-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/NousResearch/takyon-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\takyon-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\takyon-agent-extract"
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `takyon.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install takyon-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and takyon.exe broken with
    ...
    # here, not 30 seconds later when the user runs `takyon`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `takyon dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `takyon dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `takyon dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up takyon command..."
    
    if ($NoVenv) {
        $takyonBin = "$InstallDir"
    } else {
        $takyonBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so takyon is globally available
    # On Windows, the takyon.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$takyonBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$takyonBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $takyonBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set TAKYON_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\takyon instead
    # of the Unix default ~/.takyon
    $currentTakyonHome = [Environment]::GetEnvironmentVariable("TAKYON_HOME", "User")
    if (-not $currentTakyonHome -or $currentTakyonHome -ne $TakyonHome) {
        [Environment]::SetEnvironmentVariable("TAKYON_HOME", $TakyonHome, "User")
        Write-Success "Set TAKYON_HOME=$TakyonHome"
    }
    $env:TAKYON_HOME = $TakyonHome
    
    # Update current session
    $env:Path = "$takyonBin;$env:Path"
    
    Write-Success "takyon command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.takyon directory structure
    New-Item -ItemType Directory -Force -Path "$TakyonHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TakyonHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TakyonHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TakyonHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TakyonHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TakyonHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TakyonHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TakyonHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TakyonHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$TakyonHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.takyon/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.takyon/.env"
        }
    } else {
        Write-Info "~/.takyon/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$TakyonHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.takyon/config.yaml from template"
        }
    } else {
        Write-Info "~/.takyon/config.yaml already exists, keeping it"
    }
    ...
    # (the default PS5 behaviour), and Takyon's prompt-injection scanner
...
"@
        ...
        Write-Success "Created ~/.takyon/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.takyon/"
    
    # Seed bundled skills into ~/.takyon/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.takyon/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.takyon/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$TakyonHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.takyon/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'takyon setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\takyon-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\takyon-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\takyon-npm-tui-$(Get-Random).log"
...
function Install-PlatformSdks {
    ...
    # ~/.takyon/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'takyon setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m takyon_cli.main setup
    } else {
        python -m takyon_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $takyonCmd = "$InstallDir\venv\Scripts\takyon.exe"
    if (-not (Test-Path $takyonCmd)) {
        $takyonCmd = "takyon"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$TakyonHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'takyon whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: takyon gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$TakyonHome\logs\gateway.log"
            Start-Process -FilePath $takyonCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$TakyonHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: takyon gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: takyon gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$TakyonHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$TakyonHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$TakyonHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$TakyonHome\takyon-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   takyon              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   takyon setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   takyon config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   takyon config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   takyon gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   takyon update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Takyon repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    ...
    @{ Name = "path";             Title = "Adding Takyon to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/NousResearch/takyon-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
