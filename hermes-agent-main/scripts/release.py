...
VERSION_FILE = REPO_ROOT / "takyon_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "274096618+takyon-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "takyon@nousresearch.com": "NousResearch",
    ...
    "takyon@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (TAKYON_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (TAKYON_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: takyon-eval community link)
    ...
    "takyonagent26@gmail.com": "takyonagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+takyonagent26@users.noreply.github.com": "takyonagent26",
    ...
    "vigo@takyon": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.takyon@agentmail.to": "oseftg",
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"takyon-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "takyon@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/takyon-agent",
    ...
    lines.append(f"# Takyon Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Takyon Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Takyon Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Takyon Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Takyon Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Takyon Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Takyon Agent v{new_version} ({calver_date})' "
