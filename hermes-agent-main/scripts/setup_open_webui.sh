...
# Bootstrap Open WebUI against Takyon Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.takyon/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-takyon.sh
...
#   TAKYON_API_PORT=8642
#   TAKYON_API_HOST=127.0.0.1
#   TAKYON_API_MODEL_NAME='Takyon Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Takyon Agent WebUI}"
...
TAKYON_ENV_FILE="${TAKYON_ENV_FILE:-$HOME/.takyon/.env}"
TAKYON_API_PORT="${TAKYON_API_PORT:-8642}"
TAKYON_API_HOST="${TAKYON_API_HOST:-127.0.0.1}"
TAKYON_API_CONNECT_HOST="${TAKYON_API_CONNECT_HOST:-127.0.0.1}"
TAKYON_API_MODEL_NAME="${TAKYON_API_MODEL_NAME:-Takyon Agent}"
TAKYON_API_BASE_URL="http://${TAKYON_API_CONNECT_HOST}:${TAKYON_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-takyon.sh"
LOG_DIR="$HOME/.takyon/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$TAKYON_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$TAKYON_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.takyon.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.takyon</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.takyon"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.takyon"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-takyon.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Takyon Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-takyon.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.takyon/logs/openwebui.log
StandardError=append:%h/.takyon/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-takyon.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$TAKYON_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Takyon API server is configured...'
  upsert_env API_SERVER_ENABLED true "$TAKYON_ENV_FILE"
  upsert_env API_SERVER_HOST "$TAKYON_API_HOST" "$TAKYON_ENV_FILE"
  upsert_env API_SERVER_PORT "$TAKYON_API_PORT" "$TAKYON_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$TAKYON_API_MODEL_NAME" "$TAKYON_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$TAKYON_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Takyon gateway so API server settings take effect...'
  takyon gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${TAKYON_API_CONNECT_HOST}:${TAKYON_API_PORT}/health" >/dev/null; then
    log 'Takyon API server did not answer on the first check. Trying to start gateway in the background...'
    nohup takyon gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${TAKYON_API_CONNECT_HOST}:${TAKYON_API_PORT}/health" >/dev/null
  ...
  log "Takyon API endpoint: ${TAKYON_API_BASE_URL}"
