#!/usr/bin/env bash
# Canonical test runner for takyon-agent. Run this instead of calling
...
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.takyon/takyon-agent/venv"; do
...
# Unset TAKYON_* behavioral vars too.
unset TAKYON_YOLO_MODE TAKYON_INTERACTIVE TAKYON_QUIET TAKYON_TOOL_PROGRESS \
      TAKYON_TOOL_PROGRESS_MODE TAKYON_MAX_ITERATIONS TAKYON_SESSION_PLATFORM \
      TAKYON_SESSION_CHAT_ID TAKYON_SESSION_CHAT_NAME TAKYON_SESSION_THREAD_ID \
      TAKYON_SESSION_SOURCE TAKYON_SESSION_KEY TAKYON_GATEWAY_SESSION \
      TAKYON_CRON_SESSION \
      TAKYON_PLATFORM TAKYON_INFERENCE_PROVIDER TAKYON_MANAGED TAKYON_DEV \
      TAKYON_CONTAINER TAKYON_EPHEMERAL_SYSTEM_PROMPT TAKYON_TIMEZONE \
      TAKYON_REDACT_SECRETS TAKYON_BACKGROUND_NOTIFICATIONS TAKYON_EXEC_ASK \
      TAKYON_HOME_MODE 2>/dev/null || true
...
# If a system-wide takyon pytest_live_guard plugin is installed at
# $HOME/.takyon/pytest_live_guard.py, force-load it here so every test run
...
if [ -f "$HOME/.takyon/pytest_live_guard.py" ]; then
  case ":${PYTHONPATH:-}:" in
    *":$HOME/.takyon:"*) ;;
    *) export PYTHONPATH="${PYTHONPATH:+$PYTHONPATH:}$HOME/.takyon" ;;
  esac
