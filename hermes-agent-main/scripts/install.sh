...
# Python-driven tool session (e.g. Takyon terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/takyon-agent.git"
REPO_URL_HTTPS="https://github.com/NousResearch/takyon-agent.git"
TAKYON_HOME="${TAKYON_HOME:-$HOME/.takyon}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${TAKYON_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$TAKYON_INSTALL_DIR"
...
#   code at /usr/local/lib/takyon-agent, command at /usr/local/bin/takyon,
#   data still at /root/.takyon (TAKYON_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/.takyon/takyon-agent"
            echo "                   default (root, Linux): /usr/local/lib/takyon-agent"
            echo "  --takyon-home PATH  Data directory (default: ~/.takyon, or \$TAKYON_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Takyon installs the code under"
            echo "  /usr/local/lib/takyon-agent and links the command into"
            echo "  /usr/local/bin/takyon (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$TAKYON_HOME"
            echo "  (default /root/.takyon).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$TAKYON_HOME/takyon-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs takyon setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# Decide where the repo checkout + venv live, and where the `takyon` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $TAKYON_HOME/takyon-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $TAKYON_HOME/takyon-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/takyon-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $TAKYON_HOME/takyon-agent — then preserve it)
#
# Always no-op when the user set --dir or $TAKYON_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in TAKYON_HOME.
    if is_termux; then
        INSTALL_DIR="$TAKYON_HOME/takyon-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$TAKYON_HOME/takyon-agent/.git" ]; then
            INSTALL_DIR="$TAKYON_HOME/takyon-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/takyon-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/takyon-agent"
        ROOT_FHS_LAYOUT=true
        log_info "Root install on Linux — using FHS layout"
        log_info "  Code:    $INSTALL_DIR"
        log_info "  Command: /usr/local/bin/takyon"
        log_info "  Data:    $TAKYON_HOME (unchanged)"
        return 0
    fi
...
get_takyon_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/takyon" ]; then
        echo "$link_dir/takyon"
    else
        echo "takyon"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/NousResearch/takyon-agent/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/takyon-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/takyon-uv-installer.$$.sh")"
...
check_node() {
    ...
    if [ -x "$TAKYON_HOME/node/bin/node" ]; then
        ...
        local found_ver=$("$TAKYON_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Takyon-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.takyon/node/..."
    ...
    # Place into ~/.takyon/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$TAKYON_HOME/node"
    mkdir -p "$TAKYON_HOME"
    mv "$extracted_dir" "$TAKYON_HOME/node"
    rm -rf "$tmp_dir"
    ...
    local installed_ver
    installed_ver=$("$TAKYON_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.takyon/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Takyon install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Takyon Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="takyon-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Takyon behaves unexpectedly."
                    else
...
try:
    ...
    extras = []
    for s in specs:
        m = re.search(r"takyon-agent\[([\w-]+)\]", s)
        if m:
...
setup_path() {
    log_info "Setting up takyon command..."

    if [ "$USE_VENV" = true ]; then
        TAKYON_BIN="$INSTALL_DIR/venv/bin/takyon"
    else
        TAKYON_BIN="$(which takyon 2>/dev/null || echo "")"
        if [ -z "$TAKYON_BIN" ]; then
            log_warn "takyon not found on PATH after install"
            return 0
        fi
    fi

    # Verify the entry point script was actually generated
    if [ ! -x "$TAKYON_BIN" ]; then
        log_warn "takyon entry point not found at $TAKYON_BIN"
    ...
    # Create a user-facing shim for the takyon command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $TAKYON_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$TAKYON_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/takyon"
    cat > "$command_link_dir/takyon" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$TAKYON_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/takyon"
    log_success "Installed takyon launcher → $command_link_display_dir/takyon"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "takyon command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v takyon' \
                >/dev/null 2>&1; then
        ...
        log_info "takyon not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Takyon Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Takyon Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Takyon Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so takyon works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.takyon directory structure (config at top level, code in subdir)
    mkdir -p "$TAKYON_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.takyon/.env (top level, easy to find)
    if [ ! -f "$TAKYON_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$TAKYON_HOME/.env"
            log_success "Created ~/.takyon/.env from template"
        else
            touch "$TAKYON_HOME/.env"
            log_success "Created ~/.takyon/.env"
        fi
    else
        log_info "~/.takyon/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$TAKYON_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.takyon/config.yaml (top level, easy to find)
    if [ ! -f "$TAKYON_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$TAKYON_HOME/config.yaml"
            log_success "Created ~/.takyon/config.yaml from template"
        fi
    else
        log_info "~/.takyon/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$TAKYON_HOME/SOUL.md" ]; then
        cat > "$TAKYON_HOME/SOUL.md" << 'SOUL_EOF'
# Takyon Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how Takyon communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.takyon/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.takyon/"

    # Seed bundled skills into ~/.takyon/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.takyon/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.takyon/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$TAKYON_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$TAKYON_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.takyon/skills/"
        fi
    fi
}
...
configure_browser_env_from_system_browser() {
    local env_file="$TAKYON_HOME/.env"
    ...
    {
        echo ""
        echo "# Takyon Agent browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; Takyon will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (takyon --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'takyon setup' after install."
        return 0
    fi
    ...
    # Run takyon setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m takyon_cli.main setup < /dev/tty
    else
        python -m takyon_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$TAKYON_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Takyon to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$TAKYON_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'takyon whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                TAKYON_CMD="$(get_takyon_command_path)"
                $TAKYON_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'takyon whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'takyon gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $TAKYON_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $TAKYON_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: takyon gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: takyon gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $TAKYON_CMD gateway > "$TAKYON_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.takyon/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: takyon gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: takyon gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $TAKYON_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $TAKYON_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $TAKYON_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}takyon${NC}              Start chatting"
    echo -e "   ${GREEN}takyon setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}takyon config${NC}       View/edit configuration"
    echo -e "   ${GREEN}takyon config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}takyon gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}takyon update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'takyon' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'takyon' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'takyon' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$TAKYON_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$TAKYON_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$TAKYON_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$TAKYON_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$TAKYON_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Takyon for pip install"
    ...
    TAKYON_CMD="$(command -v takyon 2>/dev/null || echo "")"
    if [ -n "$TAKYON_CMD" ]; then
        log_info "Running takyon setup..."
        "$TAKYON_CMD" setup
    else
        log_warn "takyon command not found on PATH"
        log_info "Try: python -m takyon_cli.main setup"
    fi
}
...
main() {
    ...
    echo "git" > "$TAKYON_HOME/.install_method"
}
