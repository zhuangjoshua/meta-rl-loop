...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with takyon-agent)
2. User plugins: ``$TAKYON_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$TAKYON_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$TAKYON_HOME/plugins/model-providers/`` if it exists."""
    try:
        from takyon_constants import get_takyon_home

        d = get_takyon_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple TAKYON_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_takyon_user_provider_{safe_name}"
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$TAKYON_HOME/plugins/model-providers/<name>/``
    ...
    # 1. Bundled plugins — shipped with takyon-agent.
    ...
    # 2. User plugins — under $TAKYON_HOME/plugins/model-providers/<name>/.
