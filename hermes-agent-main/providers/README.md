...
Registry and ABC for every inference provider Takyon knows about.
...
`$TAKYON_HOME/plugins/model-providers/<name>/` (per-user overrides). The
...
- `takyon_cli/auth.py` extends `PROVIDER_REGISTRY` with every api-key
  profile it sees (skipping `copilot`, `kimi-coding`, `kimi-coding-cn`,
  `zai`, `openrouter`, `custom` — those need bespoke token resolution).
- `takyon_cli/models.py` extends `CANONICAL_PROVIDERS` and calls
  `profile.fetch_models()` inside `provider_model_ids()`.
- `takyon_cli/doctor.py` adds a `/models` health check for each
  `auth_type="api_key"` profile.
- `takyon_cli/config.py` injects every `env_var` into
  `OPTIONAL_ENV_VARS` so the setup wizard knows about it.
- `takyon_cli/runtime_provider.py` reads `profile.api_mode` as a fallback
...
under `$TAKYON_HOME/plugins/model-providers/` for a private plugin).
