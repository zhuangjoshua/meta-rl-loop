...
# 5. Symlinks the 'takyon' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Takyon Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/takyon-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink takyon into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up takyon command..."

TAKYON_BIN="$SCRIPT_DIR/venv/bin/takyon"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$TAKYON_BIN" "$COMMAND_LINK_DIR/takyon"
echo -e "${GREEN}✓${NC} Symlinked takyon → $COMMAND_LINK_DISPLAY_DIR/takyon"
...
# ============================================================================
# Seed bundled skills into ~/.takyon/skills/
# ============================================================================

TAKYON_SKILLS_DIR="${TAKYON_HOME:-$HOME/.takyon}/skills"
mkdir -p "$TAKYON_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.takyon/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$TAKYON_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  takyon status        # Check configuration"
if is_termux; then
    echo "  takyon gateway       # Run gateway in foreground"
else
    echo "  takyon gateway install # Install gateway service (messaging + cron)"
fi
echo "  takyon cron list     # View scheduled jobs"
echo "  takyon doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m takyon_cli.main setup
fi
