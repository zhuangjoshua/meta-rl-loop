...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, Takyon automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
If you already have an API key set in `.env`, Takyon auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
takyon auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
takyon auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (requires Claude Max plan + extra usage credits)
takyon auth add anthropic --type oauth
...
anthropic (3 credentials):
  #1  takyon_pkce          oauth   takyon_pkce ←
...
Run `takyon auth` with no subcommand for an interactive wizard:
...
| Command | Description |
|---------|-------------|
| `takyon auth` | Interactive pool management wizard |
| `takyon auth list` | Show all pools and credentials |
| `takyon auth list <provider>` | Show a specific provider's pool |
| `takyon auth add <provider>` | Add a credential (prompts for type and key) |
| `takyon auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `takyon auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `takyon auth remove <provider> <index>` | Remove credential by 1-based index |
| `takyon auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `takyon auth` → "Set rotation strategy" or in `config.yaml`:
...
When you set up a custom endpoint via `takyon model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via takyon model:
takyon auth list
# Shows:
...
takyon auth add Together.ai --api-key sk-together-second-key
```
...
Takyon automatically discovers credentials from multiple sources and seeds the pool on startup:
...
| Takyon PKCE OAuth | `~/.takyon/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `takyon auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `takyon auth add`) are never auto-pruned.
...
2. **`takyon_cli/auth_commands.py`** — CLI commands and interactive wizard
3. **`takyon_cli/runtime_provider.py`** — Pool-aware credential resolution
4. **`run_agent.py`** — Error recovery: 429/402/401 → pool rotation → fallback

## Storage

Pool state is stored in `~/.takyon/auth.json` under the `credential_pool` key:
