...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Takyon Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Takyon Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Takyon](/docs/guides/use-voice-mode-with-takyon).

## Prerequisites

Before using voice features, make sure you have:

1. **Takyon Agent installed** — `pip install takyon-agent` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `takyon model` or set your preferred provider credentials in `~/.takyon/.env`
3. **A working base setup** — run `takyon` to verify the agent responds to text before enabling voice

:::tip
The `~/.takyon/` directory and default `config.yaml` are created automatically the first time you run `takyon`. You only need to create `~/.takyon/.env` manually for API keys.
:::
...
pip install "takyon-agent[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
pip install "takyon-agent[messaging]"

# Premium TTS (ElevenLabs)
pip install "takyon-agent[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "takyon-agent[all]"
```
...
Voice mode is available in both the **classic CLI** (`takyon chat`) and the **TUI** (`takyon --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.takyon/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
takyon                # Start the interactive CLI
```
...
1. Start the CLI with `takyon` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.takyon/config.yaml` (default: `ctrl+b`).
:::
...
```bash
takyon gateway        # Start the gateway (connects to configured platforms)
takyon gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@takyonbyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.takyon/.env`:
```bash
...
```bash
takyon gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.takyon/.env`
...
- Review gateway logs: `tail -f ~/.takyon/logs/gateway.log`
