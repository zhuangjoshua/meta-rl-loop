...
chat completions — use your Takyon-managed provider subscription as their
...
| Use case | "Use Takyon as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Takyon stores
the refresh token in `~/.takyon/auth.json` — the same place all Takyon
...
```
Starting Takyon proxy for Nous Portal
...
```
Takyon proxy upstream adapters

  [nous    ] Nous Portal — ready (bearer expires 2026-05-15T06:43:21Z)
```

If you see `not logged in`, run `takyon login nous`. If you see
...
```bash
takyon proxy start --host 0.0.0.0 --port 8645
```
...
`takyon_cli/proxy/adapters/<provider>.py` and registering it in
