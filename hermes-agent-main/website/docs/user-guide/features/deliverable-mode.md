...
When Takyon Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
custom instructions in `~/.takyon/config.yaml` under `agent.custom_instructions`.
...
If you use Takyon' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.takyon/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Takyon Agent's deliverable
