...
All skills live in **`~/.takyon/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point Takyon at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Takyon to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.takyon/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
takyon chat --toolsets skills -q "What skills do you have?"
takyon chat --toolsets skills -q "Show me the axolotl skill"
```
...
/home/user/.takyon/cache/chart-q4-2025.png
...
When a missing value is encountered, Takyon asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `takyon setup` or `~/.takyon/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `takyon config migrate` prompts for unconfigured settings, and `takyon config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.takyon/skills/                  # Single source of truth
...
If you maintain skills outside of Takyon — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Takyon to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.takyon/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.takyon/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the Takyon process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Takyon ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.takyon/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Bundles live in **`~/.takyon/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `takyon bundles list`.
...
takyon bundles create research

# Overwrite an existing bundle
takyon bundles create backend-dev --skill ... --force

# Delete a bundle
takyon bundles delete backend-dev

# Re-scan ~/.takyon/skill-bundles/ and report changes
takyon bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.takyon/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.takyon/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
takyon skills list --source hub                   # List hub-installed skills
takyon skills check                               # Check installed hub skills for upstream updates
takyon skills update                              # Reinstall hub skills with upstream changes when needed
takyon skills audit                               # Re-scan all hub skills for security
takyon skills uninstall k8s                       # Remove a hub skill
takyon skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
takyon skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
takyon skills publish skills/my-skill --to github --repo owner/repo
takyon skills snapshot export setup.json          # Export skill config
takyon skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
```bash
takyon skills search https://mintlify.com/docs --source well-known
takyon skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
takyon skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```
...
```bash
takyon skills install openai/skills/k8s
...
- Takyon source id: `browse-sh`
- Trust level: `community`

```bash
takyon skills search airbnb --source browse-sh
takyon skills inspect browse-sh/airbnb.com/search-listings-ddgioa
takyon skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
takyon skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
takyon skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `takyon skills update` re-fetches from the same URL automatically when you want to refresh.
...
`takyon skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
```bash
takyon skills check          # Report which installed hub skills changed upstream
takyon skills update         # Reinstall only the skills with updates available
takyon skills update react   # Update one specific installed hub skill
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Takyon users add with `takyon skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.takyon.tags`, `version`, `author`, `platforms`, `metadata.takyon.config`).
...
Takyon discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.

#### Minimal tap example

```
my-org/takyon-skills
└── skills/
    └── deploy-runbook/
        └── SKILL.md
```
...
After pushing that to GitHub, any Takyon user can subscribe and install:

```bash
takyon skills tap add my-org/takyon-skills
takyon skills search deploy
takyon skills install my-org/takyon-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.takyon/.hub/taps.json`:
...
The `takyon skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `takyon skills tap list` shows the effective path per tap.
...
```bash
takyon skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.takyon/.hub/taps.json` (created on demand).

## Bundled skill updates (`takyon skills reset`)

Takyon ships with a set of bundled skills in `skills/` inside the repo. On install and on every `takyon update`, a sync pass copies those into `~/.takyon/skills/` and records a manifest at `~/.takyon/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Takyon recomputes the hash of your local copy and compares it to the origin hash:
...
takyon skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `TAKYON_HOME`, so `takyon -p coder skills reset <name>` only affects that profile.
:::
