...
Takyon runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Takyon
...
1. Takyon captures a baseline of current diagnostics for the file.
...
…). Takyon auto-detects the binary on PATH or in
`<TAKYON_HOME>/lsp/bin/`.
...
`node_modules` tree — Takyon installs both packages together when you
run `takyon lsp install typescript` or auto-install fires on first
use.

## CLI

```
takyon lsp status          # service state + per-server install status
takyon lsp list            # registry, optionally --installed-only
takyon lsp install <id>    # eagerly install one server
takyon lsp install-all     # try every server with a known recipe
takyon lsp restart         # tear down running clients
takyon lsp which <id>      # print resolved binary path
```

`takyon lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <TAKYON_HOME>/lsp/bin
...
When `install_strategy: auto`, Takyon installs binaries into
`<TAKYON_HOME>/lsp/bin/`. NPM packages land in
`<TAKYON_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Takyon-owned and is
...
Servers are kept alive for the life of the Takyon process. There's
...
**`takyon lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<TAKYON_HOME>/lsp/bin/`. Run
`takyon lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `takyon lsp status`**
...
When `takyon lsp status` shows a `Backend warnings` section, install
...
`~/.takyon/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.takyon/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `takyon lsp restart` to clear the set;
