...
description: "Connect Takyon Agent to external tool servers via MCP — and control exactly which MCP tools Takyon loads"
---

# MCP (Model Context Protocol)

MCP lets Takyon Agent connect to external tool servers so the agent can use tools that live outside Takyon itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted Takyon to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

## What MCP gives you

- Access to external tool ecosystems without writing a native Takyon tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want Takyon to see
...
```bash
cd ~/.takyon/takyon-agent
uv pip install -e ".[mcp]"
```

2. Add an MCP server to `~/.takyon/config.yaml`:
...
4. Ask Takyon to use the MCP-backed capability.
...
Takyon will discover the MCP server's tools and use them like any other tool.
...
HTTP MCP servers are remote endpoints Takyon connects to directly.
...
- you do not want Takyon spawning a local subprocess for that integration

## Basic configuration reference

Takyon reads MCP config from `~/.takyon/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, Takyon skips the server entirely |
...
For well-known MCP servers, `takyon mcp add` accepts a `--preset` flag that fills in the transport details so you don't have to look up the command and args. The preset only supplies defaults — anything else (env vars, headers, filtering) you pass on the same command line still wins.
...
takyon mcp add codex --preset codex
```
...
You can pick any local name (`takyon mcp add my-codex --preset codex` is fine); the preset only provides the `command`/`args` defaults.

## How Takyon registers MCP tools

Takyon prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — Takyon sees the tool and chooses it during normal reasoning.

## MCP utility tools

When supported, Takyon also registers utility tools around MCP resources and prompts:
...
- Takyon only registers resource utilities if the MCP session actually supports resource operations
- Takyon only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to Takyon, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, Takyon skips the server completely and does not even attempt a connection.
...
You can also separately disable Takyon-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, Takyon does not create an empty runtime MCP toolset for that server.
...
Takyon discovers MCP servers at startup and registers their tools into the normal tool registry.

### Dynamic Tool Discovery

MCP servers can notify Takyon when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When Takyon receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, Takyon does not blindly pass your full shell environment.
...
Then verify your config and restart Takyon.
...
Because Takyon now only registers those wrappers when both are true:
...
When `supports_parallel_tool_calls` is `true`, Takyon may execute multiple tools from that server at the same time within a single tool-call batch, just like it does for built-in read-only tools (web_search, read_file, etc.).
...
MCP servers can request LLM inference from Takyon via the `sampling/createMessage` protocol. This allows an MCP server to ask Takyon to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running Takyon as an MCP server

In addition to connecting **to** MCP servers, Takyon can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use Takyon's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through Takyon
- You want a single MCP server that bridges to all of Takyon's connected messaging platforms at once
- You already have a running Takyon gateway with connected platforms
...
Add Takyon to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed Takyon in a specific location:

```json
{
  "mcpServers": {
    "takyon": {
      "command": "/home/user/.takyon/takyon-agent/venv/bin/takyon",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a Takyon-specific channel browser:
...
The MCP server includes a live event bridge that polls Takyon's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
```bash
takyon mcp serve              # Normal mode
takyon mcp serve --verbose    # Debug logging on stderr
```

### How it works

The MCP server reads conversation data directly from Takyon's session store (`~/.takyon/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same `send_message` infrastructure as the Takyon agent itself.
...
- The embedded `takyon mcp serve` exposes a **stdio-only** MCP server today. If you need an HTTP MCP server, run a separate adapter — or, much more commonly, use the MCP **client** side of Takyon, which already speaks both stdio and HTTP (`url` + `headers` in `mcp_servers.yaml` / `config.yaml`; see [HTTP servers](#http-servers) above).
