...
**Multi-agent profiles**: When multiple Takyon instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
takyon memory setup    # select "honcho" from the provider list
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$TAKYON_HOME/honcho.json` (profile-local). The setup wizard handles this for you.
...
- `per-session` — each `takyon` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Takyon syncs them back at session init.
...
The `takyon honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). Run `takyon memory setup` and pick Honcho first; the subcommand appears on the next invocation.

```bash
takyon honcho status          # Connection status, config, and key settings
takyon honcho setup           # Redirects to `takyon memory setup`
takyon honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
takyon honcho peer            # Show or update peer names + dialectic reasoning level
takyon honcho mode            # Show or set recall mode (hybrid/context/tools)
takyon honcho tokens          # Show or set token budget for context and dialectic
takyon honcho identity        # Seed or show the AI peer's Honcho identity
takyon honcho sync            # Sync Honcho config to all existing profiles
takyon honcho peers           # Show peer identities across all profiles
takyon honcho sessions        # List known Honcho session mappings
takyon honcho map             # Map current directory to a Honcho session name
takyon honcho enable          # Enable Honcho for the active profile
takyon honcho disable         # Disable Honcho for the active profile
takyon honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `takyon honcho`

If you previously used the standalone `takyon honcho setup`:
...
No re-login or re-setup needed. Run `takyon memory setup` and select "honcho" — the wizard detects your existing config.
