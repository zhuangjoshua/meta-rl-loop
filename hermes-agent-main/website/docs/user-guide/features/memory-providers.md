...
Takyon Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
takyon memory setup      # interactive picker + configuration
takyon memory status     # check what's active
takyon memory off        # disable external provider
```

You can also select the active memory provider via `takyon plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.takyon/config.yaml`:
...
When a memory provider is active, Takyon automatically:
...
**Setup Wizard:**
```bash
takyon memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `takyon honcho setup` command still works (it now redirects to `takyon memory setup`), but is only registered after Honcho is selected as the active memory provider.

**Config:** `$TAKYON_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$TAKYON_HOME/honcho.json` > `~/.takyon/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/takyon-ai/takyon-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/takyon).
...
:::tip Migrating from `takyon honcho`
If you previously used `takyon honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Takyon profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Takyon profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Takyon profile. Host key `takyon` → default; `takyon.<profile>` for others. |
...
```bash
takyon profile create coder --clone
```

`--clone` creates a `takyon.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Takyon profile, creates host blocks for any profile without one, inherits settings from the default `takyon` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
See the [config reference](https://github.com/takyon-ai/takyon-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/takyon).
...
takyon memory setup    # select "openviking"
# Or manually:
takyon config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.takyon/.env
```
...
**Setup:**
```bash
takyon memory setup    # select "mem0"
# Or manually:
takyon config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.takyon/.env
```

**Config:** `$TAKYON_HOME/mem0.json`

| Key | Default | Description |
|-----|---------|-------------|
| `user_id` | `takyon-user` | User identifier |
| `agent_id` | `takyon` | Agent identifier |
...
**Setup:**
```bash
takyon memory setup    # select "hindsight"
# Or manually:
takyon config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.takyon/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p takyon ui start`

**Config:** `$TAKYON_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `takyon` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Takyon Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/NousResearch/takyon-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
takyon memory setup    # select "holographic"
# Or manually:
takyon config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.takyon-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$TAKYON_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
takyon memory setup    # select "retaindb"
# Or manually:
takyon config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.takyon/.env
```
...
# Then configure Takyon
takyon memory setup    # select "byterover"
# Or manually:
takyon config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$TAKYON_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
takyon memory setup    # select "supermemory"
# Or manually:
takyon config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.takyon/.env
```

**Config:** `$TAKYON_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `takyon` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `takyon-{identity}` → `takyon-coder`) to isolate memories per Takyon profile
...
- **Local storage providers** (Holographic, ByteRover) use `$TAKYON_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$TAKYON_HOME/` so each profile has its own credentials
