...
Takyon Agent can drive your Mac's desktop — clicking, typing, scrolling,
...
```
takyon computer-use install
```

This fetches and runs the upstream cua-driver installer:
`curl -fsSL https://raw.githubusercontent.com/trycua/cua/main/libs/cua-driver/scripts/install.sh`.
Use `takyon computer-use status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `takyon tools`, pick `🖱️ Computer Use (macOS)` → `cua-driver (background)`.
...
- **`takyon computer-use install --upgrade`** — manual force-refresh.
...
`takyon computer-use status` shows the installed version next to the
binary path.
...
Takyon applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.takyon/config.yaml` if you want every action confirmed.

## Token efficiency

Screenshots are expensive. Takyon applies four layers of optimisation:
...
- **Private SPI risk.** Apple can change SkyLight's symbol surface in any
  OS update. Pin the driver version with the `TAKYON_CUA_DRIVER_VERSION`
...
```
TAKYON_CUA_DRIVER_CMD=/opt/homebrew/bin/cua-driver
TAKYON_CUA_DRIVER_VERSION=0.5.0    # optional pin
```

Swap the backend entirely (for testing):

```
TAKYON_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

## Troubleshooting

**`computer_use backend unavailable: cua-driver is not installed`** — Run
`takyon computer-use install` to fetch the cua-driver binary, or run
`takyon tools` and enable the Computer Use toolset.
...
- [Universal skill: `macos-computer-use`](https://github.com/NousResearch/takyon-agent/blob/main/skills/apple/macos-computer-use/SKILL.md)
