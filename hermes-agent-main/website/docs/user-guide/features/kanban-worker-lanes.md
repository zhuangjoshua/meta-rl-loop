...
- **Plugin / integration authors** wanting to add a new lane shape (a CLI worker that wraps Codex / Claude Code / OpenCode, a containerised review worker, a non-Takyon service that pulls tasks via the API).

If you're writing the worker code itself — the agent that runs *inside* a lane — the [`kanban-worker`](https://github.com/NousResearch/takyon-agent/blob/main/skills/devops/kanban-worker/SKILL.md) skill is the deeper procedural detail.

## The hierarchy

```text
Takyon Kanban  =  canonical task lifecycle + audit trail
...
Takyon Kanban owns lifecycle truth — `ready` → `running` → `blocked` / `done` / `archived`. Worker lanes execute work but never own that truth; everything they do flows back through the kanban kernel via the `kanban_*` tools (or, for non-Takyon external workers, via the API). Reviewers gate the transition from "code change written" to "task done."
...
The dispatcher matches `task.assignee` against either a Takyon profile name (the default lane shape) or a registered non-spawnable identifier (the plugin lane shape — see [Adding an external CLI worker lane](#adding-an-external-cli-worker-lane) below). Tasks whose assignee doesn't resolve are left on `ready` with a `skipped_nonspawnable` event so a board operator can fix them; they are not silently dropped or executed by an arbitrary fallback.

### 2. A spawn mechanism

For Takyon profile lanes, the dispatcher's `_default_spawn` runs `takyon -p <assignee> chat -q <prompt>` (or the equivalent module form when the `takyon` shim isn't on `$PATH`) inside the task's pinned workspace, with these env vars set:

| Variable | Carries |
|---|---|
| `TAKYON_KANBAN_TASK` | the task id the worker is operating on |
| `TAKYON_KANBAN_DB` | absolute path to the per-board SQLite file |
| `TAKYON_KANBAN_BOARD` | board slug |
| `TAKYON_KANBAN_WORKSPACES_ROOT` | root of the board's workspace tree |
| `TAKYON_KANBAN_WORKSPACE` | absolute path to *this* task's workspace |
| `TAKYON_KANBAN_RUN_ID` | the current run's id (for the lifecycle gate) |
| `TAKYON_KANBAN_CLAIM_LOCK` | the claim lock string (`<host>:<pid>:<uuid>`) |
| `TAKYON_PROFILE` | the worker's own profile name (for `kanban_comment` author attribution) |
| `TAKYON_TENANT` | tenant namespace, if the task has one |

For non-Takyon lanes (registered via a plugin), the plugin supplies its own `spawn_fn` callable that gets `task`, `workspace`, and `board` and returns an optional pid for crash detection.
...
- **Block instead of complete**, with `reason` prefixed `review-required: ` so the dashboard / `takyon kanban show` surfaces the row as awaiting review.
...
The [`kanban-worker`](https://github.com/NousResearch/takyon-agent/blob/main/skills/devops/kanban-worker/SKILL.md) skill has worked examples for both `kanban_complete` (truly terminal tasks — typo fixes, docs changes, research writeups) and the `review-required` block pattern.
...
The historical issue for this is [#19931](https://github.com/NousResearch/takyon-agent/issues/19931) and the closed-not-merged Codex-specific PR [#19924](https://github.com/NousResearch/takyon-agent/pull/19924) — those describe the original architecture proposal but didn't land a runner.
