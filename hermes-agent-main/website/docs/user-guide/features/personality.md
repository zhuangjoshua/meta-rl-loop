...
description: "Customize Takyon Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Takyon Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `TAKYON_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Takyon is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Takyon now seeds a default `SOUL.md` automatically in:

```text
~/.takyon/SOUL.md
```

More precisely, it uses the current instance's `TAKYON_HOME`, so if you run Takyon with a custom home directory, it will use:
...
- Takyon creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Takyon loads `SOUL.md` only from `TAKYON_HOME`
- Takyon does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Takyon falls back to a built-in default identity
...
If Takyon loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `TAKYON_HOME`, the personality belongs to the Takyon instance itself.

That also makes it easier to teach users:
- "Edit `~/.takyon/SOUL.md` to change Takyon' default personality."
...
- how Takyon should handle uncertainty, disagreement, or ambiguity
...
## What Takyon injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Takyon falls back to a built-in default identity ("You are Takyon Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Takyon ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Takyon, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Takyon its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.takyon/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.takyon/SOUL.md`
...
- [SOUL.md Guide](/docs/guides/use-soul-with-takyon)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Takyon speaks
- `display.skin` and `/skin` affect how Takyon looks in the terminal
