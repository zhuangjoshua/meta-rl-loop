---
title: "Takyon Static Ad Creative Generator"
sidebar_label: "Takyon Static Ad Creative Generator"
description: "Design and generate business-scoped static performance-ad image variants from a factual brief"
---
...
Design and generate business-scoped static performance-ad image variants from a factual brief. Use when Meta, Instagram, Facebook, or Reddit needs reviewable image-ad angles, specs, prompts, and real renders. Do not use for logos, generic art, video, long-form copy, or live ad-account operation and measurement.
...
| Path | `skills/takyon/static-ad-creative-generator` |
...
> This section expands the native frontmatter description that Claude uses for autonomous routing.
> If they ever disagree, fix both in the same change.
...
- **Scripts dir:** `scripts/`, resolved relative to this native skill directory
...
- **Batch helper script:** `python scripts/batch_generate.py <batch.json> -o product/static-ads/<slug>/`
...
- **Default backend/model:** the guarded creative provider registry; the calling Agent SDK session
  never handles provider credentials.

## Prerequisites

- **Python 3.9+.**
- **Live mode (real render, real spend)** must use `business_static_ad_generate`, which gates
  creative credits and routes provider keys through Safebox. Standalone developer diagnostics need
  the `openai`
...
Common Agent SDK path (one creative, live render):

```bash
pip install -r scripts/requirements.txt
python scripts/validate_spec.py examples/example-spec.json        # must be 0 errors
```

Then call `business_static_ad_generate` with `input_path`, `slug`, a fresh
`idempotency_key`, and `budget_bucket` or `ad_metadata.channel`. For multi-size fan-out pass
`aspect_ratio: "1:1,4:5,1.91:1,9:16"`; for a batch pass the batch JSON as `input_path`.

- The **only step with an external effect / real spend** is
  `business_static_ad_generate`. Everything before it is local and deterministic.
- The bundled generation/back-end scripts preserve the original implementation method for
  maintainer diagnostics, but native Agent SDK execution never uses a direct provider-key path.
...
6. **Validate.** `python scripts/validate_spec.py <spec|batch|dir>`, resolving the path from this
   skill directory.
   Fix every ERROR; resolve each WARN.
7. **Generate into the canonical directory.** Call `business_static_ad_generate` with the
   validated spec or batch path and publication slug. Never substitute placeholders or mock output
   for a claimed creative.
...
| Problem | Fix |
| --- | --- |
| Provider unavailable | Use `business_static_ad_generate`; report its exact Safebox/provider blocker without requesting a key. |
...
| `model not found` / no access | Report the guarded provider blocker; provider selection belongs to the creative registry. |
