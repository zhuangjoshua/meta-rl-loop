---
title: "Takyon App Runtime"
sidebar_label: "Takyon App Runtime"
description: "Configure and verify a business application's declared customer-facing runtime features and real signed-in actions"
---
...
Configure and verify a business application's declared customer-facing runtime features and real signed-in actions. Use when auth, profiles, plans, entitlements, checkout, usage, or actions must be wired and proven through existing rails. Do not use to administer real customers, forge payment/webhook state, or perform purely visual layout work.
...
| Path | `skills/takyon/takyon-app-runtime` |
...
- Tool names used by this skill: `business_read_business`, `business_read_file`, `business_upsert_app_surface_contract`, `business_check_runtime_capabilities`, `business_upsert_app_plan`, `business_upsert_app_customer`, `business_upsert_app_profile`, `business_grant_app_entitlement`, `business_supabase_login`, `business_read_app_account`, `business_read_app_profile`, `business_create_app_checkout`, `business_record_stripe_webhook`, `business_record_app_usage`, `business_invoke_app_action`, `business_send_app_email`, `business_write_file`, `business_patch_file`
...
- Product media (images/avatars): `media` is default-on; upload from the UI through the kit's `uploadMedia(file)` (multipart) and reference stored images via `mediaUrl(id)` — never base64-into-records. Allowed types image/jpeg,png,webp,gif under per-user (50MB) and per-business (1GB) byte quotas; serving is session-gated; each store meters as a `media_store` usage event with a receipt under `metrics/receipts/app-media/`. `business_list_app_media` reports usage/quota. Records-v2: for feeds/browse, pass `listRecords({filters, sort, cursor})` (bounded server-side query) instead of fetching all records.
- Product email: when email is already present on the surface contract, send with `business_send_app_email` (recipient `app_user_id`, `subject`, `text`, slug-like `purpose`, `idempotency_key`). Test mode suppresses the real send and writes a receipt under `metrics/receipts/app-email/`; live sends go through the platform Postmark credentials and meter as `email_send` usage events with a per-business daily cap. Schedule-triggered actions send via `POST email/send` with their service session; customer sessions and service-identity recipients are refused truthfully. Verification = send in test mode, then read back the receipt and the `email_send` usage event — a suppressed test send proves the rail wiring, not live deliverability
...
- If `product/surface.md` has no real `source_path` or `product/site/` does not exist yet, route software-business bootstrap work back through `takyon-product` before expanding runtime mirror docs.
...
- Use `business_upsert_app_surface_contract` if the app surface contract is missing routes, source path, or truthful runtime notes. Do not select `runtime_features`: bootstrap defaults `auth`, `account`, `profile`, `records`, `actions`, `media`, `checkout`, `entitlements`, and `usage`; product refresh auto-adds or removes `generate`/`search` from source use and writes the rail receipt.
- Use `business_upsert_app_plan` for plan policy and pricing metadata. The product AI usage budget is NOT a separate operator cap: it derives strictly from the active paid subscription's `included_ai_budget_microusd` (the `y` term of x+y+z). No active paid subscription means 0 budget and product AI spend is refused (402); there is no flat per-business pool cap and no free-tier floor.
- Use `business_upsert_app_customer`, `business_upsert_app_profile`, `business_grant_app_entitlement`, `business_supabase_login`, `business_read_app_account`, and `business_read_app_profile` for customer, profile, session, and entitlement flows. Product browsers complete Supabase Auth first, then exchange the verified Supabase access token through `business_supabase_login` / `POST auth/session`.
...
- To set the customer-facing product chat/assistant VOICE (the `generate` rail — this is the product app's assistant, NOT the operator CEO shell), pass `tone` to `business_upsert_app_surface_contract`. Presets: `default` (neutral) and `poke` (short 1-3 sentences, warm and lightly playful, zero corporate hedging, always ends on a proactive next step, at most one emoji). The selection records on the surface contract, shows in `product/surface.md` under "Product chat tone", and is injected into the product-build worker contract so the generated assistant's system prompt is written in that voice. Omitting `tone` keeps the prior selection; it never alters the operator shell tone.
- The prepared `_takyon/` kit inside `product/site/` is a shared substrate, not a claim that
  every rail is live. Keep `rail_state` truthful so the kit and the primary SDK session see the
  same runtime reality.
...
2. Read `runtime_features` from the app surface contract as runtime truth; do not choose or patch rails manually.
3. If product source adds or removes `generate`/`search`, run the normal product refresh so policy reconciles the contract and writes `metrics/receipts/runtime-rails/`; route product-source repairs through `takyon-product`.
4. During bootstrap for a software business, do not let these runtime mirror files become the first main product artifact. If `product/site/` is still missing or `product/surface.md` has no real source path, route back to `takyon-product` first unless runtime-first work was explicitly requested.
5. For plan and budget work, call `business_upsert_app_plan` before editing any notes — it defines the real pricing and each plan's `included_ai_budget_microusd`, which IS the product AI usage budget (the `y` term). There is no separate operator usage-cap tool: budget is plan-derived (active paid subscription → its included AI budget; no subscription → 0 → product AI spend refused with 402), with no flat per-business pool cap and no free-tier floor.
6. For customer/profile/auth work, use `business_upsert_app_customer`, `business_upsert_app_profile`, `business_grant_app_entitlement`, `business_supabase_login`, `business_read_app_account`, and `business_read_app_profile` in the order needed by the flow. If Supabase or another provider credential is missing, keep the blocker visible instead of faking a session.
...
8. For usage metering, call `business_record_app_usage`. Product AI generation is metered through the canonical runtime route (`POST /generate` on product hosts, or `POST /api/takyon/apps/<slug>/generate` off-host): that route brokers server-side through the shared Takyon AI authority and reserves then settles against the plan-derived budget (the paying sub-user's active subscription `included_ai_budget_microusd`; no active paid subscription → 0 budget → 402). On the pinned Vite scaffold, action code reaches that broker via `ctx`; the browser should call a named action, not `/generate` directly. Until a dedicated usage read rail is finished on the app plane, treat `business_read_app_account` as the current read source for usage summary in the product shell. Surface `402` (over budget) and `503` (generation not configured) honestly, and never embed the platform provider key in the app or call internal authority endpoints directly from product code.
9. For product actions, treat the code as the source of truth: confirm UI action call names match real files under `product/site/actions/`, and verify those actions with real receipts. For the exact gated workflow doctrine, read `takyon-product`; for the runtime recipe, this skill is authoritative.
...
11. HTTP action verification recipe: in test mode, sign in as a normal test customer through the Supabase browser flow, capture the resulting Takyon `session_token` from `POST auth/session`, then call `business_invoke_app_action {business, action, payload, session_token}`. The invoke tool never mints or impersonates sessions for you.
...
- `product/surface.md` should summarize the real shared runtime contract, policy-derived rails, and visible blockers.
