...
description: "Configure and use Honcho memory with Takyon -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Takyon -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `takyon skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Takyon

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Takyon profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/takyon#running-honcho-locally-with-takyon

### Verify

```bash
takyon honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Takyon profile's AI peer
...
Honcho models conversations as interactions between **peers**. Takyon creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Takyon instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Takyon run |
| `global` | Single session across all directories |

Manual override: `takyon honcho map my-project-name`
...
Each Takyon profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block takyon.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `takyon.coder` host block in `honcho.json`
...
```bash
takyon honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Takyon when Honcho memory is active.
...
Config file: `$TAKYON_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `takyon honcho setup`. Ensure `memory.provider: honcho` is in `~/.takyon/config.yaml`.

### Memory not persisting across sessions
Check `takyon honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `takyon profile create <name> --clone`. For existing profiles: `takyon honcho sync`.
...
| Command | Description |
|---------|-------------|
| `takyon honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `takyon honcho status` | Show resolved config, connection test, peer info for active profile |
| `takyon honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `takyon honcho disable` | Disable Honcho for the active profile |
| `takyon honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `takyon honcho peers` | Show peer identities across all profiles |
| `takyon honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `takyon honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `takyon honcho sessions` | List known directory-to-session-name mappings |
| `takyon honcho map <name>` | Map current working directory to a Honcho session name |
| `takyon honcho identity` | Seed AI peer identity or show both peer representations |
| `takyon honcho sync` | Create host blocks for all Takyon profiles that don't have one yet |
| `takyon honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Takyon + Honcho |
| `takyon memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `takyon memory status` | Show active memory provider and config |
| `takyon memory off` | Disable external memory provider |
