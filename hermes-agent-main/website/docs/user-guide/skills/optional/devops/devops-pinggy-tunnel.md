...
| | |
|---|---|
| Source | Optional — install with `takyon skills install official/devops/pinggy-tunnel` |
| Path | `optional-skills/devops/pinggy-tunnel` |
| Version | `0.1.0` |
| Author | Teknium (teknium1), Takyon Agent |
| License | MIT |
...
The remote client connects to `$URL` with `Authorization: Bearer $TOKEN`. Takyon' own native MCP client config: `{"transport": "http", "url": "<URL>", "headers": {"Authorization": "Bearer <TOKEN>"}}`.
