...
| | |
|---|---|
| Source | Optional — install with `takyon skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Takyon or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Takyon. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.takyon/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.takyon/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Takyon integration, either:

- configure the server in `~/.takyon/config.yaml` using the `native-mcp` skill, or
...
### Takyon cannot see the deployed server

The server-building part may be correct while the Takyon config is not. Load the `native-mcp` skill and configure the server in `~/.takyon/config.yaml`, then restart Takyon.
