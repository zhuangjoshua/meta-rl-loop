...
| | |
|---|---|
| Source | Optional — install with `takyon skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Takyon Agent |
| License | MIT |
...
import upstream classes into Takyon itself.
...
```bash
mkdir -p ~/.takyon/cache/darwinian-evolver && cd ~/.takyon/cache/darwinian-evolver
...
```bash
cd ~/.takyon/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.takyon/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.takyon/skills/research/darwinian-evolver
DE_DIR=~/.takyon/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Takyon core.
   Custom driver scripts under `~/.takyon/skills/...` are user-side and fine.
