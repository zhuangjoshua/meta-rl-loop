...
| | |
|---|---|
| Source | Optional — install with `takyon skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Takyon Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Takyon terminal calls (desktop app flow only)
...
### Service Account (recommended for Takyon)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.takyon/.env` (the skill will prompt for this on first load).
...
## Takyon Execution Pattern (desktop app flow)

Takyon terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/takyon-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/takyon-op.sock"
