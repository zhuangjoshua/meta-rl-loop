...
| | |
|---|---|
| Source | Optional — install with `takyon skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${TAKYON_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${TAKYON_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${TAKYON_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client takyon
```
...
```bash
PUBLISH="${TAKYON_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client takyon
```
...
```bash
DRIVE="${TAKYON_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `takyon`)    |
