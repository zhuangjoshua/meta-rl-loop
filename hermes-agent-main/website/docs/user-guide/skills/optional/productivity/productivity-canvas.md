...
| | |
|---|---|
| Source | Optional — install with `takyon skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Takyon Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.takyon/.env`:
...
```bash
CANVAS="python $TAKYON_HOME/skills/productivity/canvas/scripts/canvas_api.py"
