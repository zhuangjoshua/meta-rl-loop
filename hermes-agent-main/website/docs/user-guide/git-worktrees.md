...
title: "Git Worktrees"
description: "Run multiple Takyon agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Takyon Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Takyon so each session has a clean, isolated working directory.

## Why Use Worktrees with Takyon?

Takyon treats the **current working directory** as the project root:

- CLI: the directory where you run `takyon` or `takyon chat`
...
git worktree add ../repo-feature feature/takyon-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/takyon-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Takyon there:

```bash
cd ../repo-feature

# Start Takyon in the worktree
takyon
```
...
git worktree add ../repo-experiment-a feature/takyon-a
git worktree add ../repo-experiment-b feature/takyon-b
```
...
- Works on its own branch (`feature/takyon-a` vs `feature/takyon-b`).
...
- Takyon checkpoint data under `~/.takyon/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Takyon experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/takyon-checkpoints-docs`, `feature/takyon-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Takyon from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `takyon -w` (Automatic Worktree Mode)

Takyon has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `takyon/takyon-<hash>`).
...
```bash
takyon -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `takyon -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Takyon session its own clean checkout.
