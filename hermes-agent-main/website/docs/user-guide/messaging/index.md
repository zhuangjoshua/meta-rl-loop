...
description: "Chat with Takyon from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Takyon from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/docs/user-guide/features/voice-mode) and [Use Voice Mode with Takyon](/docs/guides/use-voice-mode-with-takyon).
...
```mermaid
flowchart TB
    subgraph Gateway["Takyon Gateway"]
...
```bash
takyon gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
takyon gateway              # Run in foreground
takyon gateway setup        # Configure messaging platforms interactively
takyon gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo takyon gateway install --system   # Linux only: install a boot-time system service
takyon gateway start        # Start the default service
takyon gateway stop         # Stop the default service
takyon gateway status       # Check default service status
takyon gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Takyon Agent to the latest version |
...
Configure per-platform overrides in `~/.takyon/gateway.json`:
...
takyon pairing approve telegram XKGH5N7P

# Other pairing commands:
takyon pairing list          # View pending + approved users
takyon pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Takyon appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.takyon/config.yaml`:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.takyon/config.yaml`:
...
```bash
TAKYON_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
takyon gateway install               # Install as user service
takyon gateway start                 # Start the service
takyon gateway stop                  # Stop the service
takyon gateway status                # Check status
journalctl --user -u takyon-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo takyon gateway install --system
sudo takyon gateway start --system
sudo takyon gateway status --system
journalctl -u takyon-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Takyon will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Takyon installations on the same machine (with different `TAKYON_HOME` directories), each gets its own systemd service name. The default `~/.takyon` uses `takyon-gateway`; other installations use `takyon-gateway-<hash>`. The `takyon gateway` commands automatically target the correct service for your current `TAKYON_HOME`.
:::

### macOS (launchd)

```bash
takyon gateway install               # Install as launchd agent
takyon gateway start                 # Start the service
takyon gateway stop                  # Stop the service
takyon gateway status                # Check status
tail -f ~/.takyon/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.takyon.gateway.plist`. It includes three environment variables:
...
- **TAKYON_HOME** — scopes the gateway to your Takyon installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `takyon gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `TAKYON_HOME` directory gets its own launchd label. The default `~/.takyon` uses `ai.takyon.gateway`; other installations use `ai.takyon.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `takyon-cli` | Full access |
| Telegram | `takyon-telegram` | Full tools including terminal |
| Discord | `takyon-discord` | Full tools including terminal |
| WhatsApp | `takyon-whatsapp` | Full tools including terminal |
| Slack | `takyon-slack` | Full tools including terminal |
| Google Chat | `takyon-google_chat` | Full tools including terminal |
| Signal | `takyon-signal` | Full tools including terminal |
| SMS | `takyon-sms` | Full tools including terminal |
| Email | `takyon-email` | Full tools including terminal |
| Home Assistant | `takyon-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `takyon-mattermost` | Full tools including terminal |
| Matrix | `takyon-matrix` | Full tools including terminal |
| DingTalk | `takyon-dingtalk` | Full tools including terminal |
| Feishu/Lark | `takyon-feishu` | Full tools including terminal |
| WeCom | `takyon-wecom` | Full tools including terminal |
| WeCom Callback | `takyon-wecom-callback` | Full tools including terminal |
| Weixin | `takyon-weixin` | Full tools including terminal |
| BlueBubbles | `takyon-bluebubbles` | Full tools including terminal |
| QQBot | `takyon-qqbot` | Full tools including terminal |
| Yuanbao | `takyon-yuanbao` | Full tools including terminal |
| Microsoft Teams | `takyon-teams` | Full tools including terminal |
| API Server | `takyon-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `takyon-webhook` | Full tools including terminal |
...
1. **Gateway log** (`~/.takyon/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
