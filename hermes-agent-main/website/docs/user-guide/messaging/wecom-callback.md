...
Connect Takyon to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Takyon supports two WeCom integration modes:
...
3. Takyon decrypts the message, queues it for the agent
...
(Use `takyon gateway start` only after `takyon gateway install` has registered the systemd/launchd service.)
