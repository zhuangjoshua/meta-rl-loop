...
<div style={{position: 'relative', width: '100%', aspectRatio: '16 / 9', marginBottom: '1.5rem'}}>
  <iframe
    ...
    title="Takyon Agent — Webhooks Tutorial"
...
1. Enable via `takyon gateway setup` or environment variables
2. Define routes in `config.yaml` **or** create them dynamically with `takyon webhook subscribe`
...
Add the `github-pr` route to your `~/.takyon/config.yaml` as shown in the example above.
...
Open a pull request on the repository. The webhook fires, Takyon processes the event, and posts a review comment on the PR.
...
```bash
takyon webhook subscribe antenna-matches \
  --deliver telegram \
...
In addition to static routes in `config.yaml`, you can create webhook subscriptions dynamically using the `takyon webhook` CLI command. This is especially useful when the agent itself needs to set up event-driven triggers.
...
```bash
takyon webhook remove github-issues
```

### Test a subscription

```bash
takyon webhook test github-issues
takyon webhook test github-issues --payload '{"issue": {"number": 42, "title": "Test"}}'
```

### How dynamic subscriptions work

- Subscriptions are stored in `~/.takyon/webhook_subscriptions.json`
...
The agent can create subscriptions via the terminal tool when guided by the `webhook-subscriptions` skill. Ask the agent to "set up a webhook for GitHub issues" and it will run the appropriate `takyon webhook subscribe` command.
...
- Run the gateway in foreground to see logs: `takyon gateway run`
