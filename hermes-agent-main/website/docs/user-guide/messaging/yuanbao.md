...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Takyon Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Takyon to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.takyon/.env`:
...
All standard Takyon commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.takyon/logs/gateway.log`
...
4. Consider enabling verbose logging: `TAKYON_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Takyon automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
TAKYON_LOG_LEVEL=debug takyon gateway
```
...
```bash
takyon chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
