---
sidebar_position: 6
title: "Use MCP with Takyon"
description: "A practical guide to connecting MCP servers to Takyon Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Takyon

This guide shows how to actually use MCP with Takyon Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Takyon tool
- you want Takyon to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Takyon to internal APIs, databases, or company systems without modifying Takyon core

Do not use MCP when:
- a built-in Takyon tool already solves the job well
...
- Takyon remains the agent
- MCP servers contribute tools
- Takyon discovers those tools at startup or reload time
...
If you installed Takyon with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Takyon banner/status should show MCP integration when configured
- ask Takyon what tools it has available
...
## WSL2: bridge Takyon in WSL to Windows Chrome

This is the practical setup when:

- Takyon runs inside WSL2
...
In this setup, Takyon does **not** connect to Chrome directly. Instead:

- Takyon runs in WSL
- Takyon starts a local stdio MCP server
...
```text
Takyon (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Takyon stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Takyon core browser transport
...
```bash
takyon mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
takyon mcp test chrome-devtools-win
```

Then start a fresh Takyon session or run:
...
Once loaded, Takyon can use the MCP-prefixed browser tools directly. For example:
...
If Takyon runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Takyon from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Takyon from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Takyon:
...
2. Takyon-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Takyon will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Takyon to reason over a bounded workspace.
...
Now Takyon can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Takyon core.
...
Because Takyon now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
