---
sidebar_position: 7
title: "Use SOUL.md with Takyon"
description: "How to use SOUL.md to shape Takyon Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Takyon

`SOUL.md` is the **primary identity** for your Takyon instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Takyon to feel like the same assistant every time you talk to it — or if you want to replace the Takyon persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Takyon should be
- what Takyon should avoid stylistically
- how Takyon should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Takyon is and how Takyon speaks
...
Takyon now uses only the global SOUL file for the current instance:

```text
~/.takyon/SOUL.md
```

If you run Takyon with a custom home directory, it becomes:
...
Takyon automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Takyon does not overwrite it
- if the file exists but is empty, Takyon adds nothing from it to the prompt

## How Takyon uses it

When Takyon starts a session, it reads `SOUL.md` from `TAKYON_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Takyon feels.
...
Takyon already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Takyon should sound.

# Avoid
What Takyon should not do.

# Defaults
How Takyon should behave when ambiguity appears.
```
...
Then restart Takyon or start a new session.
...
### I edited SOUL.md but Takyon still sounds the same

Check:
- you edited `~/.takyon/SOUL.md` or `$TAKYON_HOME/SOUL.md`
...
### Takyon is ignoring parts of my SOUL.md
