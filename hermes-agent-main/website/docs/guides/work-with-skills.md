...
description: "Find, install, use, and create skills — on-demand knowledge that teaches Takyon new workflows"
---

# Working with Skills

Skills are on-demand knowledge documents that teach Takyon how to handle specific tasks — from generating ASCII art to managing GitHub PRs. This guide walks you through using them day to day.
...
Every Takyon installation ships with bundled skills. See what's available:
...
You can also trigger skills through natural conversation — ask Takyon to use a specific skill, and it will load it via the `skill_view` tool.
...
What happens:
1. The skill directory is copied to `~/.takyon/skills/`
...
For how to ship skills in your own plugin, see [Build a Takyon Plugin → Bundle skills](/docs/guides/build-a-takyon-plugin#bundle-skills).
...
When a skill with config is first loaded, Takyon prompts you for the values. They're stored in `config.yaml` under `skills.config.*`.

Manage skill config from the CLI:

```bash
# Interactive config for a specific skill
takyon skills config gif-search

# View all skill config
takyon config get skills.config
```
...
```bash
mkdir -p ~/.takyon/skills/my-category/my-skill
```

### 2. Write SKILL.md

```markdown title="~/.takyon/skills/my-category/my-skill/SKILL.md"
---
name: my-skill
...
```bash
takyon chat -q "/my-skill help me with the thing"
```

The skill appears automatically — no registration needed. Drop it in `~/.takyon/skills/` and it's live.

:::info
The agent can also create and update skills itself using `skill_manage`. After solving a complex problem, Takyon may offer to save the approach as a skill for next time.
:::
...
**Let the agent create skills.** After a complex multi-step task, Takyon will often offer to save the approach as a skill. Say yes — these agent-authored skills capture the exact workflow including pitfalls that were discovered along the way.

**Use categories.** Organize skills into subdirectories (`~/.takyon/skills/devops/`, `~/.takyon/skills/research/`, etc.). This keeps the list manageable and helps the agent find relevant skills faster.

**Update skills when they go stale.** If you use a skill and hit issues not covered by it, tell Takyon to update the skill with what you learned. Skills that aren't maintained become liabilities.
