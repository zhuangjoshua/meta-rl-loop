---
sidebar_position: 8
title: "Use Voice Mode with Takyon"
description: "A practical guide to setting up and using Takyon voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Takyon
...
- you want Takyon sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Takyon.
...
## Step 1: make sure normal Takyon works first
...
Takyon supports both local and cloud speech stacks.
...
### If you use `takyon setup`

If you choose NeuTTS in the setup wizard, Takyon checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Takyon transcribes and responds
...
- asking Takyon to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Takyon loop.

## Tuning CLI behavior

### Silence threshold

If Takyon starts/stops too aggressively, tune:
...
Takyon stays a normal chat bot, but can speak replies.
...
- you want Takyon to function like a portable research or ops assistant
...
Takyon joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Takyon detects speech boundaries
- transcripts are posted in the associated text channel
- Takyon responds in text and audio
...
1. get text Takyon working
2. install `takyon-agent[voice]`
