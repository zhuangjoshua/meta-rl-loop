...
This tutorial walks you through setting up a Telegram bot powered by Takyon Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Takyon Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.takyon/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Takyon itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Takyon Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_takyon_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Takyon Agent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Takyon Gateway...
...
# View live logs
journalctl --user -u takyon-gateway -f
...
takyon gateway start
takyon gateway stop
tail -f ~/.takyon/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `takyon gateway install` to update the plist.
:::
...
```bash
takyon gateway stop && takyon gateway start
```
...
takyon pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.takyon/.env`:
...
Control how much detail the bot shows when using tools. In `~/.takyon/config.yaml`:
...
Customize how the bot communicates by editing `~/.takyon/SOUL.md`:

For a full guide, see [Use SOUL.md with Takyon](/docs/guides/use-soul-with-takyon).
...
```bash
# From the CLI
takyon cron list          # View all scheduled jobs
takyon cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u takyon-gateway` (Linux) or `~/.takyon/logs/gateway.log` (macOS) |
| Cron job output | `~/.takyon/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.takyon/cron/jobs.json` |
| Pairing data | `~/.takyon/pairing/` |
| Session history | `~/.takyon/sessions/` |
