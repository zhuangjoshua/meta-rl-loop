...
```bash
takyon teams-pipeline validate
```
...
```bash
takyon teams-pipeline token-health
takyon teams-pipeline token-health --force-refresh
```
...
```bash
takyon teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
takyon teams-pipeline maintain-subscriptions
takyon teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Takyon cron (recommended if you already run the Takyon gateway)

Takyon ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.takyon/scripts/`. First create the script:

```bash
mkdir -p ~/.takyon/scripts
cat > ~/.takyon/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec takyon teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.takyon/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
takyon cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/takyon-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Takyon Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=takyon
EnvironmentFile=/etc/takyon/env
ExecStart=/usr/local/bin/takyon teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/takyon-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Takyon Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now takyon-teams-pipeline-maintain.timer
systemctl list-timers takyon-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/takyon teams-pipeline maintain-subscriptions >> /var/log/takyon/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.takyon/.env` at the top of a wrapper script that crontab calls.
...
```bash
takyon teams-pipeline subscriptions   # should show expirationDateTime advanced
takyon teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
takyon teams-pipeline list
takyon teams-pipeline list --status failed
takyon teams-pipeline show <job-id>
```

### Replay a stored job

```bash
takyon teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
takyon teams-pipeline fetch --meeting-id <meeting-id>
takyon teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `takyon teams-pipeline maintain-subscriptions --dry-run`
- inspect `takyon teams-pipeline list --status failed`
...
- run `takyon teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `takyon teams-pipeline run`
...
- [ ] `takyon teams-pipeline validate` returns an OK snapshot
- [ ] `takyon teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Takyon cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
