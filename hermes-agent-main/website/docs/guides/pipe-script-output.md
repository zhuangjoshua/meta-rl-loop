...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `takyon send`."
---

# Pipe Script Output to Messaging Platforms

`takyon send` is a small, scriptable CLI that pushes a message to any
messaging platform Takyon is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | takyon send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `takyon
...
takyon send --to telegram "deploy finished"

# Pipe in stdout from anything
echo "RAM 92%" | takyon send --to telegram:-1001234567890

# Send a file
takyon send --to discord:#ops --file /tmp/report.md

# Attach a subject/header line
takyon send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
takyon send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
takyon send --list

# Filter by platform
takyon send --list telegram
```
...
Any platform Takyon ships adapters for works as a target:
...
`takyon send` resolves the message body in this order:

1. **Positional argument** — `takyon send --to telegram "hi"`
2. **`--file PATH`** — `takyon send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | takyon send --to telegram`

When stdin is a TTY (no pipe), Takyon does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  takyon send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `takyon send` reuses your Takyon config, the same script works on
any host where Takyon is installed — no need to export bot tokens into
...
`takyon send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  takyon send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | takyon send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.takyon/bin/takyon send \
...
```bash
./train.py --epochs 200 && \
  takyon send --to telegram "training done" || \
  takyon send --to telegram "training failed (exit $?)"
```
...
takyon send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(takyon send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `takyon send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `takyon send` calls
the platform's REST endpoint directly using credentials from
`~/.takyon/.env` and `~/.takyon/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `takyon gateway start` and retry.
...
# Machine-readable
takyon send --list --json
```

The listing is built from `~/.takyon/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`takyon gateway
...
| Approach | Multi-platform | Reuses Takyon creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `takyon send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`takyon send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `takyon send`.
...
- [Gateway Internals](/docs/developer-guide/gateway-internals) —
  the delivery router that `takyon send` shares with cron delivery.
