...
description: "How Takyon builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Takyon deliberately separates:
...
1. agent identity — `SOUL.md` from `TAKYON_HOME` when available, otherwise falls back to `DEFAULT_AGENT_IDENTITY` in `prompt_builder.py`
...
```
# Layer 1: Agent Identity (from ~/.takyon/SOUL.md)
You are Takyon, an AI assistant created by Nous Research.
...
`SOUL.md` lives at `~/.takyon/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:

```python
# From agent/prompt_builder.py (simplified)
def load_soul_md() -> Optional[str]:
    soul_path = get_takyon_home() / "SOUL.md"
...
```
You are Takyon Agent, an intelligent AI assistant created by Nous Research.
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_takyon_md(cwd_path)       # 1. .takyon.md / TAKYON.md (walks to git root)
    ...
    # SOUL.md from TAKYON_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.takyon.md`, `TAKYON.md` | CWD up to git root | Takyon-native project config |
...
- **YAML frontmatter stripped** — `.takyon.md` frontmatter is removed (reserved for future config overrides)
...
1. `.takyon.md` / `TAKYON.md` (walks to git root)
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs Takyon already loads, rather than editing Python templates in place.

### Use these surfaces first

- `~/.takyon/SOUL.md` — replace the built-in default identity block with your own agent persona and standing behavior.
- `~/.takyon/MEMORY.md` and `~/.takyon/USER.md` — provide durable cross-session facts and user profile data that should be snapshotted into new sessions.
- Project context files such as `.takyon.md`, `TAKYON.md`, `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` — inject repo-specific working rules.
- Skills — package reusable workflows and references without editing core prompt code.
- Optional system prompt config / API overrides — add deployment-specific instruction text without forking Takyon.
- Ephemeral overlays such as `TAKYON_EPHEMERAL_SYSTEM_PROMPT` or prefill messages — add turn-scoped guidance that should not become part of the cached prompt prefix.
...
- if you want to change how Takyon assembles prompts for everyone, change Python and treat it as a code contribution
