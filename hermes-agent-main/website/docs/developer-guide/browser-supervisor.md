...
[PR #12550](https://github.com/NousResearch/takyon-agent/pull/12550) proposed a
...
(`takyon-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
One `asyncio.Task` running in a background daemon thread per Takyon `task_id`.
...
- `toolsets.py` — register `browser_dialog` in `browser`, `takyon-acp`, `takyon-api-server`, core toolsets (gated on CDP reachability)
...
- `takyon_cli/config.py` — add `browser.dialog_policy` and `browser.dialog_timeout_s` to `DEFAULT_CONFIG`
