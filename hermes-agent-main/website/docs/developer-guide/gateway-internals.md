...
The messaging gateway is the long-running process that connects Takyon to 20+ external messaging platforms through a unified architecture.
...
1. `resolve_command()` from `takyon_cli/commands.py` maps input to canonical name (handles aliases, prefix matching)
...
|--------|-----------------|
| `~/.takyon/.env` | API keys, bot tokens, platform credentials |
| `~/.takyon/config.yaml` | Model settings, tool configuration, display options |
...
- **Explicit target delivery** — `send_message` tool specifying `telegram:-1001234567890`, or the [`takyon send` CLI](/docs/guides/pipe-script-output) wrapping the same tool for shell scripts
...
Hooks are discovered from `gateway/builtin_hooks/` (an extension point — currently empty in the shipped distribution; `_register_builtin_hooks()` is a no-op stub) and `~/.takyon/hooks/` (user-installed). Each hook is a directory with a `HOOK.yaml` manifest and `handler.py`.
...
- `takyon gateway start` / `takyon gateway stop` — manual control
- `systemctl` (Linux) or `launchctl` (macOS) — service management
- PID file at `~/.takyon/gateway.pid` — profile-scoped process tracking

**Profile-scoped vs global**: `start_gateway()` uses profile-scoped PID files. `takyon gateway stop` stops only the current profile's gateway. `takyon gateway stop --all` uses global `ps aux` scanning to kill all gateway processes (used during updates).
