...
The ACP adapter wraps Takyon' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
takyon acp / takyon-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.takyon/.env
  -> configure stderr logging
  -> construct TakyonACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'takyon-agent[acp]==<version>' takyon-acp`, pointed at the `takyon-agent` PyPI release.
...
- `allow_once` -> Takyon `once`
- `allow_always` -> Takyon `always`
- reject options -> Takyon `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Takyon tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["takyon-acp"])
...
Instead it reuses Takyon' runtime resolver:

- `acp_adapter/auth.py`
- `takyon_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Takyon provider/credentials. It also always advertises a terminal setup auth method (`takyon-setup`, args `--setup`) so first-run registry clients can open Takyon' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.takyon/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `takyon-acp` toolset definition
- `takyon_cli/main.py` — `takyon acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `takyon-acp` script
