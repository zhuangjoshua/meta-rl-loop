...
sidebar_position: 1
title: "Architecture"
description: "Takyon Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Takyon Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ takyon_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # TakyonCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── takyon_state.py           # SQLite session/state database with FTS5
├── takyon_constants.py       # TAKYON_HOME, profile-aware paths
...
│
├── takyon_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `takyon` subcommands (large file)
...
│   ├── skills_config.py      # takyon skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # takyon tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # takyon gateway start/stop
│
...
```text
User input → TakyonCLI.process_input()
...
- **`prompt_builder.py`** — Assembles the system prompt from: personality (SOUL.md), memory (MEMORY.md, USER.md), skills, context files (AGENTS.md, .takyon.md), tool-use guidance, and model-specific instructions
...
Three discovery sources: `~/.takyon/plugins/` (user), `.takyon/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `takyon plugins` or `config.yaml`.

→ [Plugin Guide](/docs/guides/build-a-takyon-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Takyon as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`takyon -p <name>`) gets its own TAKYON_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
