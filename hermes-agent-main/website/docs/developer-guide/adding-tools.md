...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to Takyon Agent — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in Takyon tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying Takyon core, use the plugin route instead:

- [Plugins](/docs/user-guide/features/plugins)
- [Build a Takyon Plugin](/docs/guides/build-a-takyon-plugin)
...
2. **`toolsets.py`** — add tool name to `_TAKYON_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `takyon_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `takyon_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `takyon chat -q "Use the weather tool for London"`
