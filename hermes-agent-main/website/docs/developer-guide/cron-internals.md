...
description: "How Takyon stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `takyon_cli/cron.py` | CLI `takyon cron` subcommands |
...
Jobs are stored in `~/.takyon/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `takyon cron` commands are run or during active CLI sessions.
...
```python
# ~/.takyon/scripts/check_competitors.py
...
2. **Environment variable** — `TAKYON_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.takyon/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `takyon cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `takyon cron` CLI provides direct job management:

```bash
takyon cron list                    # Show all jobs
takyon cron create                  # Interactive job creation (alias: add)
takyon cron edit <job_id>           # Edit job configuration
takyon cron pause <job_id>          # Pause a running job
takyon cron resume <job_id>         # Resume a paused job
takyon cron run <job_id>            # Trigger immediate execution
takyon cron remove <job_id>         # Delete a job
```
