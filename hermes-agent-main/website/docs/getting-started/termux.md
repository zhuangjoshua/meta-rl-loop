...
description: "Run Takyon Agent directly on an Android phone with Termux"
---

# Takyon on Android with Termux

This is the tested path for running Takyon Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Takyon from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Takyon now ships a Termux-aware installer path:
...
- links `takyon` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `takyon` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/takyon" "$PREFIX/bin/takyon"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `takyon` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.takyon/.env`.
...
### `takyon doctor` says ripgrep or Node is missing
