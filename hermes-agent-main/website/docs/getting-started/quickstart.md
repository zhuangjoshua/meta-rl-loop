...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Takyon Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Takyon setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Takyon Agent Tutorials & Use Cases](https://www.youtube.com/channel/UCqB1bhMwGsW-yefBxYwFCCg) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Takyon Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Takyon for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Takyon working on my machine | `takyon setup` | Run a real chat and verify it responds |
| I already know my provider | `takyon model` | Save the config, then start chatting |
| I want a bot or always-on setup | `takyon gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `takyon model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `takyon model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Takyon cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Takyon Agent

**Option A — pip (simplest):**

```bash
pip install takyon-agent
takyon postinstall     # optional: installs Node.js, browser, ripgrep, ffmpeg + runs setup
```
...
The single most important setup step. Use `takyon model` to walk through the choice interactively:
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `takyon model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `takyon model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `takyon model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax-M2.7 via browser OAuth — no API key needed | `takyon model` → MiniMax (OAuth) |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `takyon model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `takyon model` (requires `copilot` CLI + `copilot login`) |
...
Takyon Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `takyon model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Takyon separates secrets from normal config:

- **Secrets and tokens** → `~/.takyon/.env`
- **Non-secret settings** → `~/.takyon/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
takyon config set model anthropic/claude-opus-4.6
takyon config set terminal.backend docker
takyon config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
takyon            # classic CLI
takyon --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Takyon ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `takyon` vs `takyon --tui`.
:::
...
```bash
takyon --continue    # Resume the most recent session
takyon -c            # Short form
```
...
```bash
takyon gateway setup    # Interactive platform configuration
```
...
- `takyon tools` — tune tool access per platform
- `takyon skills` — browse and install reusable workflows
...
```bash
takyon config set terminal.backend docker    # Docker isolation
takyon config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
# From the Takyon install directory (the curl installer placed it at
# ~/.takyon/takyon-agent on Linux/macOS or %LOCALAPPDATA%\takyon\takyon-agent on Windows):
...
```bash
takyon skills search kubernetes
...
(If you installed without `[all]`, run `cd ~/.takyon/takyon-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Takyon opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `takyon model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `takyon gateway setup` and check `takyon gateway status` |
| `takyon --continue` can't find old session | Switched profiles or session never saved | Check `takyon sessions list` and confirm you're in the right profile |
...
| `takyon doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `takyon model`
3. `takyon setup`
4. `takyon sessions list`
5. `takyon --continue`
6. `takyon gateway status`
...
| `takyon tools` | Configure which tools are enabled per platform |
| `takyon setup` | Full setup wizard (configures everything at once) |
| `takyon doctor` | Diagnose issues |
| `takyon update` | Update to latest version |
| `takyon gateway` | Start the messaging gateway |
| `takyon --continue` | Resume last session |
