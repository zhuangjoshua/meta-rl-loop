...
- For `product/site/`, assume the worker will receive a prepared shared subuser app kit under `_takyon/` plus a generated `surface-context.js`; extend the business-specific UI around that substrate instead of re-explaining the app plane in the task body.
