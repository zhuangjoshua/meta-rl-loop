...
- The seeded AppKit source now includes canonical starter shells for `/`, `/app`, and `/app/profile`. Reuse those route intentions, auth/paywall/account boundaries, and helper functions by default; redesign the visual system and content aggressively, but do not reinvent the gating helpers unless the task explicitly changes the rail logic.
