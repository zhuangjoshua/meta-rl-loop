...
5. Delegate ONE bounded `business_claude_agent_task` on `product/site/` for the selected target:
   ...
   - The worker runtime installs the canonical pinned `design-taste-frontend` once as a native Claude Code skill and includes it in every Agent SDK session. Do not pass skill content, excerpts, summaries, aliases, or a `guidance_skills` field in the task payload. Business files cannot install or override it.
   - **Initial public landing:** use `effort: medium`, `max_turns: 60`, and `timeout_ms: 900000`. Ask the worker to invoke native `design-taste-frontend`; the skill owns brief inference, its Design Read and dials, foundation choice, implementation, real-image decisions, rendered inspection, and its complete pre-flight. Persist the durable direction and image plan in `product/site/DESIGN.md`. The shared publication gate requires inspected 1440x900 and 390x844 renders plus honest evidence for every canonical pre-flight item. Any failed or missing item blocks publication.
   - **Dense or multi-step product UI:** the same native skill remains installed and must be invoked, but its own scope boundary says product UI follows the appropriate product-system patterns rather than forcing marketing-page layouts onto the app. Read `product/site/DESIGN.md` and preserve the existing landing, tokens, typography, palette, spacing, motion, and generated assets. Use `effort: high`, `max_turns: 90`, `budget_usd: 25.0`, and `timeout_ms: 1800000`.
   - **Generic `product/site` task:** native Taste remains available automatically. Preserve the established direction and let the skill decide what applies; never add another design preset or paste design rules into the instruction. The injected App Kit contract still owns routes, navigation, auth, checkout, entitlements, account behavior, and actions.
   - The worker owns image art direction through the existing Safebox-gated `business_generate_site_image` capability. Generate only truthful assets the skill calls for, use every generated asset in the rendered page, and never generate filler or fabricate product UI inside an image.
...
6. The worker self-verifies build/typecheck green within its one bounded call. Read its structured result, not its prose. The first API retry, unchanged deterministic failure, timeout, or explicit blocker ends the attempt; never start or recommend another coding worker against the same failed state.
...
- Delegate the public landing with native `design-taste-frontend`, medium effort, 60 turns, and one 900000ms total deadline. Build a truthful, branded `/`; leave `/app` + `/app/profile` as the honest auth/subscription/account shell. Do not finish the deep `/app` workflow in this phase.

### Flesh out `/app`

- Confirm bootstrap already produced `/` plus the `/app` shell, then delegate with `effort: high`, `max_turns: 90`, `budget_usd: 25.0`, and `timeout_ms: 1800000` to turn `/app` into the real product. Invoke the installed native skill, honor its product-UI scope boundary, and preserve the canonical `product/site/DESIGN.md`, landing, tokens, and assets. Let the worker create real `product/site/actions/<name>.ts` files when backend behavior is needed; the injected contract defines their shape.

### Surgical iterate (carries the pricing grandfather rail)

- Start from the existing source. Pick the concrete target: one page, flow, action, pricing move, paywall/account issue, or conversion blocker. Preserve the Taste-authored DESIGN.md and established direction; do not inject design rules into the worker prompt.
