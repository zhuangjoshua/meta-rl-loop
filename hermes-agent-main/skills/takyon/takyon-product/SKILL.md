...
5. Delegate ONE bounded `business_claude_agent_task` on `product/site/`:
   ...
   - `guidance_skills`: pass `taste-frontend` first and `claude-design` second on every `product/site` build or iteration. Add at most one brief-fitting `claude-design-*` visual system. Taste is the top-level art director that adapts the selected system to the business; it does not replace the system. The injected App Kit contract still owns routes, navigation, auth, checkout, entitlements, account behavior, and actions.
