...
5. Delegate ONE bounded `business_claude_agent_task` on `product/site/` for the selected target:
   ...
   - The worker runtime best-effort materializes the shared `design-taste-frontend` native Claude Code skill and requests it from user settings in every Agent SDK session. Installation, symlink, digest, discovery, inclusion, and invocation checks are advisory. Do not pass skill content, excerpts, summaries, aliases, or a `guidance_skills` field in the task payload. Business files cannot install or override it.
   - **Initial public landing:** use `effort: medium`, `max_turns: 60`, and `timeout_ms: 900000`. Ask the worker to invoke native `design-taste-frontend`; the skill owns design and image decisions when available. Build/typecheck remains the mechanical publication check. Generated-asset counts, DESIGN.md, screenshots, audits, and aesthetic scores are not publication conditions.
   - **Dense or multi-step product UI:** the same native skill remains installed and must be invoked, but its own scope boundary says product UI follows the appropriate product-system patterns rather than forcing marketing-page layouts onto the app. Preserve the useful existing landing and design direction. Use `effort: high`, `max_turns: 90`, `budget_usd: 25.0`, and `timeout_ms: 1800000`.
   ...
   - The worker may use the Safebox-gated `business_generate_site_image` capability when the native skill decides original imagery improves the product. Images are optional; never generate filler or fabricate product UI inside an image.
...
- Confirm bootstrap already produced `/` plus the `/app` shell, then delegate with `effort: high`, `max_turns: 90`, `budget_usd: 25.0`, and `timeout_ms: 1800000` to turn `/app` into the real product. Invoke the installed native skill, honor its product-UI scope boundary, and preserve useful existing product and design work. Let the worker create real `product/site/actions/<name>.ts` files when backend behavior is needed; the injected contract defines their shape.
...
3. Do not restate the code-injected worker contract (action-file shape, `user`+`entitlements[]`, forbidden legacy gates, route skeleton, no-free-tier/no-trial); point the worker at it and let it carry the invariants. Native Taste controls presentation, never the click graph.
4. Use one bounded worker session for the selected phase. The runtime invokes the native Taste skill for product/site work; do not add a separate design pass, rendered-audit pass, or per-business skill installation.
