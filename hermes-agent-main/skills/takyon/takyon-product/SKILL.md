...
metadata:
  hermes:
    ...
    related_skills: [takyon-market-research, takyon-app-runtime, takyon-distribution, taste-frontend, claude-design, taste-imagegen-web]
...
5. Delegate ONE bounded `business_claude_agent_task` on `product/site/` for the selected target:
   ...
   - **Initial public landing only:** pass exactly `guidance_skills: ["taste-frontend"]`, `effort: medium`, `max_turns: 60`, and `timeout_ms: 900000`. The pinned full Taste skill owns one continuous Design Read, implementation, build/typecheck, source-responsive audit, and final preflight. Before editing the landing, persist the one-line Design Read, exact `DESIGN_VARIANCE`, `MOTION_INTENSITY`, and `VISUAL_DENSITY` values, selected foundation, and durable asset decisions in `product/site/DESIGN.md`. The SDK worker must not claim live visual inspection; that proof belongs to post-publish browser acceptance. Do not stack `claude-design` or any `claude-design-*` preset into this call. The 900000ms is one total deadline; any internal recovery can use only the time remaining on that same call and must not start a second Taste worker.
   - **Dense or multi-step product UI:** pass exactly `guidance_skills: ["claude-design"]`. Do not pass Taste: its upstream scope explicitly excludes dashboards, data tables, and multi-step product UI. Read `product/site/DESIGN.md`, then inspect and preserve the landing source, tokens, typography, palette, spacing, motion, and existing assets as the fixed brand source of truth instead of choosing another visual direction. A real workflow build keeps its established capacity: `effort: high`, `max_turns: 90`, `budget_usd: 25.0`, and `timeout_ms: 1800000`; those limits belong to the non-Taste product continuation and do not change the landing's exact medium/60/900 bound.
   - **Generic `product/site` task:** omitting `guidance_skills` defaults to `claude-design` only. Passing the explicit empty list `guidance_skills: []` means no design guidance and must stay empty; use that for a bounded copy/data correction that must not reopen design. Never auto-add Taste or a style preset. The injected App Kit contract still owns routes, navigation, auth, checkout, entitlements, account behavior, and actions.
...
- Confirm bootstrap already produced `/` plus the `/app` shell, then delegate with exactly `guidance_skills: ["claude-design"]`, `effort: high`, `max_turns: 90`, `budget_usd: 25.0`, and `timeout_ms: 1800000` to turn `/app` into the real product. Read and preserve the canonical `product/site/DESIGN.md` plus the established landing tokens and assets; do not rerun Taste or select a new style. Let the worker create real `product/site/actions/<name>.ts` files when backend behavior is needed; the injected contract defines their shape.

### Surgical iterate (carries the pricing grandfather rail)

- Start from the existing source. Pick the smallest real target: one page, flow, action, pricing move, paywall/account issue, or conversion blocker. Keep it surgical, not a rebuild. Use `claude-design` alone for product UI; use the explicit empty guidance list for a copy/data-only correction that must preserve the established design exactly.
...
4. Keep the public landing to one full Taste-only worker session with `effort: medium`, `max_turns: 60`, and one `timeout_ms: 900000` total deadline. Real workflow continuation uses `claude-design` alone with the established high/90/25/1800000 capacity and inherits the landing system. Never auto-inject Taste into generic product work, and preserve an explicit empty guidance list.
