...
5. Delegate ONE bounded `business_claude_agent_task` on `product/site/` for the selected target:
   ...
   - **Initial public landing only:** pass exactly `guidance_skills: ["taste-frontend"]`, `effort: medium`, `max_turns: 60`, and `timeout_ms: 900000`. The pinned full Taste skill owns one continuous Design Read, implementation, build/typecheck, rendered browser review, and final preflight. Before editing the landing, persist the one-line Design Read, exact `DESIGN_VARIANCE`, `MOTION_INTENSITY`, and `VISUAL_DENSITY` values, selected foundation, and durable asset decisions in `product/site/DESIGN.md`. After the final green build/typecheck, call the same-session `business_render_landing_preflight` tool exactly once and read its 1440x900 and 390x844 screenshots; do not start Vite, Chromium, or agent-browser yourself. The first viewport must read as one complete composition with the primary CTA visible and no accidental early next-section intrusion. A render failure is a precise blocker, not permission to repeat the preflight. The 900000ms is one total deadline and must not start a second Taste worker.
