...
5. Delegate ONE bounded `business_claude_agent_task` on `product/site/` for the selected target:
   ...
   - **Initial public landing:** use `effort: medium`, `max_turns: 60`, and `timeout_ms: 900000`. Ask the worker to invoke native `design-taste-frontend`; the skill owns design and image decisions. Native install/discovery/inclusion/invocation plus build/typecheck are the machine checks. Generated-asset counts, DESIGN.md, screenshots, audits, and aesthetic scores are not publication conditions.
