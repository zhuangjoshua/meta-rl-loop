# Creative Brief — Intake

Fill this before generating any ad spec. Strategy is decided from this brief; the image
model is never asked to invent it. Leave a field blank only if it is genuinely unknown —
do not invent facts, testimonials, or metrics. Mark anything fictional as
`[FICTIONAL PLACEHOLDER]`.

## 1. Product & Offer
- **Product / company name:**
- **Product URL:**
- **One-line description:**
- **Category (what shelf is this on):**
- **Core value proposition:**
- **Differentiation (why this, not the alternative):**
- **Offer type:** product | feature | free trial | lead magnet | discount | bundle
- **Specific offer / price / terms:**
- **Landing page promise (the headline the click must match):**

## 2. Audience & Intent
- **Primary persona (their self-image, not a demographic row):**
- **Top pain (the felt problem):**
- **Desire / end state they want:**
- **Awareness level:** unaware | problem_aware | solution_aware | product_aware | most_aware
- **Top 3 objections / doubts:**
- **Words they actually use (voice-of-customer phrases):**

## 3. Proof & Assets
- **Real proof available:** demo / stat / testimonial / case study / rating / founder story
- **Reviews / testimonials (verbatim, with source):**
- **Hard metrics (only if real and defensible):**
- **Brand assets:** logo files, brand colors (hex), fonts, product photos, UI screenshots
- **Mandatory elements (legal, logo, disclaimers):**
- **Things that must never appear:**

## 4. Platform & Format
- **Platform(s):** meta | reddit
- **Placement(s):** feed | story | reels | reddit_feed | reddit_comments
- **Aspect ratio(s):** 1:1 | 4:5 | 9:16
- **Primary goal:** clicks | leads | trials | purchases | awareness

## 5. Constraints & Context
- **Brand voice (3 adjectives):**
- **Compliance / regulated claims (health, finance, etc.):**
- **Competitor references / example ads (for contrast, not copying):**
- **Performance data (if iterating):** what won, what lost, current CTR/CVR/ROAS

## 6. Angle Shortlist
List 3–5 angles to produce from `references/angle-taxonomy.md`, with a one-line reason each:
1.
2.
3.
4.
5.
