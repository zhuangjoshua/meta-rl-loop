...
- If starter source already exists under `product/site/src/`, treat generic layout/copy as disposable, but preserve any AppKit-owned rail components or helpers that already implement declared rails correctly. Wrap, restyle, or reposition those components instead of rewriting their behavior unless you are explicitly changing that rail's logic.
