...
- For any monthly paid surface, create or update the canonical `monthly` app plan with `business_upsert_app_plan` before claiming a paid CTA is live. Do not leave pricing as frontend copy with no real `app_plan_policies` row behind it.
- For the first app-like bootstrap seed, keep the contract minimal and executable: `product/site`, `required_routes: ["/", "/app"]`, monthly paid conversion, and no speculative `/editor` or `/documents` routes, free-tier/trial copy, or customer-visible debug/blocker language.
...
- If starter source already exists under `product/site/src/`, treat it as raw plumbing only: keep the generic package/runtime wiring you still need, but replace the starter structure, labels, and styling with the real product UI.
...
7. If the surface presents a paid monthly CTA or named monthly price, call `business_upsert_app_plan` for plan key `monthly` before final refresh/publish so checkout has a real canonical plan object.
