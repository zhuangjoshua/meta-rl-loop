...
- For product AI generation, the shared broker route still exists (`POST /generate` on product hosts, or `POST /api/takyon/apps/<slug>/generate` off-host), but customer-facing products should reach it through action files rather than direct client calls. Old `legacy` lane values normalize onto the scaffold lane; do not preserve direct-client `generate` as a supported product shape.
...
- Embedding the platform provider key in the generated app, or on the `vite_react_ts` lane bypassing real action files and calling the shared generate rail directly from product code
