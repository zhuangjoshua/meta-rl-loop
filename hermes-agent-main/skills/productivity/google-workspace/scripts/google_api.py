#!/usr/bin/env python3
"""Google Workspace API CLI for Takyon Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Takyon-facing JSON contract and falls back to the Python client
...
from _takyon_home import get_takyon_home
...
def _gws_binary() -> str | None:
    override = os.getenv("TAKYON_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Takyon Agent")
