...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("TAKYON_HOME", os.path.expanduser("~/.takyon")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("TAKYON_HOME", Path.home() / ".takyon")) / "skills" / "red-teaming" / "godmode"
...
TAKYON_HOME = Path(os.getenv("TAKYON_HOME", Path.home() / ".takyon"))
CONFIG_PATH = TAKYON_HOME / "config.yaml"
PREFILL_PATH = TAKYON_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Takyon models — already uncensored, just needs clean prompt
    "takyon": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "takyon" in model_lower or "nous" in model_lower:
        return "takyon"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Takyon config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.takyon/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Takyon for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Takyon for changes to take effect.")
