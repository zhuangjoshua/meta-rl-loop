...
## 5. Takyon Configuration

### Config location

`$TAKYON_HOME/config.yaml` (defaults to `~/.takyon/config.yaml` when `TAKYON_HOME` is unset)
...
Restart the Takyon session for changes to take effect. The MCP connection is
