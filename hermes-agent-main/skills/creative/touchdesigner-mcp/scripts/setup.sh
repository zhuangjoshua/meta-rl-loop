...
TAKYON_HOME_DIR="${TAKYON_HOME:-$HOME/.takyon}"
TAKYON_CFG="${TAKYON_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Takyon config has twozero_td MCP entry ──
if [[ ! -f "$TAKYON_CFG" ]]; then
    echo -e " ${FAIL} Takyon config not found at ${TAKYON_CFG}"
    manual_steps+=("Create ${TAKYON_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$TAKYON_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Takyon config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Takyon config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${TAKYON_CFG} manually"); }
    manual_steps+=("Restart Takyon session to pick up config change")
fi
