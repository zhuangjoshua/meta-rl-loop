"""Centralized logging setup for Takyon Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.takyon/logs/`` (profile-aware via ``get_takyon_home()``).
...
from takyon_constants import get_config_path, get_takyon_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_takyon_session_injector", False):
    ...
    _session_record_factory._takyon_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``takyon logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "takyon_plugins"),
...
def setup_logging(
    *,
    ...
    """Configure the Takyon logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    takyon_home
        Override for the Takyon home directory.  Falls back to
        ``get_takyon_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_takyon_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._takyon_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the takyon group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from takyon_cli.config import is_managed
