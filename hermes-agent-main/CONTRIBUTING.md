# Contributing to Takyon Agent

Thank you for contributing to Takyon Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Takyon to work everywhere.
...
Bundled skills (in `skills/`) ship with every Takyon install. They should be **broadly useful to most users**:
...
**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.takyon/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(takyon_home, config)` for setup-wizard integration
...
- Integrate with `takyon memory setup` via `post_setup()` — no need to touch core code
...
```bash
git clone --recurse-submodules https://github.com/NousResearch/takyon-agent.git
cd takyon-agent
...
```bash
mkdir -p ~/.takyon/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.takyon/config.yaml
touch ~/.takyon/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.takyon/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/takyon" ~/.local/bin/takyon
...
├── cli.py                    # TakyonCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (takyon-cli, takyon-telegram, etc.)
├── takyon_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── takyon_cli/               # CLI command implementations
...
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.takyon/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.takyon/`)

| Path | Purpose |
|------|---------|
| `~/.takyon/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.takyon/.env` | API keys and secrets |
| `~/.takyon/auth.json` | OAuth credentials (Nous Portal) |
| `~/.takyon/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.takyon/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.takyon/state.db` | SQLite session database |
| `~/.takyon/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.takyon/cron/` | Scheduled job data |
| `~/.takyon/whatsapp/session/` | WhatsApp bridge credentials |
...
(for example `_TAKYON_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.takyon`:
...
The user may skip setup and keep loading the skill. Takyon only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `takyon setup` or update `~/.takyon/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Takyon tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Takyon Agent" is the secondary collaborator. If the contributor's commit shows "Takyon Agent" as author because they used Takyon to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Takyon tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `takyon --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Takyon uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.takyon/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `takyon_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `takyon_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Takyon runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the takyon wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `takyon_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `takyon_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `takyon_cli/gateway_windows.py::
...
Takyon has terminal access. Security matters.
...
2. **Test manually**: Run `takyon` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/NousResearch/takyon-agent/issues)
- Include: OS, Python version, Takyon version (`takyon version`), full error traceback
