# Takyon Agent Has Had "Routines" Since March
...
Takyon equivalent — works today:
```bash
takyon webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Takyon implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Takyon Agent |
|---|---|---|
...
## Things Takyon Does That Routines Can't
...
Takyon has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Takyon Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
takyon gateway setup    # enable webhooks
takyon webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation templates gallery: [takyon-agent.nousresearch.com/docs/guides/automation-templates](https://takyon-agent.nousresearch.com/docs/guides/automation-templates)

Documentation: [takyon-agent.nousresearch.com](https://takyon-agent.nousresearch.com)

GitHub: [github.com/NousResearch/takyon-agent](https://github.com/NousResearch/takyon-agent)

---

*Takyon Agent is built by [Nous Research](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*
