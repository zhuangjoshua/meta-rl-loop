"""
Takyon MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Takyon-specific extra)

Usage:
    takyon mcp serve
    takyon mcp serve --verbose
...
logger = logging.getLogger("takyon.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using TAKYON_HOME."""
    try:
        from takyon_constants import get_takyon_home
        return get_takyon_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("TAKYON_HOME", Path.home() / ".takyon")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from takyon_constants import get_takyon_home
        directory_file = get_takyon_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("TAKYON_HOME", Path.home() / ".takyon")
...
class EventBridge:
    ...
    This is the Takyon equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from takyon_constants import get_takyon_home
            db_file = get_takyon_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("TAKYON_HOME", Path.home() / ".takyon")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Takyon MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "takyon",
        instructions=(
            "Takyon Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Takyon MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
