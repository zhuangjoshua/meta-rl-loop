...
_TAKYON_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (TAKYON_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `takyon tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (TAKYON_KANBAN_TASK env "
    ...
    # ==========================================================================
    # Full Takyon toolsets (CLI + messaging platforms)
    #
    ...
    "takyon-cli": {
        ...
        "tools": _TAKYON_CORE_TOOLS,
        "includes": []
    },

    "takyon-cron": {
        # Mirrors takyon-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `takyon tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as takyon-cli; gated by `takyon tools`",
        "tools": _TAKYON_CORE_TOOLS,
        "includes": []
    },
    ...
    "takyon-email": {
        "description": "Email bot toolset - interact with Takyon via email (IMAP/SMTP)",
        "tools": _TAKYON_CORE_TOOLS,
        "includes": []
    },
    ...
    "takyon-sms": {
        "description": "SMS bot toolset - interact with Takyon via SMS (Twilio)",
        "tools": _TAKYON_CORE_TOOLS,
        "includes": []
    },
    ...
    "takyon-gateway": {
        ...
        "tools": [],
        "includes": ["takyon-telegram", "takyon-discord", "takyon-whatsapp", "takyon-slack", "takyon-signal", "takyon-bluebubbles", "takyon-homeassistant", "takyon-email", "takyon-sms", "takyon-mattermost", "takyon-matrix", "takyon-dingtalk", "takyon-feishu", "takyon-wecom", "takyon-wecom-callback", "takyon-weixin", "takyon-qqbot", "takyon-webhook", "takyon-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (takyon-<name>).
        # Gives them _TAKYON_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("takyon-"):
            platform_name = name[len("takyon-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_TAKYON_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
