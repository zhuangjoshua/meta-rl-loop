"""
Configuration management for Takyon Agent.

Config files are stored in ~/.takyon/ for easy access:
- ~/.takyon/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.takyon/.env         - API keys and secrets

This module provides:
- takyon config          - Show current configuration
- takyon config edit     - Open config in editor
- takyon config set      - Set a specific value
- takyon config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.takyon/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``takyon logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  takyon config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `takyon plugins enable
    # observability/langfuse`); credentials gate the plugin at runtime.
    "TAKYON_LANGFUSE_ENV",
    "TAKYON_LANGFUSE_RELEASE",
    "TAKYON_LANGFUSE_SAMPLE_RATE",
    "TAKYON_LANGFUSE_MAX_CHARS",
    "TAKYON_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from takyon_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("TAKYON_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_takyon_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Takyon is running in package-manager-managed mode.

    Two signals: the TAKYON_MANAGED env var (set by the systemd service),
    or a .managed marker file in TAKYON_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade takyon-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Takyon was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.takyon/.install_method`` file (written by installers)
    2. TAKYON_MANAGED env / .managed marker (NixOS, Homebrew)
    3. Container detection (/.dockerenv, /run/.containerenv, cgroup)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'
    """
    stamp = get_takyon_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.takyon/.install_method."""
    stamp = get_takyon_home() / ".install_method"
    try:
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade takyon-agent"
    if method == "docker":
        return "docker pull nousresearch/takyon-agent:latest"
    if method == "pip":
        import shutil
        uv = shutil.which("uv")
        if uv:
            return "uv pip install --upgrade takyon-agent"
        return "pip install --upgrade takyon-agent"
    return "takyon update"
...
def format_managed_message(action: str = "modify this Takyon installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("TAKYON_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Takyon installation is managed by NixOS "
            f"(TAKYON_MANAGED={env_hint}).\n"
            "Edit services.takyon-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Takyon installation is managed by Homebrew "
            f"(TAKYON_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade takyon-agent"
        )

    return (
        f"Cannot {action}: this Takyon installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Takyon."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from TAKYON_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, takyon_bin
    or None if container mode is not active, we're already inside the
    container, or TAKYON_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("TAKYON_DEV") == "1":
        return None
    ...
    container_mode_file = get_takyon_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "takyon-agent")
    exec_user = info.get("exec_user", "takyon")
    takyon_bin = info.get("takyon_bin", "/data/current-package/bin/takyon")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "takyon_bin": takyon_bin,
    }
...
# Re-export from takyon_constants — canonical definition lives there.
from takyon_constants import get_takyon_home  # noqa: F811,E402
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the takyon group can
    share state with the gateway service.

    The mode can be overridden via the TAKYON_HOME_MODE environment variable
    (e.g. TAKYON_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse TAKYON_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("TAKYON_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Takyon runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("TAKYON_CONTAINER") or os.environ.get("TAKYON_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set TAKYON_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into TAKYON_HOME if the user doesn't have one yet."""
...
def ensure_takyon_home():
    """Ensure ~/.takyon directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_takyon_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_takyon_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"TAKYON_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Takyon-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, Takyon auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Takyon sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Takyon
        ...
        # bundled Takyon image, which drops to the `takyon` user via gosu).
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Takyon sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Takyon should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``takyon chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.takyon/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: takyon sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Takyon truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``takyon kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``takyon kanban decompose`` and the kanban
        ...
        # ``takyon profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `takyon model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `takyon --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `takyon -c` muscle memory.  Default off so existing
        # users aren't surprised.  TAKYON_TUI_RESUME=<id> always wins.
        ...
        # e.g. `model · 68% · ~/projects/takyon`. Per-platform overrides go under
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.takyon/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.takyon/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Takyon feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Takyon auto-pauses the goal and
    ...
    # always goes to ~/.takyon/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${TAKYON_SKILL_DIR} and ${TAKYON_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `takyon curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot ~/.takyon/skills/ into
        # ~/.takyon/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `takyon curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for takyon-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Takyon Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (TAKYON_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.takyon/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or TAKYON_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `takyon doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``takyon_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Takyon to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via TAKYON_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `takyon -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # falls back to the default profile (the one `takyon` launches with
        ...
        # decomposition is manual via `takyon kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     takyon-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.takyon/logs/.
    ...
    # update model picker lists without shipping a takyon-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://takyon-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `takyon model` invocation; network failures
    ...
    # Session storage — controls automatic cleanup of ~/.takyon/state.db.
    ...
    "sessions": {
        ...
        # default of ``takyon sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.takyon/sessions/session_{sid}.json`` on every turn
    ...
    # ``takyon update`` behaviour.
    "updates": {
        # Run a full ``takyon backup``-style zip of TAKYON_HOME before every
        # ``takyon update``.  Backups land in ``<TAKYON_HOME>/backups/`` and
        # can be restored with ``takyon import <path>``.  Off by default —
        # on large TAKYON_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<TAKYON_HOME>/lsp/bin/`` on first use.
    ...
    # `takyon tools`. These settings tune the backing Responses API call.
    "x_search": {
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'takyon model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Takyon auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @takyon:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. TAKYON_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #takyon)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: takyon-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'takyon-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Takyon API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Takyon API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Takyon API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # TAKYON_TOOL_PROGRESS and TAKYON_TOOL_PROGRESS_MODE are deprecated —
    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).
    # Gateway falls back to these env vars for backward compatibility.
    "TAKYON_TOOL_PROGRESS": {
    ...
    "TAKYON_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.takyon.config`` entries, then checks
    ...
    try:
        ...
        # should never break `takyon update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Takyon versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``takyon_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'takyon setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Takyon won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'takyon doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("TAKYON_HOME", "~/.takyon")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("TAKYON_TOOL_PROGRESS")
            old_mode = get_env_value("TAKYON_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from TAKYON_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from TAKYON_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("TAKYON_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from TAKYON_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$TAKYON_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_takyon_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `takyon plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `takyon curator status` has no
    ...
    #   3. Creates `~/.takyon/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_takyon_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_takyon_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: takyon config set <key> <value>")
    ...
    # metadata.takyon.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _takyon_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.takyon/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.takyon/config.yaml.
    ...
    (which change ``TAKYON_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
#   openai-codex (OAuth — takyon auth) — OpenAI Codex
#   nous         (OAuth — takyon auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.takyon/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.takyon/.env.
    ...
    interactive menu render (`takyon tools`, `takyon setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `takyon tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Takyon env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.takyon/.env in place.
...
def reload_env() -> int:
    """Re-read ~/.takyon/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Takyon — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Takyon vars that are no longer in .env
...
def show_config():
    ...
    print(color("│              ⚕ Takyon Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  takyon config edit     # Edit config file", Colors.DIM))
    print(color("  takyon config set <key> <value>", Colors.DIM))
    print(color("  takyon setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: takyon config set <key> <value>")
            print()
            print("Examples:")
            print("  takyon config set model anthropic/claude-sonnet-4")
            print("  takyon config set terminal.backend docker")
            print("  takyon config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'takyon config migrate' to add them")
        ...
        print("  takyon config           Show current configuration")
        print("  takyon config edit      Open config in editor")
        print("  takyon config set <key> <value>   Set a config value")
        print("  takyon config check     Check for missing/outdated config")
        print("  takyon config migrate   Update config with new options")
        print("  takyon config path      Show config file path")
        print("  takyon config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``takyon config`` UI so users
