"""
Dump command for takyon CLI.

Outputs a compact, plain-text summary of the user's Takyon setup
...
from takyon_cli.config import get_takyon_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``takyon dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from takyon_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(takyon_home: Path) -> int:
...
def _cron_summary(takyon_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = takyon_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"takyon_home:      {display_takyon_home()}")
    ...
    toolsets = config.get("toolsets", ["takyon-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(takyon_home)}")
    lines.append(f"  skills:             {_count_skills(takyon_home)}")
