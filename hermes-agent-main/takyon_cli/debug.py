"""``takyon debug`` debug tools for Takyon Agent.

Currently supports:
    takyon debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.takyon/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[takyon debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.takyon/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``takyon debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``takyon debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, Takyon version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `takyon debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "takyon-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``takyon debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.takyon/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``takyon debug share`` also runs an opportunistic sweep as a fallback
...
def _delete_hint(url: str) -> str:
    ...
    paste_id = _extract_paste_id(url)
    if paste_id:
        return f"takyon debug delete {url}"
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "takyon-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----TakyonDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from takyon_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_takyon_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``takyon dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``takyon dump`` is run
        internally.
...
def run_debug_share(args):
    ...
    if redact:
        logger.info(
            "takyon debug share: applied force-mode redaction to log snapshots before upload"
        )
    ...
    print(f"To delete now:  takyon debug delete <url>")

    print(f"\nShare these links with the Takyon team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: takyon debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'takyon debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``takyon debug`` call.
    ...
    # best-effort — any failure is swallowed so ``takyon debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: takyon debug <command>")
        print()
        print("Commands:")
