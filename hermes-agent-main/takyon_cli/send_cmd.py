"""CLI subcommand: ``takyon send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"takyon send: cannot read {file_path}: {exc}", file=sys.stderr)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"takyon send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"takyon send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"takyon send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"takyon send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `takyon gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.takyon/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_takyon_env() -> None:
    """Populate ``os.environ`` from ``~/.takyon/.env`` AND bridge top-level
    ...
    startup that ``takyon send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.takyon/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.takyon/config.yaml`` into
       ...
       via ``takyon config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``takyon send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.takyon/.env and ~/.takyon/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "takyon send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  takyon send --to telegram \"hello\"\n"
            "  takyon send --to discord:#ops --file report.md\n"
            "  takyon send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "takyon send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `takyon send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Takyon "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.takyon/.env + ~/.takyon/config.yaml) — no LLM, "
        ...
        epilog=(
            ...
            "  takyon send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  takyon send --list                  # all platforms\n"
            "  takyon send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `takyon send --list telegram`.",
    )
