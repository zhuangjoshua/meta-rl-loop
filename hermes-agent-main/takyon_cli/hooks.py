"""takyon hooks — inspect and manage shell-script hooks.

Usage::

    takyon hooks list
    takyon hooks test <event> [--for-tool X] [--payload-file F]
    takyon hooks revoke <command>
    takyon hooks doctor

Consent records live under ``~/.takyon/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.takyon/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``takyon hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: takyon hooks {list|test|revoke|doctor}")
        print("Run 'takyon hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.takyon/config.yaml.")
        print("See `takyon hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `takyon hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `takyon hooks test` and `takyon hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Takyon wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `takyon hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `takyon hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `takyon hooks doctor`.")
