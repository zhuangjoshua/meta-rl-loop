"""`takyon checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.takyon/checkpoints/``.  Actions:

    takyon checkpoints               # same as `status`
    takyon checkpoints status        # total size, project count, breakdown
    takyon checkpoints list          # per-project checkpoint counts + workdir
    takyon checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    takyon checkpoints clear [-f]    # nuke the entire base (asks first)
    takyon checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    takyon checkpoints
    takyon checkpoints prune --retention-days 3 --max-size-mb 200
    takyon checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: takyon checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``takyon checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `takyon checkpoints` → status
