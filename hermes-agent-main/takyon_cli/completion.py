"""Shell completion script generation for takyon CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_takyon_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Takyon Agent bash completion
# Add to ~/.bashrc:
#   eval "$(takyon completion bash)"

_takyon_profiles() {{
    local profiles_dir="$HOME/.takyon/profiles"
...
_takyon_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_takyon_profiles)" -- "$cur"))
        return
    fi
...
complete -F _takyon_completion takyon
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef takyon
# Takyon Agent zsh completion
# Add to ~/.zshrc:
...
_takyon_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.takyon/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.takyon/profiles 2>/dev/null)}}")
    fi
...
_takyon() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_takyon_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'takyon command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Takyon Agent fish completion",
        "# Add to your config:",
        "#   takyon completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __takyon_profiles",
        "    echo default",
        "    if test -d $HOME/.takyon/profiles",
        "        ls $HOME/.takyon/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c takyon -f -s p -l profile"
        " -d 'Profile name' -xa '(__takyon_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__takyon_profiles)' -d 'Profile name'"
                )
