...
Used by ``takyon profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``takyon_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Takyon Agent kanban board.
...
Rules:
  ...
  - Never write "Takyon Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_takyon_home())
        else:
