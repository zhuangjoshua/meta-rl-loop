...
on Japanese locales, etc.).  Takyon's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``takyon_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Takyon is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``TAKYON_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("TAKYON_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Takyon-managed Git install directories so
    ...
    # already-running shells don't see the change — which means takyon
    ...
    # install without Takyon-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Takyon's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Takyon's
    ``takyon config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Takyon (User env var in Windows Settings, or
    export in a PowerShell profile) and Takyon picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Takyon-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\takyon\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``takyon`` from right after install) retain their old PATH.

    Any subprocess Takyon spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Takyon-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "takyon", "git", "cmd"),
        os.path.join(local_appdata, "takyon", "git", "bin"),
        os.path.join(local_appdata, "takyon", "git", "usr", "bin"),
        # Takyon venv Scripts directory — host of the takyon.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes takyon, but harmless to include.
        os.path.join(local_appdata, "takyon", "takyon-agent", "venv", "Scripts"),
