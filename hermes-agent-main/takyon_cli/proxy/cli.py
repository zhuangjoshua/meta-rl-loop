"""CLI handlers for the ``takyon proxy`` subcommand."""
...
from takyon_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "takyon proxy requires aiohttp. Install one of:\n"
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"takyon login {adapter.name}")
        print(
    ...
    print(
        f"Starting Takyon proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Takyon proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: takyon proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``takyon proxy <subcommand>``."""
    ...
    print(
        "takyon proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  takyon proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
        "      Run the proxy in the foreground.\n"
        "  takyon proxy status\n"
        "      Show which upstream adapters are ready.\n"
        "  takyon proxy providers\n"
