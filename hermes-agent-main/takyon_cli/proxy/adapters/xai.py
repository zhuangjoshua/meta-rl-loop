...
# xAI's public API is OpenAI-compatible for the endpoints Takyon commonly
# uses. The Responses endpoint is included because Takyon' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Takyon-managed OAuth credentials."""

    auth_hint = "takyon auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`takyon auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`takyon auth reset xai-oauth` or re-authenticate with "
                    "`takyon auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `takyon auth add xai-oauth --type oauth`."
            )
