"""
Gateway subcommand for takyon CLI.

Handles: takyon gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_takyon_home is imported lazily at call sites to avoid ImportError
# when takyon_constants is cached from a pre-update version during `takyon update`.
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "takyon-gateway*",
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "takyon CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``takyon gateway status`` from falsely counting the ``takyon`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``takyon gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    exclude_pids = exclude_pids | _get_ancestor_pids()
    pids: list[int] = []
    patterns = [
        "takyon_cli.main gateway",
        "takyon_cli.main --profile",
        "takyon_cli.main -p",
        "takyon_cli/main.py gateway",
        "takyon_cli/main.py --profile",
        "takyon_cli/main.py -p",
        "takyon gateway",
        "gateway/run.py",
    ]
    current_home = str(get_takyon_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"TAKYON_HOME={current_home}" in command
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. TAKYON_HOME
        ...
        # TAKYON_HOME= in argv is.
        if "--profile " in command or " -p " in command:
            return False
        if "TAKYON_HOME=" in command and f"TAKYON_HOME={current_home}" not in command:
            return False
        return True
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``takyon update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Takyon profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Takyon profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "takyon_cli.main"]
    if profile != "default":
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``takyon update`` users
    # with no running gateway until they re-invoke ``takyon gateway``
...
def _sync_takyon_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``TAKYON_HOME``.

    Under ``sudo``, ``TAKYON_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_takyon_home` falls back to ``/root/.takyon`` — the wrong
    profile. The unit file pins ``TAKYON_HOME`` for the actual gateway
    ...
    unit_home = env.get("TAKYON_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("TAKYON_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["TAKYON_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}takyon gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}takyon gateway restart{scope_flag}")
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    from takyon_constants import is_container
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `takyon gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``takyon gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from takyon_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (TAKYON_HOME-scoped).
...
from takyon_constants import is_container, is_termux, is_wsl
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``takyon gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``TAKYON_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("TAKYON_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "takyon-gateway"
SERVICE_DESCRIPTION = "Takyon Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current TAKYON_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (TAKYON_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from takyon_constants import get_default_takyon_root
    home = get_takyon_home().resolve()
    default = get_default_takyon_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary TAKYON_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(takyon_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when TAKYON_HOME is a named profile.

    For ``~/.takyon/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        takyon_home: Optional explicit TAKYON_HOME path. Defaults to the current
            ``get_takyon_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from takyon_constants import get_default_takyon_root
    home = Path(takyon_home or str(get_takyon_home())).resolve()
    default = get_default_takyon_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this TAKYON_HOME.

    Default ``~/.takyon`` returns ``takyon-gateway`` (backward compatible).
    Profile ``~/.takyon/profiles/coder`` returns ``takyon-gateway-coder``.
    Any other TAKYON_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Takyon installs that predate the
# takyon-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (takyon-gateway-*.service) and unrelated third-party
# "takyon" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("takyon.service",)
...
def _find_legacy_takyon_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Takyon gateway units.

    Detects unit files installed by older Takyon versions that used a
    different service name (e.g. ``takyon.service`` before the rename to
    ``takyon-gateway.service``). When both a legacy unit and the current
    ``takyon-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``takyon-gateway-coder.service`` and unrelated third-party
      ``takyon-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``takyon.service`` running an unrelated
...
def has_legacy_takyon_units() -> bool:
    """Return True when any legacy Takyon gateway unit files exist."""
    return bool(_find_legacy_takyon_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Takyon gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_takyon_units()
    if not legacy:
        return
    print_warning("Legacy Takyon gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current takyon-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    takyon gateway migrate-legacy")


def remove_legacy_takyon_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Takyon gateway unit files.

    Iterates over whatever ``_find_legacy_takyon_units()`` returns — which is
    ...
    legacy = _find_legacy_takyon_units()
    if not legacy:
        print("No legacy Takyon gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Takyon gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: takyon gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo takyon gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    takyon gateway uninstall")
    print_info("    sudo takyon gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning("  System service install requires sudo, so Takyon can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo takyon gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo takyon gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo takyon gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Takyon often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.takyon`` → ``ai.takyon.gateway.plist`` (backward compatible).
    Profile ``~/.takyon/profiles/coder`` → ``ai.takyon.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.takyon.gateway-{suffix}" if suffix else "ai.takyon.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.takyon/takyon-agent  -> /home/alice/.takyon/takyon-agent
      /opt/takyon                 -> /opt/takyon  (kept as-is)
...
def _takyon_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current TAKYON_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_takyon_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.takyon                    → /home/alice/.takyon
      /root/.takyon/profiles/coder     → /home/alice/.takyon/profiles/coder
      /opt/custom-takyon               → /opt/custom-takyon  (kept as-is)
    """
    current_takyon = get_takyon_home().resolve()
    current_default = (Path.home() / ".takyon").resolve()
    target_default = Path(target_home_dir) / ".takyon"

    # Default ~/.takyon → remap to target user's default
    if current_takyon == current_default:
        return str(target_default)

    # Profile or subdir of ~/.takyon → preserve the relative structure
    try:
        relative = current_takyon.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.takyon) — keep as-is
        return str(current_takyon)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    takyon_node = takyon_home / "node" / "bin"
    if _is_dir(takyon_node):
        candidates.append(str(takyon_node))
    takyon_nm = takyon_home / "node_modules" / ".bin"
    if _is_dir(takyon_nm):
        candidates.append(str(takyon_nm))
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        takyon_home = _takyon_home_for_target_user(home_dir)
        profile_arg = _profile_arg(takyon_home)
...
User={username}
Group={group_name}
ExecStart={python_path} -m takyon_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="TAKYON_HOME={takyon_home}"
Restart=always
RestartSec=5
...
"""
    ...
    takyon_home = str(get_takyon_home().resolve())
    profile_arg = _profile_arg(takyon_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only TAKYON_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp TAKYON_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../takyon_test`` path into
    # ``Environment="TAKYON_HOME=..."``. Writing that to the developer's
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Takyon install")
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo takyon gateway uninstall --system")
    print_info("         takyon gateway install")
    print_info("         takyon gateway start")
...
def systemd_install(
    ...
    # Offer to remove legacy units (takyon.service from pre-rename installs)
    # before installing the new takyon-gateway.service. If both remain, they
    ...
    if has_legacy_takyon_units():
        print()
        print_legacy_unit_warning()
        print()
        if prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_takyon_units(interactive=False)
            print()
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}takyon gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}takyon gateway status{scope_flag}             # Check status")
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}takyon gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    _sync_takyon_home_from_systemd_unit(system=system)
    try:
    ...
    try:
        ...
        print(
            ...
            "check `takyon gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `takyon gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}takyon gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}takyon gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run after the start-limit window expires: {'sudo ' if system else ''}takyon gateway restart{scope_flag}")
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}takyon gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.takyon.gateway-{suffix}" if suffix else "ai.takyon.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Takyon install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  takyon gateway status             # Check status")
    from takyon_constants import display_takyon_home as _dhh
...
def launchd_stop():
    ...
    # `takyon gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Takyon install")
    else:
        print("⚠ Service definition is stale relative to the current Takyon install")
        print("  Run: takyon gateway start")
    ...
    if deep:
        log_file = get_takyon_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("TAKYON_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Takyon gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'takyon' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/takyon/docker/entrypoint.sh before the Takyon command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$TAKYON_HOME and break later non-root dashboard/gateway runs."
    )
    print("  Set TAKYON_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk.")
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # from sibling CLI processes, but foreground `takyon gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set TAKYON_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `takyon gateway restart` which already
    ...
    # `takyon gateway start/restart` — leaving the gateway vulnerable to
    ...
    # TAKYON_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("TAKYON_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from takyon_constants import get_takyon_home as _ghh
            log_dir = _ghh() / "logs"
            log_dir.mkdir(parents=True, exist_ok=True)
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Takyon will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @takyon:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. takyon) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where Takyon delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Takyon agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Takyon will use (e.g., takyon@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Takyon connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: takyon pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Takyon will connect automatically over WebSocket",
        ],
        "vars": [
...
def _all_platforms() -> list[dict]:
    ...
    ``takyon setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run takyon under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.takyon/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_takyon_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'takyon pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: takyon pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'takyon gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from takyon_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from takyon_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Takyon will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. Takyon will store the returned account_id/token in ~/.takyon/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `takyon gateway setup`.")
        return
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_takyon_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `takyon pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `takyon pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"TakyonAgent\"")
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``takyon_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from takyon_cli import setup as _s
    return {
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.takyon/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.takyon/.env: {', '.join(required)}")
    else:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: takyon gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: takyon gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: takyon gateway install")
                    if supports_systemd_services():
                        print_info("  Or as a boot-time service: sudo takyon gateway install --system")
                    print_info("  Or run in foreground:  takyon gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: takyon gateway run")
                print_info("  For persistence:   tmux new -s takyon 'takyon gateway run'")
                print_info("  To enable systemd: add systemd=true to /etc/wsl.conf, then 'wsl --shutdown'")
            elif is_termux():
                from takyon_constants import display_takyon_home as _dhh
                print_info("  Termux does not use systemd/launchd services.")
                print_info("  Run in foreground: takyon gateway run")
                print_info(f"  Or start it manually in the background (best effort): nohup takyon gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
            else:
                print_info("  Service install not supported on this platform.")
                print_info("  Run in foreground: takyon gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'takyon gateway setup' when ready.")
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``takyon gateway install|uninstall|start|stop|restart``
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: takyon gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: takyon gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s takyon 'takyon gateway run'")
                print()
            ...
            print()
            print("  takyon gateway run                              # direct foreground")
            print("  tmux new -s takyon 'takyon gateway run'         # persistent via tmux")
            print("  nohup takyon gateway run > ~/.takyon/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: takyon gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: takyon gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: takyon gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: takyon gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: takyon gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  takyon gateway install")
                else:
                    print("To install as a service:")
                    print("  takyon gateway install")
                    print("  sudo takyon gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  takyon gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup takyon gateway run > ~/.takyon/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s takyon 'takyon gateway run'         # persistent via tmux")
                    print("  nohup takyon gateway run > ~/.takyon/logs/gateway.log 2>&1 &  # background")
                elif is_windows():
                    print("  takyon gateway install  # Install as Windows Scheduled Task (auto-start on login)")
                else:
                    print("  takyon gateway install  # Install as user service")
                    print("  sudo takyon gateway install --system  # Install as boot-time system service")
        ...
        # Stop, disable, and remove legacy Takyon gateway unit files from
        # pre-rename installs (e.g. takyon.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_takyon_units(interactive=not yes, dry_run=dry_run)
