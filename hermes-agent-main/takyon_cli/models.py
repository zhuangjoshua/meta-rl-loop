...
Add, remove, or reorder entries here — both `takyon setup` and
`takyon` provider-selection will pick up the change automatically.
"""
...
_TAKYON_USER_AGENT = f"takyon-cli/{_TAKYON_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `takyon model`
    flow without maintaining a separate static list.
    """
    from takyon_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $TAKYON_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $TAKYON_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Takyon refreshes the cache from
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Takyon that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Takyon that doesn't ship
...
def check_nous_free_tier() -> bool:
    ...
    try:
        from takyon_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
# this list:  takyon model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `takyon model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `takyon model` TUI
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``takyon model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``takyon auth add openai-codex`` without
    ``takyon model``).
    """
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    takyon-agent is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from takyon_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — takyon-agent
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from takyon_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return takyon-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; takyon's picker expects
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    ...
    """
    try:
        from takyon_cli.auth import resolve_nous_runtime_credentials
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``takyon model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from takyon_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from takyon_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Takyon' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``takyon_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Takyon should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``takyon auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.takyon/.env``.
    ...
    """
    try:
        from takyon_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        from takyon_cli.auth import read_credential_pool
        from takyon_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Takyon release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Takyon release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        try:
            from takyon_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from takyon_cli.copilot_auth import copilot_request_headers
        return copilot_request_headers(is_agent_turn=True)
    except ImportError:
        return {
            "Editor-Version": COPILOT_EDITOR_VERSION,
            "User-Agent": "TakyonAgent/1.0",
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _TAKYON_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Takyon' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _TAKYON_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    return get_takyon_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Takyon will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Takyon cannot verify the model name."
