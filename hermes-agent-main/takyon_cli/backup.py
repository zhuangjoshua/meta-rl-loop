"""
Backup and import commands for takyon CLI.

`takyon backup` creates a zip archive of the entire ~/.takyon/ directory
(excluding the takyon-agent repo and transient files).

`takyon import` restores from a backup zip, overlaying onto the current
TAKYON_HOME root.
"""
...
from takyon_constants import get_default_takyon_root, get_takyon_home, display_takyon_home
...
_EXCLUDED_DIRS = {
    "takyon-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to takyon root) should be skipped."""
...
def run_backup(args) -> None:
    """Create a zip backup of the Takyon home directory."""
    takyon_root = get_default_takyon_root()

    if not takyon_root.is_dir():
        print(f"Error: Takyon home directory not found at {takyon_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"takyon-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"takyon-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_takyon_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(takyon_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(takyon_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(takyon_root)

            if _should_exclude(rel):
                continue

            # Skip the output zip itself if it happens to be inside takyon root
            try:
    ...
    print(f"\nRestore with: takyon import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Takyon backup.
    ...
    # Look for telltale files that a takyon home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Takyon backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.takyon/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a takyon dir name
        if prefix in {".takyon", "takyon"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_takyon_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (takyon_root / "config.yaml").exists()
        has_env = (takyon_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Takyon configuration.")
        ...
        for member in members:
            ...
            target = takyon_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(takyon_root.resolve())
            except ValueError:
        ...
        profiles_dir = takyon_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # takyon_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: takyon profile list  (after installing takyon)")

        # Guidance
        print()
        if not (takyon_root / "takyon-agent").is_dir():
            print("Note: The takyon-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: takyon update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  takyon -p {pname} gateway install")

        print("Done. Your Takyon configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and takyon backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to TAKYON_HOME).
...
# — `takyon update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(takyon_home: Optional[Path] = None) -> Path:
    home = takyon_home or get_takyon_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    takyon_home: Optional[Path] = None,
) -> Optional[str]:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(takyon_home)
    if not root.exists():
        return []
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(takyon_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for takyon backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_takyon_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, takyon_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``takyon_root`` to ``out_path``.
...
def _pre_update_backup_dir(takyon_home: Optional[Path] = None) -> Path:
    home = takyon_home or get_takyon_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of TAKYON_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<TAKYON_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``takyon update``) should continue even if the backup fails.
    """
    takyon_root = takyon_home or get_default_takyon_root()
    if not takyon_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(takyon_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, takyon_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `takyon claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of TAKYON_HOME under ``backups/`` before a
    ``takyon claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``takyon import <archive>``.  Writes to
    ``<TAKYON_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `takyon import` and the
