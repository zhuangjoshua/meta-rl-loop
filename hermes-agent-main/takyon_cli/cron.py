"""
Cron subcommand for takyon CLI.
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'takyon cron create ...' or the /cron command in chat.", Colors.DIM))
        return
    ...
    if not find_gateway_pids():
        ...
        print(color("     Start it with: takyon gateway install", Colors.DIM))
        print(color("                    sudo takyon gateway install --system  # Linux servers", Colors.DIM))
        print()
...
def cron_status():
    ...
    if pids:
        ...
        print("    takyon gateway install    # Install as a user service")
        print("    sudo takyon gateway install --system  # Linux servers: boot-time system service")
        print("    takyon gateway            # Or run in foreground")
...
def cron_command(args):
    ...
    print("Usage: takyon cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
