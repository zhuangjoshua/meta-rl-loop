"""
Single source of truth for provider identity in Takyon Agent.
...
2. **Takyon overlays** — transport type, auth patterns, aggregator flags,
...
# -- Takyon overlay ----------------------------------------------------------
# Takyon-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class TakyonOverlay:
    """Takyon-specific provider metadata layered on top of models.dev."""
...
TAKYON_OVERLAYS: Dict[str, TakyonOverlay] = {
    "openrouter": TakyonOverlay(
    ...
    "openai-codex": TakyonOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "xai-oauth": TakyonOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": TakyonOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="TAKYON_QWEN_BASE_URL",
    ),
    "google-gemini-cli": TakyonOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "lmstudio": TakyonOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": TakyonOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": TakyonOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": TakyonOverlay(
    ...
    "kimi-for-coding": TakyonOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": TakyonOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": TakyonOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": TakyonOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": TakyonOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": TakyonOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": TakyonOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": TakyonOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "vercel": TakyonOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ),
    "opencode": TakyonOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": TakyonOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "huggingface": TakyonOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="HF_BASE_URL",
    ),
    "novita": TakyonOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nvidia": TakyonOverlay(
        transport="openai_chat",
        base_url_override="https://integrate.api.nvidia.com/v1",
        base_url_env_var="NVIDIA_BASE_URL",
    ),
    "xiaomi": TakyonOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "tencent-tokenhub": TakyonOverlay(
        transport="openai_chat",
    ...
    "azure-foundry": TakyonOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": TakyonOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "takyon", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Takyon overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Takyon overlay
    ...
    overlay = TAKYON_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + takyon extra
    ...
    if overlay is not None:
        # Takyon-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in TAKYON_OVERLAYS
    if provider == "bedrock":
