...
- ``takyon_cli.providers``        -- canonical provider identity + overlays
- ``takyon_cli.model_normalize``  -- per-provider name formatting
...
_HERMES_MODEL_WARNING = (
    ...
    "for use with Takyon Agent. They lack the tool-calling capabilities "
...
# unrelated local Modelfiles that carried "hermes" in their tag but were fully
# tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Hermes-3-Llama-3.1-70B, hermes-4-405b, openrouter/hermes3:70b
# Negative examples it must NOT match:
#   takyon-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``takyon config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from takyon_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'takyon model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'takyon doctor' — config issues detected:"
    ...
    # Mirrors the same logic in takyon_cli.runtime_provider.resolve_runtime_provider;
    ...
    takyon_warn = _check_hermes_model_warning(new_model)
    if takyon_warn:
        warnings.append(takyon_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from takyon_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from takyon_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by takyon provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://takyon-agent.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Takyon release. Falls back to the in-repo
    ...
    # --- 1. Check Takyon-mapped providers ---
    for takyon_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(takyon_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (TAKYON_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                ...
                if store and store.get("credential_pool", {}).get(takyon_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # show up in the picker without requiring a Takyon release.
        model_ids = curated.get(takyon_id, [])
        if takyon_id in _MODELS_DEV_PREFERRED:
            model_ids = _merge_with_models_dev(takyon_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check Takyon-only providers (nous, openai-codex, copilot, opencode-go) ---
    from takyon_cli.providers import TAKYON_OVERLAYS
    from takyon_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Takyon provider ID.
    # TAKYON_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Takyon IDs ("copilot").
    _mdev_to_takyon = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in TAKYON_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Takyon slug — e.g. "github-copilot" → "copilot"
        takyon_slug = _mdev_to_takyon.get(pid, pid)
        if takyon_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(takyon_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, takyon_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or takyon_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(takyon_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", takyon_slug, exc)
        ...
        # configured.
        if not has_creds and takyon_slug == "anthropic":
            try:
                ...
                if (takyon_creds and takyon_creds.get("accessToken")) or \
        ...
        if takyon_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = provider_model_ids(takyon_slug)
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                model_ids = _ids if _ids is not None else (curated.get(takyon_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(takyon_slug, []) or curated.get(pid, [])
        else:
            # Use curated list — look up by Takyon slug, fall back to overlay key
            model_ids = curated.get(takyon_slug, []) or curated.get(pid, [])
            # Merge with models.dev for preferred providers (same rationale as above).
            if takyon_slug in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(takyon_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": takyon_slug,
            "name": get_label(takyon_slug),
            "is_current": takyon_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "takyon",
        ...
        seen_slugs.add(takyon_slug.lower())
        _record_builtin_endpoint(takyon_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or TAKYON_OVERLAYS (keeps /model in sync
    # with `takyon model`).
    try:
        from takyon_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Takyon's canonical write key (matches
            ...
            # Takyon writes ``models:`` as a dict keyed by model id
            # (see takyon_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Takyon's own writer uses; a hyphen variant is accepted
            ...
            # active model. Takyon's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, takyon_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`takyon_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from takyon_cli.models import fetch_openrouter_models
