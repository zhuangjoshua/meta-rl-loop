...
Pure logic: walks a Takyon config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `takyon doctor` and a future `takyon migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Takyon config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Takyon config and return retirement issues.
