"""
Profile management for multiple isolated Takyon instances.

Each profile is a fully independent TAKYON_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.takyon/profiles/<name>/`` by default.

The "default" profile is ``~/.takyon`` itself — backward compatible,
zero migration needed.

Usage::

    takyon profile create coder          # fresh profile + bundled skills
    takyon profile create coder --clone  # also copy config, .env, SOUL.md, skills
    takyon profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    takyon -p coder chat                 # or via flag
    takyon profile use coder             # set as sticky default
    takyon profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See takyon_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# default profile (``~/.takyon``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   takyon-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
# Marker file written by `takyon profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `takyon
...
# `takyon skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Takyon infrastructure directories that only the default profile
         (``~/.takyon``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_takyon_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.takyon) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "takyon-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "takyon", "default", "test", "tmp", "root", "sudo",
})

# Takyon subcommands that cannot be used as profile names/aliases
_TAKYON_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the takyon root, NOT to the current TAKYON_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where TAKYON_HOME points outside
    ``~/.takyon``, profiles live under ``TAKYON_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_takyon_home() / "profiles"


def _get_default_takyon_home() -> Path:
    """Return the default (pre-profile) TAKYON_HOME path.

    In standard deployments this is ``~/.takyon``.
    In Docker/custom deployments where TAKYON_HOME is outside ``~/.takyon``
    (e.g. ``/opt/data``), returns TAKYON_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_takyon_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Takyon installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its TAKYON_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_takyon_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, takyon subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _TAKYON_SUBCOMMANDS:
        return f"'{canon}' conflicts with a takyon subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``takyon profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Takyon config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_takyon_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``takyon update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.takyon)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.takyon/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `takyon update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`takyon profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `takyon update`.\n",
                encoding="utf-8",
            )
        except OSError:
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `takyon profile describe`
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches TAKYON_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``takyon profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "TAKYON_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.takyon).\n"
            "To remove everything, use: takyon uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set TAKYON_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("TAKYON_HOME")
    try:
        os.environ["TAKYON_HOME"] = str(profile_dir)
        from takyon_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["TAKYON_HOME"] = old_home
        elif "TAKYON_HOME" in os.environ:
            del os.environ["TAKYON_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.takyon/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: takyon profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from TAKYON_HOME.

    Returns ``"default"`` if TAKYON_HOME is not set or points to ``~/.takyon``.
    Returns the profile name if TAKYON_HOME points into ``~/.takyon/profiles/<name>``.
    Returns ``"custom"`` if TAKYON_HOME is set to an unrecognized path.
    """
    from takyon_constants import get_takyon_home
    takyon_home = get_takyon_home()
    resolved = takyon_home.resolve()

    default_resolved = _get_default_takyon_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.takyon itself — its parent is ~/ and its
        # directory name is ".takyon", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: takyon profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.takyon itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.takyon). "
            "Specify a different name: takyon profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="takyon_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"takyon.{old_name}"
    new_host = f"takyon.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_takyon_home() / "honcho.json",
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a TAKYON_HOME path string.

    Called early in the CLI entry point, before any takyon modules
    are imported, to set the TAKYON_HOME environment variable.
    """
