"""Profile distributions — shareable, packaged Takyon profiles via git.

A distribution is a Takyon profile published as a git repository (or
...
* ``takyon profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``takyon skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``takyon profile``, not a parallel tree):

    takyon profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "takyon-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            takyon_requires=str(data.get("takyon_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.takyon_requires:
            out["takyon_requires"] = self.takyon_requires
        if self.author:
...
def check_takyon_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Takyon {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``takyon profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Takyon profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Takyon distribution."
        )

    # Version check up-front so we fail fast
    check_takyon_requires(manifest.takyon_requires, takyon_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.takyon).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="takyon_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `takyon profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`takyon profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="takyon_dist_update_") as tmp:
        plan = plan_install(
