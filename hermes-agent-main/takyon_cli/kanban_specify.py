...
Used by ``takyon kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``takyon_cli/goals.py`` — same aux
...
TAKYON_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    int(os.getenv("TAKYON_KANBAN_SPECIFY_MAX_TOKENS", "6000")),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Takyon Agent board.
...
def _profile_author() -> str:
    """Mirror of ``takyon_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("TAKYON_PROFILE")
    ...
    *,
    ...
    try:
        resp = client.chat.completions.create(
            ...
            temperature=0.3,
            max_tokens=TAKYON_KANBAN_SPECIFY_MAX_TOKENS,
