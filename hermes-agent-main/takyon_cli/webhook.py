"""takyon webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.takyon/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _takyon_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: takyon gateway run
"""
...
def webhook_command(args):
    """Entry point for 'takyon webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: takyon webhook {subscribe|list|remove|test}")
        print("Run 'takyon webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (takyon gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: takyon webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from takyon webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (takyon gateway run)")
