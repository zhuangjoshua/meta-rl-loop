"""``takyon logs`` — view and filter Takyon log files.
...
Usage examples::
    ...
    takyon logs                    # last 50 lines of agent.log
    takyon logs -f                 # follow agent.log in real time
    takyon logs errors             # last 50 lines of errors.log
    takyon logs gateway -n 100    # last 100 lines of gateway.log
    takyon logs --level WARNING    # only WARNING+ lines
    takyon logs --session abc123   # filter by session ID substring
    takyon logs --component tools  # only tool-related lines
    takyon logs --since 1h         # lines from the last hour
    takyon logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_takyon_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Takyon runs — try 'takyon chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_takyon_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_takyon_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_takyon_home()}/logs/")
        return

    print(f"Log files in {display_takyon_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'takyon chat' to generate logs)")
