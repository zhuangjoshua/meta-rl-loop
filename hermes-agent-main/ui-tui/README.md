...
React + Ink terminal UI for Takyon. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `TAKYON_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.takyon/.takyon_history` or under `TAKYON_HOME`.
...
```text
ui-tui/
  packages/takyon-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      takyon-ink.d.ts    type declarations for @takyon/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent TakyonCLI subprocess for slash commands
```
