...
await build({
  ...
  // Skip the prebuilt @takyon/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@takyon/ink': resolve(root, 'packages/takyon-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The takyon_cli launcher
