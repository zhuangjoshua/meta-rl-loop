import { forceRedraw, useInput } from '@takyon/ink'
...
import { handlePagerKey, PAGER_UNHANDLED } from './pagerKeymap.js'
...
export function useInputHandlers(ctx: InputHandlerContext): InputHandlerResult {
  ...
  useInput((ch, key) => {
    ...
    if (isBlocked) {
      ...
      if (overlay.pager) {
        const result = handlePagerKey(ch, key, pagerPageSize)

        if (result !== PAGER_UNHANDLED) {
          patchOverlayState(result)
        }
