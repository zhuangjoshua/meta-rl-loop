 ...
 * takyon-ink sets ``key.meta`` for plain Alt/Option on every platform
 ...
 * (``takyon_cli/voice.py::normalize_voice_record_key_for_prompt_toolkit``)
 ...
 * modifier — but takyon-ink reports Alt as ``key.meta`` on many
...
export const parseVoiceRecordKey = (raw: unknown): ParsedVoiceRecordKey => {
  ...
  // On macOS takyon-ink reports Alt as ``key.meta``, which ``isActionMod``
...
export const isVoiceToggleKey = (
  ...
  switch (configured.mod) {
    case 'alt':
      ...
      // alt binding.
      //
      // Bare Escape on takyon-ink can arrive as ``key.meta=true`` on some
    ...
    case 'ctrl':
      ...
      // ``key.meta`` is takyon-ink's Alt signal and accepting it would
