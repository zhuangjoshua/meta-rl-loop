...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.TAKYON_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Takyon on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via TAKYON_TUI_TRUECOLOR=1.
