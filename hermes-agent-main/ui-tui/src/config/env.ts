...
export const STARTUP_RESUME_ID = (process.env.TAKYON_TUI_RESUME ?? '').trim()
export const STARTUP_QUERY = (process.env.TAKYON_TUI_QUERY ?? '').trim()
export const STARTUP_IMAGE = (process.env.TAKYON_TUI_IMAGE ?? '').trim()

const mouseTrackingOverride = parseToggle(process.env.TAKYON_TUI_MOUSE_TRACKING)
const mouseTrackingDisabledLegacy = truthy(process.env.TAKYON_TUI_DISABLE_MOUSE)
...
export const NO_CONFIRM_DESTRUCTIVE = truthy(process.env.TAKYON_TUI_NO_CONFIRM)

const inlineOverride = parseToggle(process.env.TAKYON_TUI_INLINE)
...
// less fragile. Override explicitly with TAKYON_TUI_INLINE=0/1.
...
// synthetic timer).
export const SHOW_FPS = truthy(process.env.TAKYON_TUI_FPS)
