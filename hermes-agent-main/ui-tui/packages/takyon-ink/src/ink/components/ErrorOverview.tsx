...
let codeExcerpt: typeof import('code-excerpt').default | undefined
let StackUtilsCtor: typeof import('stack-utils').default | undefined

function getStackUtils(): StackUtils {
  return (stackUtils ??= new StackUtilsCtor!({
    cwd: process.cwd(),
    internals: StackUtilsCtor!.nodeInternals()
  }))
}

// Heavy parsing libraries (code-excerpt + stack-utils) are only needed when an
// uncaught error is actually rendered. Load them lazily so they stay out of the
// startup module graph.
let heavyDepsPromise: Promise<void> | undefined

function loadHeavyDeps(): Promise<void> {
  return (heavyDepsPromise ??= Promise.all([
    import('code-excerpt'),
    import('stack-utils')
  ]).then(([codeExcerptMod, stackUtilsMod]) => {
    codeExcerpt = codeExcerptMod.default
    StackUtilsCtor = stackUtilsMod.default
  }))
}
...
function ErrorOverviewInner({ error }: Props) {
...
const LazyErrorOverview = React.lazy(async () => {
  await loadHeavyDeps()
  return { default: ErrorOverviewInner }
})

export default function ErrorOverview({ error }: Props) {
  return (
    <Suspense fallback={<Text> {error.message}</Text>}>
      <LazyErrorOverview error={error} />
    </Suspense>
  )
}
