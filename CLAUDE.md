...
6. If the workflow fails, inspect `gh run view <run-id> --log-failed`, patch the failing tracked rail, push again, and watch the new run.
7. Report the run id, the VPS activation evidence (rsync + migrate-if-needed + restart + is-active), and direct smoke checks. Push main AND activate the runtime — one without the other is not a deploy.
