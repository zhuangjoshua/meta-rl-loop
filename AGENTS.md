...
When implementing custom backend or own-runtime product capability later, update the authoritative Takyon capability gate where the CEO can discover it: the relevant `TAKYON_SKILL_REGISTRY` metadata, the guarded product verification/deploy tool behavior, and the owning product/app-runtime skills. Do not leave a stale "unavailable" registry gate while adding a backend path elsewhere, and do not add a hidden backend path that the CEO cannot see through `business_registry`, tool schemas/results, or business receipts.
...
When adding or changing a Takyon tool or skill, update its readable display metadata in `plugins/takyon/registry.py` at the same time: `display_name`, useful activity detail hints, and implementation status. Display labels must describe implemented behavior unless the registry clearly marks the capability as gated, unavailable, or aspirational. Do not add UI-only aliases for tools or skills; dashboard, shell, and activity surfaces should read display names from registry/tool metadata and fall back to humanized ids with the raw id available in debug/detail.
