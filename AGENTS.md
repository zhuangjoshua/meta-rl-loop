...
Named operator profiles are shared across the rails: `scripts/operator-users.sh` maps a short name (e.g. `sai`, `josh`) to a Takyon operator user-id and is sourced by BOTH `scripts/takyon-operator-prod.sh` and `scripts/takyon-operator-dev.sh`, so `... sai` / `... josh` mean the same person in dev and prod (add a teammate with one line there or in `scripts/operator-users.conf`; raw `--user-id`/UUIDs still work). `scripts/takyon-operator-dev.sh` is the DEV mirror of the prod rail: same named commands, but it runs against the DEV TWIN (`TAKYON_ENV=dev` → the `TAKYON_DEV_*` DSN twins on four-manifold-dev's own Supabase, never prod), brings up the dev Safebox from the dev store, and scopes the session to the user's `owner_user_id` (isolation proven identical to prod). `scripts/takyon-operator-dev.sh seed <name>...` creates those users in the twin once. User-facing dev-vs-prod usage is documented in `devprod.md`; the modular feature-addition seams in `modular.md`. Keep these rails source-of-truth driven — do not duplicate the name→uuid map.
