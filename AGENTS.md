...
1. Git push uses the outer workspace repo at `/Users/Zygote/Downloads/takyon`, not the nested `hermes-agent-main` git metadata. Stage only the intended hunks, commit in the outer repo, and push `origin main` unless the operator asked for a branch.
   **Branch model (operator, 2026-07-03): `main` = live-in-prod; `dev` = the pre-prod integration branch, always a superset of `main`.** Normal dev-first work lands on `dev`, is proven on the dev env, then reaches prod ONLY via `takyon env promote dev` (a deliberate fast-forward of `main` to a dev-green SHA). The reverse direction is automatic and one-way: every commit that reaches `main` (a promote OR a direct hotfix) is force-carried into `dev` by `.github/workflows/forward-main-to-dev.yml` (merge; fails closed on a real content conflict). Never test on dev against stale prod code, and never hand-merge `dev`→`main`; use promote. The forwarding workflow only triggers on `main` pushes and only pushes `dev`, so it cannot loop and never deploys to prod; it activates the first time `dev` is promoted onto `main`. Until then keep `dev ⊇ main` by hand after any prod push.
...
- **Land dev-first changes on the `dev` branch** (prod→dev auto-forward keeps `dev ⊇ main`), then deploy with **`TAKYON_HOME=/Users/Zygote/Downloads/takyon/.takyon <clean-target-checkout>/takyon env deploy dev --rev <sha>`** (`EnvironmentProvisioner.deploy`). The clean checkout and `<sha>` must be the same pushed revision. It is **payload-revision-aware**: it `git archive`s the pinned rev (`environments/dev.yaml: code_revision: dev` → resolves the PUSHED `origin/dev`, never the payload from a stale local branch or dirty working tree) and fans that exact tree out to every dev host read from the env config's `dev_split` block — **safebox + operator** (rsync + restart in place) and the **subuser replicas** (rsync then the zero-loss drain rail). It is **CODE-ONLY** (excludes `.env`/`secrets`): it must NOT re-run `bootstrap-dev-droplet.sh`, which would overwrite the dev safebox's Doppler managed-secret config. Before claiming success, verify each host's `.takyon-deploy-artifact.json.source_revision` and runtime release SHA both equal `<sha>`.
