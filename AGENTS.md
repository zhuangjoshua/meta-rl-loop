...
Canonical state and deployer code are separate concerns. `/Users/Zygote/Downloads/takyon/.takyon` is the canonical operator state, but never run `takyon env deploy` from a stale or dirty checkout merely to access that state. Run the deploy CLI from a clean checkout at the exact target revision and pass `TAKYON_HOME=/Users/Zygote/Downloads/takyon/.takyon` explicitly. `--rev` pins the deployed payload; it does not pin or update the deployer implementation that is already running.
...
Use focused tests for touched surfaces first; never run broad suites without explicit operator approval. After focused tests pass, proceed to the requested deploy and production CLI proof, fixing only failures encountered on that path. Do not go down speculative rabbit holes: follow the shortest authoritative path to the requested outcome, stop unrelated audits, and report an exact blocker instead of expanding scope.
...
- Sub-user VPS replica: `206.81.10.173` (`takyon-subuser-2`, prod VPC `10.116.0.4`) — a SECOND subuser box behind a DO load balancer (UC2 prod scale-out, 2026-07-03). Cloned from the verified box (subuser-only secrets, ZERO operator authority — verified in `/proc`). **The subuser plane is now N-replica: any subuser deploy (runtime rsync, `takyon-subuser.service` restart, `deploy/takyon-subuser/Caddyfile` apply) MUST hit BOTH `134.209.123.8` AND `206.81.10.173`, or the LB serves drifted code.** DO LB `takyon-subuser-lb` = `24.144.65.246` (nyc1, health-check `GET /healthz` on :80 — the Caddyfile serves it via `@health → :9119`), members = both boxes. **Additive only: the LB is NOT yet in the live serving path — Cloudflare still points `*.coscale.app` product `/api` at the single box `134.209.123.8`; the cutover (repoint Cloudflare → the LB IP) is a deliberate operator-signed step, instantly reversible.** Restart the replica split drain-aware (never a bare hard restart on a serving member once cut over — that black-holes ~4.5s of LB traffic).
...
The narrow approved exception for operator-owned local production compute is `scripts/takyon-operator-prod.sh`. It is an operator-only Mac rail that fetches the live operator env over SSH, reaches the private Safebox only through an explicit localhost SSH tunnel, sets `TAKYON_ALLOW_POSTGRES_OUTSIDE_VPS=1`, and stores its local home under `~/.takyon-fourmanifold-operator-prod/`. Do not recreate this as loose shell exports, do not run a local dashboard on it, do not point product/sub-user hosts at it, and do not copy its operator authority to the sub-user plane. The intended production-compute posture is Mac-primary/VPS-fallback: keep `takyon-worker.service` active on the operator VPS with a queue-age fallback delay, and run `scripts/takyon-operator-prod.sh worker <concurrency>` locally when the Mac should claim fresh bootstrap/build/wake jobs first. If the Mac worker is off, the VPS worker drains queued work after the configured delay.

Named operator profiles are shared across the rails: `scripts/operator-users.sh` maps a short name (e.g. `sai`, `josh`) to a Takyon operator user-id and is sourced by BOTH `scripts/takyon-operator-prod.sh` and `scripts/takyon-operator-dev.sh`, so `... sai` / `... josh` mean the same person in dev and prod (add a teammate with one line there or in `scripts/operator-users.conf`; raw `--user-id`/UUIDs still work). `scripts/takyon-operator-dev.sh` is the DEV mirror of the prod rail: same named commands, but it runs against the DEV TWIN (`TAKYON_ENV=dev` → the `TAKYON_DEV_*` DSN twins on four-manifold-dev's own Supabase, never prod), brings up the dev Safebox from the dev store (with the operator token + `TAKYON_OPERATOR_USAGE_GATE_DISABLED=1` in the Safebox process, mirroring prod's worker AND Safebox hosts), and scopes the session to the user's `owner_user_id` (isolation proven identical to prod). Because a dev operator binds to an explicit user id (`josh`/`sai`) instead of the Auth0 first-login path, `load_dev_operator_env` also provisions that user's money rail up front (`ensure_dev_operator_budget_ready`: idempotent billing + custody account open + starter allowance) so the first CLI bootstrap `billing.reserve` cannot die on missing/exhausted allowance — the 3-layer money rail (Safebox auth token → billing account → usage-gate posture) all mirror prod. It also SELF-HEALS schema drift: `ensure_dev_schema_current` runs the tracked `takyon migrate` rail against the dev twin whenever the repo's migration file count changes (marker-gated at `$OPERATOR_HOME/.dev-schema-migrated`, so a current twin is a zero-cost skip) — because the twin is bootstrapped once and would otherwise fall behind new migrations (the `businesses.money_shape`/0062 drift that silently broke dev `/create`'s product-surface tools). Unlike prod (where `takyon migrate` is a deliberate deploy step), the dev twin keeps its schema current automatically. `scripts/takyon-operator-dev.sh seed <name>...` creates those users in the twin once. User-facing dev-vs-prod usage is documented in `devprod.md`; the modular feature-addition seams in `modular.md`. Keep these rails source-of-truth driven — do not duplicate the name→uuid map.

Dev connection posture (mirror prod exactly): the dev twin's RUNTIME planes (operator/runtime/safebox `TAKYON_DEV_*_DATABASE_URL`) must use the Supabase **transaction pooler `:6543`** — same as prod (verified live: prod operator + safebox both `port=6543`), which multiplexes and is RLS-transaction-safe (role GUCs are transaction-scoped; verified: `assert_takyon_pg_role`+owner-scoped read pass on 6543 and the GUC resets after the txn). Only the MIGRATION/ADMIN DSNs stay on the SESSION pooler `:5432` (DDL/multi-statement needs session mode). A dev store on `:5432` for the runtime planes hits the shared session-pooler's ~15-client cap → `EMAXCONNSESSION` → worker lease-heartbeat starvation → job churn. `TAKYON_PG_POOL_SIZE` (default 8) is set small for the dev twin as a secondary guard.

**KNOWN ARCHITECTURAL GAP (dev is NOT yet prod-equal — do not claim it is):** prod runs on THREE dedicated systemd-managed VPS hosts (operator `takyon-dashboard`+`takyon-worker`, subuser, safebox). Dev's control plane (four-manifold-dev Supabase) is prod-shaped, and its subuser plane is real (the 4b LB split: `takyon-dev-subuser-1/2` + LB), BUT dev's OPERATOR + SAFEBOX compute currently runs on the developer's **Mac** as ad-hoc foreground processes started by `takyon-operator-dev.sh` (no systemd → orphaned worker/shell processes accumulate, exhaust the pooler, and orphan job claims; not persistent; not concurrent-operator-safe). The idle `takyon-dev-safebox` droplet exists but the CLI uses a Mac `uvicorn` safebox instead. **Intended end-state (parsimony = same topology as prod):** `takyon env create dev` provisions a dev OPERATOR droplet running `takyon-dashboard.service`+`takyon-worker.service`, runs the safebox as `takyon-safebox.service` on the existing dev-safebox droplet (reusing the tracked `deploy/argon-alpha-14/` + `deploy/takyon-safebox/` rails), leaving `takyon-operator-dev.sh` as a THIN CLIENT (SSH-tunnel + optional Mac worker) exactly like `takyon-operator-prod.sh`. Until that lands, treat Mac-hosted dev as a fragile stopgap, not a real environment.
...
1. Git push uses the outer workspace repo at `/Users/Zygote/Downloads/takyon`, not the nested `hermes-agent-main` git metadata. Stage only the intended hunks, commit in the outer repo, and push `origin main` unless the operator asked for a branch.
   **Temporary branch model (operator, 2026-07-13): `main` is the only release branch and the exact source of production. The `dev` publish/promotion rail is deprecated until the operator explicitly re-enables it.** Do not deploy a release through the dev environment and do not run `takyon env promote dev`. Work from a clean outer-repo checkout, fetch immediately before release, fast-forward `origin/main` with the tested local commit, then deploy that same commit through the tracked Mac production scripts. Because GitHub forwarding is disabled, keep `origin/dev` equal to `origin/main` as a dormant mirror after each main push; equality does not make dev a release gate or authorize a dev deployment.
2. Operator deploy updates the active operator runtime on `137.184.75.57`. The operator runtime is `/opt/takyon/hermes-agent-main` and may not be a git checkout, so use the tracked operator rails under `deploy/argon-alpha-14/`: bootstrap the host with `deploy/argon-alpha-14/bootstrap-host.sh` when needed, then deploy with `deploy/argon-alpha-14/deploy-runtime.sh`. The primary agent runs from the deploy-created, read-only Claude Agent SDK release; Docker remains a separate isolated product/build-process rail, so deploy must fail fast if either the sealed SDK release or tracked execution image is unavailable. Verify with `systemctl is-active takyon-dashboard.service`, `systemctl is-active takyon-worker.service`, `docker version`, the exact SDK/plugin release, and source checks on the operator VPS.
...
2b. **Schema migrations ride the deploy, not hand SQL:** if the deploy includes new files under `hermes-agent-main/plugins/takyon/db/migrations/`, run `takyon migrate` once on the operator host BEFORE restarting services — the tracked rail resolves the migration DSN, asserts topology, and replays every file idempotently as `takyon_migration` (re-runs are no-ops). This stays an explicit deploy step by design, never a service-start side effect: runtime roles cannot DDL and the migration DSN exists only on the operator rail — that privilege boundary is the schema-change security model. Migrations are additive/nullable by convention, so migrate-before-restart is always safe.
...
4. Sub-user deploy updates the public app/runtime host(s). The sub-user plane is N-replica behind a DO LB (see the host list above): a subuser deploy MUST update BOTH `134.209.123.8` and `206.81.10.173` (runtime rsync + `takyon-subuser.service` restart + Caddyfile apply on each). Do not claim product-app routing, `tkg_` isolation, or app-plane code changed in production unless BOTH boxes were updated — otherwise the LB serves drifted code once the Cloudflare cutover happens.
...
Current operator AI posture: the primary Agent SDK already uses a Safebox `operator.session` capability with `ANTHROPIC_BASE_URL` pointed at Safebox, and `TAKYON_PROVIDER_BROKER=1` is pinned in tracked units. Remaining cutover steps to reach "no raw key on any runtime plane": delete `/v1/env/{key,first,snapshot}` provider-key vending once no consumer calls it; drop the `bypass=True` ledger conn (`runtime_app.py:231`) and run ledger writes as `takyon_app`; LAST, `git rm secrets/.env`, scrub history, and rotate every key. The `/v1/env` egress + tracked `secrets/.env` (with the master `TAKYON_SAFEBOX_TOKEN`) are the highest-severity remaining exfil surfaces — they are independent of the money gate and must be closed before the cutover is "done."
...
## Dev environment publish rail (DEPRECATED — do not use for releases)

The dev branch and droplets are retained for future repair, but they are not currently a trustworthy release gate. Their source markers, credentials, topology, and running processes may disagree. Do not publish to dev, use dev proof for a production claim, or route a release through dev.

- The only active code release path is: clean local commit → `origin/main` → tracked Mac production deploy scripts → on-host SHA/service/health verification → production E2E.
- Keep `origin/dev` as an exact dormant mirror of `origin/main`; do not deploy it.
- Re-enabling dev requires an explicit operator decision after one command can deploy and restart every dev plane at one exact source revision, status reads remote refs rather than stale local branches, required credentials resolve, and a fresh-business E2E passes.
...
- The tracked operator services run with `TERMINAL_ENV=docker`, `TERMINAL_DOCKER_MOUNT_CWD_TO_WORKSPACE=true`, and non-persistent containers so file/process work stays in the current business scratch workspace. Docker is the product/build execution sandbox, not a second model agent; if the host cannot run the tracked execution image, operator deploy must fail loudly.

## Model Tier & Local Batch Capacity

Operational rules learned running the pipeline on a higher-priced model tier (`claude-fable-5`, ~5x Sonnet) and building many businesses in parallel on the Mac-local rail. These are runtime/operator concerns, not code changes — the code fixes are already landed.

- **Production model selection is strict: the Claude Agent SDK primary CEO = `deepseek-v4-pro` through the Safebox Anthropic-compatible broker.** There is no second coding model-agent role. The tracked dashboard/worker units and Mac production rail must pin that model for every interactive, bootstrap, wake, and continuation turn; no provider fallback chain, caller override, missing-config default, or Safebox model rewrite is allowed. Provider/model failure must surface as the exact blocker instead of substituting another model.
- **Every SDK invocation has two fail-closed Safebox ceilings:** `TAKYON_PRIMARY_AGENT_MAX_BUDGET_USD` is the cumulative turn/job envelope and `TAKYON_PRIMARY_AGENT_PER_CALL_MAX_BUDGET_USD` is the per-provider-call cap, additionally bounded by `TAKYON_OPERATOR_SESSION_MAX_COST_MICROUSD`. Bootstrap supplies its larger code-owned job total but keeps the same per-call ceiling; continuations/retries reuse one cumulative envelope. `agent/usage_pricing.py` must price the exact pinned model or every paid call refuses.
- **Mac-local worker concurrency must not exceed the Docker VM's CPU capacity.** Each build sandbox is capped ~0.85 CPU (`core._build_sandbox_resource_args`), so on a 4-CPU Docker Desktop VM only ~4 builds run cleanly. Oversubscribing (e.g. `worker 10` on a 4-CPU VM) makes builds take 15-42 min instead of ~4, which blows the CEO-turn inactivity/heartbeat window and terminalizes bootstraps as `handler_error`. Match `worker <N>` to the Docker VM's CPUs, or raise the VM (Docker Desktop → Settings → Resources). `TAKYON_BUILD_DOCKER_CPUS` / `TAKYON_BUILD_DOCKER_MEMORY` tune the per-build caps.
- **Restarting the Mac worker orphans in-flight jobs to the dead pool's reservation for the full lease (~1h).** The pool id is pid-derived (`mac-operator-<host>-<pid>`), so a fresh worker cannot claim jobs reserved to the previous pid's pool until the reservation lapses. To reclaim immediately, restart the worker with `TAKYON_WORKER_POOL_ID=<the dead pool id>` (the native ClaimScope env) so it adopts that identity — never hand-edit the `jobs` control-plane table (forbidden; direct control-plane SQL is a bug). Also note `pkill -f "takyon-cli worker"` misses a worker started via `python -m takyon_cli.main worker` (different argv) — verify by pid that only one Mac worker survives, or two will re-oversubscribe Docker.
...
Practice parsimony and no slop. Parsimony means the system works through the smallest native SDK skill, scoped tool, and durable runtime surface that genuinely handles the job for every business and every relevant mode. Do not create a second path for test mode, a one-off shell workaround, a special-case business bootstrap, or a fake/demo product behavior when the same skill, business-scoped tool, receipt, or runtime rail can express the work with the right gates. Mode differences should be explicit guardrails on the same path: in test mode suppress external side effects with truthful receipts; in live mode require the real provider, budget, permission, and receipt. Prefer one clear operator affordance with flags over multiple overlapping commands, aliases, or workflow shortcuts.
...
Do not add deterministic business-action routers, fixed if/then workflow funnels, or forced artifact shortcuts to make the CEO pick a business move. Let the CEO select from approved native skill descriptions and scoped tool schemas; use exact gate errors and regression tests to catch bad substitutions. Deterministic code owns invocation modes, bootstrap/wake checkpoints, scope isolation, path containment, idempotency, credential checks, budget caps, audit receipts, kill/pause controls, and UI rendering mechanics.

Use canonical sources of truth wherever they exist. Runtime config belongs in `$TAKYON_HOME/config.yaml`; stable agent policy belongs in `plugins/takyon/prompts/ceo.md`; complete native skill routing, methods, tool usage, references, scripts, templates, and assets belong in `skills/*/<skill>/`; the approved production inventory and bundle paths belong in `skills/release-skills.yaml`; mode-level tool and write-path restrictions belong in `skills/sdk-runtime-policy.yaml`; tenant scope, authority, publication, receipts, validators, and completion gates belong in runtime code; `skills/HANDOFF/POLICY.md` is only a future-skill authoring guide. Shell/harness command metadata belongs in `plugins/takyon/harness/settings.json` and `plugins/takyon/harness/commands/*.md`; business facts belong in the Postgres control plane plus the canonical per-business object-store workspace. On the operator host, the local materialization is cache/scratch only.
...
Hardcode only durable rails and safety primitives: business isolation, invocation modes and checkpoints, path containment, idempotency, credential gates, budget caps, audit events, pause/resume/kill controls, conservative cleanup, and the shared product app runtime APIs. Strategy, prioritization, product direction, outreach motion, design, and learning belong in per-business state plus active approved skills.

When changing CEO/runtime behavior, edit the stable policy at `hermes-agent-main/plugins/takyon/prompts/ceo.md`, approved native skills and release/runtime policy under `hermes-agent-main/skills/`, and the relevant tool/cron/harness source. In particular:

- `plugins/takyon/prompts/ceo.md` is stable base policy; bootstrap, wake, and interactive differences are code-owned invocation policies and fresh per-turn context.
- `scripts/takyon-claude-primary-runtime.mjs` is the only model-agent runtime; `takyon-worker.service` is the durable queue consumer, not another model agent.
- All approved skills are discoverable by default from one versioned read-only plugin; bodies load on demand through the native `Skill` tool, and the SDK `Agent`/subagent tool is unavailable.
- Do not reintroduce Hermes `delegate_task`, `business_claude_agent_task`, ambient skill settings, or mutable `$TAKYON_HOME/skills` discovery into operator runs.
...
2. Put autonomous `Use when` and `Do not use` routing in the native description; keep the complete reusable behavior in `SKILL.md`, including concrete tools, Inputs, Method, Verification, and Failure Conditions.
3. Add `references/`, `templates/`, `scripts/`, or `assets/` when the method needs them; do not compress or drop existing behavior during a runtime migration.
4. Add or modify a `business_*` tool only for a new canonical state change, guarded side effect, provider call, receipt, budget gate, or durable runtime rail.
5. Add the reviewed skill and every bundled file to `skills/release-skills.yaml`; run `python3 scripts/build_approved_skills_manifest.py` and then `--check`. Deployment publishes that exact bundle into a content-addressed read-only standard Claude plugin. Never publish or sync it into mutable `$TAKYON_HOME/skills`.
6. Change `skills/sdk-runtime-policy.yaml` only when mode-level tool exposure or denied write paths change; authority, bootstrap/wake phase order, retries, spend settlement, validators, publication, and done gates remain runtime code, not skills.
7. Keep `skills/HANDOFF/POLICY.md` documentation-only; runtime must not read it.

For every new feature, skill, or tool, first understand when it should be used and verify that existing Takyon piping exposes it without hardcoding a deterministic workflow. Check the native description and full method, generated approved manifest, `sdk-runtime-policy.yaml`, bootstrap/wake/interactive phase policy, shell/harness metadata, and canonical tools/runtime rails.

Takyon skills must keep their full operational detail, including the real tool names and relative resources Claude needs to execute the method. Deployment destinations and mode-level authority remain higher-level policy; tenant scope, spend, publication, receipts, and validators remain enforced by guarded runtime code. A skill's method and verification must stay concrete enough to distinguish success from a fabricated or incomplete result.
...
Before adding a feature, skill, tool, store, command, metric, or prompt rule, perform a redundancy and conflict check against the active Takyon trunk. Also check initial bootstrap, scheduled wakes, manual CEO turns, work focus, test/live mode, business isolation, app-runtime rails, cron behavior, and slash-command policy. If valid, describe where the SDK discovers it through the approved native description and where runtime/mode policy limits its exact side effects.
...
Implement Takyon behavior through full native skills, reviewed SDK runtime policy, scoped tools, and durable business state. Do not add local deterministic business flows, one-off model-worker stages, hardcoded startup sequences, or UI-only command lists when a skill, harness command file, or business-scoped tool can express the behavior.
...
Default rule: everything business-facing should be an approved native skill discoverable in the immutable production plugin or a business-scoped tool exposed by the code-owned SDK runtime policy. Shared safety/control rails remain enforced in code.

Slash-command UI must be source-of-truth driven. Shell palettes/help should derive from `hermes-agent-main/plugins/takyon/harness/settings.json`, `hermes-agent-main/plugins/takyon/harness/commands/*.md`, and the approved SDK skill manifest; do not duplicate names/descriptions in UI code.
