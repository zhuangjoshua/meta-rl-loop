...
Use canonical sources of truth wherever they exist. Runtime config belongs in `$TAKYON_HOME/config.yaml`; stable agent policy belongs in `plugins/takyon/prompts/ceo.md`; portable skill methods belong in `skills/*/SKILL.md` plus `contract.yaml`; Takyon-specific tools, paths, modes, authority, publication, receipts, and validators belong in `skills/HANDOFF/`; the approved production inventory belongs in `skills/release-skills.yaml`; shell/harness command metadata belongs in `plugins/takyon/harness/settings.json` and `plugins/takyon/harness/commands/*.md`; business facts belong in the Postgres control plane plus the canonical per-business object-store workspace. On the operator host, the local materialization is cache/scratch only.
...
When changing CEO/runtime behavior, edit the stable policy at `hermes-agent-main/plugins/takyon/prompts/ceo.md`, approved skills and HANDOFF bindings under `hermes-agent-main/skills/`, and the relevant tool/cron/harness source. In particular:
...
2. Put autonomous `Use when` and `Do not use` routing in the native description; keep the body provider- and deployment-agnostic with Inputs, Method, Verification, and Failure Conditions.
3. Add `contract.yaml` with semantic capabilities and outputs.
4. Put exact tools, roots, paths, modes, authority, publication, idempotency, receipts, validators, and completion gates in `skills/HANDOFF/bindings.yaml`, never in portable skill prose.
5. Add `references/`, `templates/`, `scripts/`, or `assets/` only when the method needs them; production publishes only a reviewed non-executable allowlist.
6. Add or modify a `business_*` tool only for a new canonical state change, guarded side effect, provider call, receipt, budget gate, or durable runtime rail, then bind the semantic capability to it in HANDOFF.
7. Add the reviewed skill and exact `publish_files` to `skills/release-skills.yaml`; run `python3 scripts/build_approved_skills_manifest.py` and then `--check`. Deployment publishes that exact inventory into a content-addressed read-only plugin, and acceptance must prove its native description is discoverable there. Never publish or sync it into mutable `$TAKYON_HOME/skills`.
8. Edit the base or mode policy only when general behavior changes; bootstrap/wake phase order, retries, spend settlement, and done gates remain runtime code, not skills.

For every new feature, skill, or tool, first understand when it should be used and verify that existing Takyon piping exposes it without hardcoding a deterministic workflow. Check the native description, `contract.yaml`, HANDOFF mode/capability bindings, generated approved manifest, bootstrap/wake/interactive mode policy, shell/harness metadata, and canonical tools/runtime rails.

Takyon skills must keep rich portable operational detail without embedding Takyon tool names, paths, providers, authority, spend, or publication targets; those exact bindings belong in HANDOFF. A skill's method and verification must remain concrete enough to distinguish success from a fabricated or incomplete result.
...
Default rule: everything business-facing should be an approved native skill discoverable in the immutable production plugin or a business-scoped tool visible through HANDOFF-approved schemas/results. Shared safety/control rails remain enforced in code.
