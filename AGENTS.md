...
- **Production model selection is strict: CEO = OpenAI `gpt-5.5`; coding worker = DeepSeek `deepseek-v4-pro`.** The tracked dashboard/worker units and Mac production rail must pin those roles. A requested model is forwarded unchanged for its entire run: no provider fallback chain, no caller override of the worker pin, no missing-config default, and no Safebox model rewrite (including `TAKYON_ANTHROPIC_MODEL_REWRITE`). Provider/model failure must surface as the exact blocker instead of substituting another model.
- **Switching the runtime to a pricier model tier requires raising the per-call cost ceilings, or spendful calls fail closed.** The safebox reserve estimate bills `estimated_input + max_tokens` at the model's FULL price, so a Sonnet-sized ceiling trips `estimate_exceeds_ceiling` (HTTP 402/503) on a top-tier model. The ceilings default to the Sonnet-era values and are env-overridable (defaults unchanged): `TAKYON_CLAUDE_AGENT_BUDGET_DEFAULT_USD` / `TAKYON_CLAUDE_AGENT_BUDGET_MAX_USD` (coding-worker budget clamp), `TAKYON_CLAUDE_AGENT_BROKER_MAX_COST_MICROUSD` (coding-broker per-call), `TAKYON_OPERATOR_SESSION_MAX_COST_MICROUSD` (CEO-turn per-call). Set the model with `TAKYON_CLAUDE_AGENT_MODEL` (coding worker + intake) and `model.default` / `model.claude_agent_default` in `config.yaml` (CEO turns); `TAKYON_MODEL` in the process env beats config and is immune to a concurrent config revert. `agent/usage_pricing.py` must have a pricing entry for the model or every metered call refuses (unpriced = fail-closed, by design).
