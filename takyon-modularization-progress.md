...
**Stage 1 — 🟡 STARTED 2026-07-02.** Q1 posture: the plan's own recommendation ("Stages 0–1 are safe to start now — behavior-preserving") + operator re-issued the implement directive. Discipline for the live tree (another session actively commits to `worker.py`/`core.py`; a live prod-draining worker runs from this tree): all Stage-1 edits in an **isolated worktree off origin/main**, small commits, rebase-before-land, never stash.

**Live-system baseline OBSERVED DIRECTLY (not a test), per your "observe directly" rule:** `scripts/takyon-operator-prod.sh status` → safebox + dashboard tunnels healthy, a live Mac operator worker (PID 15286) draining prod, VPS worker `disabled/inactive`. This directly confirms the plan's premise that operator capacity is the local Mac pool over SSH tunnels + safebox, not the VPS.

**Nothing is deployed. Nothing on the hot files (`worker.py`/`jobs.py`/`core.py`) has been edited.**

---

## Decisions still needed from you (⛔ = gates real work)

- ⛔ **Q1 — sequence vs the active MVP bootstrap goal.** Stage 1+ edits `worker.py`/`jobs.py`/`core.py`; the MVP goal edits the same files on this shared tree. Serialize (recommended: pause the goal, or run this after), or interleave carefully? **I will not mutate those files until you answer.**
...
### Stage 1 — extract WorkerPool, converge launch paths · 🟡 BUILT in worktree, not yet landed/deployed
_Code complete on branch `stage1-workerpool` (worktree `/Users/Zygote/Downloads/takyon-stage1-workerpool`, commits f2e4e64a Stage-0 nets + 198529e9 Stage-1). NOT checked ✅ until landed on main + deployed + live-E2E-proven — the checkbox rule._
...
- 🟡 worker→cli inversion FIXED: the 37 helpers worker.py lazily imported from the interactive shell module moved verbatim (733 loc) to `turn_runtime.py`; cli.py re-exports (shim per §5). worker.py imports ZERO cli names. New AST layering guard (`test_takyon_worker_layering_guard.py`) pins worker/worker_pool/turn_runtime never import cli — lazy imports included.
...
- **Remaining for ✅:** live CLI E2E on Stage-1 code (fresh business via real `console` + `/create`, PYTHONPATH-pinned) + land on main + deploy both hosts per the agents.md rail (push main AND rsync both hosts + restart — GH runner can't SSH the VPS, so rsync is the activation path) + fresh-business browser E2E. Gated on: the other session's live bootstrap (`nutrientdaily0702c`) still draining on the shared local pool — my console start would kill it mid-flight (debounced monitor watching).
