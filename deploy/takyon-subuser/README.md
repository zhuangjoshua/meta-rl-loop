...
The Caddyfile uses `rate_limit` for origin-side abuse control on the shared product plane
(a global per-IP cap plus tighter per-IP zones on product-app `auth/*`, legacy
`auth/request`, and `checkout`). Stock apt `caddy` lacks that module, so
