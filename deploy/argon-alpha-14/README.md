...
- shared dynamic `slug.coscale.app` product hosts, terminated at the operator edge and proxied over the private VPC to the sub-user host on `10.116.0.3:80`

Do not add a Caddy block per new normal business here. Shared dynamic
`slug.coscale.app` product routing is decided by the product-host matcher
