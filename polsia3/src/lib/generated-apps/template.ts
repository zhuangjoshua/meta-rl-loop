...
export async function writeGeneratedAppTemplate(rootDir: string, config: GeneratedAppTemplateConfig) {
  ...
  await fs.mkdir(path.join(rootDir, "src", "app", "account"), { recursive: true });
  ...
  files.set("src/app/account/page.tsx", accountPageTsx(config));
...
function homePageTsx(config: GeneratedAppTemplateConfig) {
  return `import Link from "next/link";\n\nconst businessName = ${escapeText(config.company.name)};\n\nexport default function HomePage() {\n  return (\n    <main className="surface-root rails-placeholder">\n      <nav className="topbar compact" aria-label="Main navigation">\n        <Link className="brand-lockup" href="/">\n          <span className="brand-mark" aria-hidden />\n          <span>{businessName}</span>\n        </Link>\n        <div className="nav-links">\n          <Link href="/product">Product</Link>\n          <Link href="/signup">Sign up</Link>\n        </div>\n      </nav>\n      <section className="placeholder-panel" aria-label="Surface scaffold">\n        <p className="eyebrow">Rails scaffold</p>\n        <h1>Customer surface pending</h1>\n        <p>This compile-only placeholder must be replaced by the generated-app surface builder before deployment is marked published.</p>\n      </section>\n    </main>\n  );\n}\n`;
}
...
function accountPageTsx(config: GeneratedAppTemplateConfig) {
  return `"use client";\n\nimport { useEffect, useState } from "react";\nimport Link from "next/link";\nimport { generatedAppBillingPortalUrl, loadCustomerAccount } from "@/lib/platform-client";\n\nconst businessName = ${escapeText(config.company.name)};\n\ntype AccountPayload = {\n  ok?: boolean;\n  error?: string;\n  account?: {\n    effectiveTier?: string;\n    billingPortalAvailable?: boolean;\n    user?: { email?: string; status?: string; tier?: string };\n    plan?: { plan_key?: string; included_action_quota?: number; price_usd_cents?: number } | null;\n    usageThisPeriod?: { actionCount?: number };\n    revenue?: { totalPaidDisplay?: string };\n    entitlements?: Array<{ id: string; tier: string; status: string; source: string; current_period_end?: string | null }>;\n  };\n};\n\nfunction money(cents?: number) {\n  return typeof cents === "number" ? \`$\${(cents / 100).toFixed(2)}\` : "$0.00";\n}\n\nexport default function AccountPage() {\n  const [payload, setPayload] = useState<AccountPayload | null>(null);\n  const [loading, setLoading] = useState(true);\n\n  useEffect(() => {\n    let cancelled = false;\n    loadCustomerAccount()\n      .then((next) => {\n        if (!cancelled) setPayload(next);\n      })\n      .catch((error) => {\n        if (!cancelled) setPayload({ ok: false, error: error instanceof Error ? error.message : "Account is not available." });\n      })\n      .finally(() => {\n        if (!cancelled) setLoading(false);\n      });\n    return () => {\n      cancelled = true;\n    };\n  }, []);\n\n  const account = payload?.account;\n  const plan = account?.plan;\n\n  return (\n    <main className="surface-root account-page">\n      <nav className="topbar compact" aria-label="Account navigation">\n        <Link className="brand-lockup" href="/">\n          <span className="brand-mark" aria-hidden />\n          <span>{businessName}</span>\n        </Link>\n        <div className="nav-links">\n          <Link href="/">Home</Link>\n          <Link href="/product">Product</Link>\n          <Link href="/signup">Sign up</Link>\n        </div>\n      </nav>\n\n      <section className="account-layout">\n        <div className="account-copy">\n          <p className="eyebrow">Account</p>\n          <h1>{loading ? "Loading account" : account?.user?.email || "Sign in required"}</h1>\n          <p>{account ? \`Plan: \${plan?.plan_key || account.effectiveTier || "free"}\` : payload?.error || "Use a magic link to view account and billing details."}</p>\n          {account?.billingPortalAvailable ? <a className="button primary" href={generatedAppBillingPortalUrl()}>Billing portal</a> : <Link className="button primary" href="/signup">Start or upgrade</Link>}\n        </div>\n\n        <div className="account-grid">\n          <article>\n            <span>Tier</span>\n            <strong>{account?.effectiveTier || "free"}</strong>\n          </article>\n          <article>\n            <span>Usage</span>\n            <strong>{account?.usageThisPeriod?.actionCount ?? 0}/{plan?.included_action_quota ?? 0}</strong>\n          </article>\n          <article>\n            <span>Price</span>\n            <strong>{money(plan?.price_usd_cents)}</strong>\n          </article>\n          <article>\n            <span>Paid</span>\n            <strong>{account?.revenue?.totalPaidDisplay || "$0.00"}</strong>\n          </article>\n        </div>\n\n        <section className="result-panel account-entitlements" aria-live="polite">\n          <div className="panel-header">\n            <span>Entitlements</span>\n            <strong>{account?.entitlements?.length ?? 0}</strong>\n          </div>\n          {account?.entitlements?.length ? account.entitlements.map((row) => (\n            <p key={row.id}>{row.source} / {row.tier} / {row.status}{row.current_period_end ? \` through \${row.current_period_end.slice(0, 10)}\` : ""}</p>\n          )) : <p>No entitlement rows are visible for this session.</p>}\n        </section>\n      </section>\n    </main>\n  );\n}\n`;
}
